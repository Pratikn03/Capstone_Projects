...
CRASH_KEYWORDS = frozenset([
    "collision", "crash", "accident", "hit", "rear-end",
    "sideswipe", "near-miss", "near miss", "emergency",
    "hard brake", "hard-brake", "evasive",
])
...
def _label_crash_scenarios(
    ...
    if "accel_mps2" in ego_df.columns:
        ...
        if min_accel < HARD_BRAKE_MS2:
            ...
            labels["hard_brake_steps"] = ego_df.index[
                ego_df["accel_mps2"] < HARD_BRAKE_MS2
            ].tolist()
    ...
    if lead_gap.notna().any():
        ...
        if min_gap < NEAR_COLLISION_GAP_M:
            ...
            labels["near_collision_steps"] = ego_df.index[
                lead_gap < NEAR_COLLISION_GAP_M
            ].tolist()

    labels["has_crash_keyword"] = _scene_has_crash_keyword(scene_description)

    labels["is_crash_scenario"] = (
        labels["has_hard_brake"]
        or labels["has_near_collision"]
        or labels["has_crash_keyword"]
    )
    return labels
...
def bridge_nuscenes(
    ...
    for scene_idx, scene in enumerate(nusc.scene):
        ...
        for _, row in ego_df.iterrows():
            orius_row: dict[str, Any] = {
                ...
                "ts_utc": pd.Timestamp(row["timestamp_us"], unit="us", tz="UTC").isoformat().replace("+00:00", "Z"),
            ...
            for inst_token, inst_df in tracks_df.groupby("instance_token"):
                ...
                for _, r in inst_df.iterrows():
                    all_rows.append({
                        ...
                        "speed_limit_mps": SPEED_LIMIT_DEFAULT_MPS,
                        "lead_position_m": "",
                        "ts_utc": pd.Timestamp(r["timestamp_us"], unit="us", tz="UTC").isoformat().replace("+00:00", "Z"),
                        "source_split": "nuscenes_" + version.replace("v1.0-", ""),
                        "scene_name": scene_name,
                        "is_crash_scenario": labels["is_crash_scenario"],
                        "accel_mps2": 0.0,
    ...
    print(f"\n{'=' * 70}")
    print(f"nuScenes → ORIUS Bridge Complete")
    print(f"{'=' * 70}")
    print(f"  Version:        {version}")
    print(f"  Scenes:         {n_total} total, {n_crash} crash/near-crash ({100*n_crash/max(n_total,1):.0f}%)")
...
def main() -> int:
    ...
    if not args.dataroot.exists():
        ...
        print(f"  1. Create account at https://www.nuscenes.org/download")
        print(f"  2. Download v1.0-mini (or v1.0-trainval for full)")
        print(f"  3. Extract to a directory, e.g. /data/sets/nuscenes/")
        print(f"  4. Re-run with --dataroot /data/sets/nuscenes")
        return 1
