...
def main() -> None:
    p = argparse.ArgumentParser(description='Run reliability group audit wrapper')
    p.add_argument('--input-trace', default='reports/publication/48h_trace_final_de.csv')
    ...
    rows, summary = build_summary(inp, y_true_col='y_true', y_pred_col='y_pred', reliability_col='reliability_w', lower_col='lower', upper_col='upper', alpha=0.10, n_bins=6, min_bin_size=8, binning='quantile')
    rows.to_csv(out/'reliability_group_audit.csv', index=False)
    (out/'reliability_group_audit.json').write_text(__import__('json').dumps(summary, indent=2) + '\n', encoding='utf-8')
