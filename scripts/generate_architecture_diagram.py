...
def _box(ax, x, y, w, h, lines, *, fc="#FFF", ec="#94A3B8", fs: float = 8.5, bold_first=True, lw=1.3):
...
def _pill(ax, cx, cy, label, *, fc=DOMAIN_FC, ec=DOMAIN_EC, fs: float = 7.8, pw=0.100, ph=0.028):
    ax.add_patch(
        FancyBboxPatch(
            (cx - pw / 2, cy - ph / 2),
            pw,
    ...
    ax.text(cx, cy, label, ha="center", va="center", fontsize=fs, fontweight="bold", color=TEXT, zorder=4)
...
def main() -> None:
    ...
    s4 = _box(
        ...
        SW,
        SH,
        ["\u2463  SHIELD  (Repair)", "L2 project  a\u2217 \u2192 A\u209c", "domain adapter repair()"],
        fc=STAGE_FC,
        ec=STAGE_EC,
        fs=8.5,
        lw=1.8,
    ...
    s5 = _box(
        ...
        SW,
        SH,
        ["\u2464  CERTIFY  (CERTos)", "hash-linked certificate", "SHA-256 chain \u00b7 DuckDB audit"],
        fc=CERTIFY_FC,
        ec=CERT_EC,
        fs=8.5,
        lw=1.8,
    ...
    _arr(
        ax,
        (pill_x0 + pill_total_w / 2, DOMAIN_CY + pill_h / 2),
        (_cx(s5), BOT_ROW_Y),
        lw=1.0,
        ls="--",
        color=DOMAIN_EC,
    ...
    # =========================================================================
    leg_y = 0.060
    ax.plot([0.03, 0.97], [leg_y + 0.026, leg_y + 0.026], color=LANE_EDGE, lw=0.6, zorder=0)
    ...
    for lx, fc, ec, lbl in items:
        ...
        ax.text(lx + sq + 0.005, leg_y + 0.013, lbl, fontsize=7.5, color=TEXT, va="center")
