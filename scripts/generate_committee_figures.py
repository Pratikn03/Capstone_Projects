...
def make_domain_comparison() -> None:
    ...
    ax.bar(x - width / 2, base, width, label="Baseline (no DC3S)", color=COLORS["baseline"], alpha=0.8)
    ax.bar(
        x + width / 2,
        orius,
        width,
    ...
    ax.axhline(0.25, ls="--", color=COLORS["target"], lw=1.2, label="25% reduction target")
...
def make_calibration_curve() -> None:
    ...
    for i, (dom, vals) in enumerate(empirical.items()):
        ax.plot(nominal, vals, marker="o", markersize=5, lw=1.4, color=cmap(i), label=dom)
...
def make_per_domain_fault_breakdown() -> None:
    ...
    fig.suptitle("Per-Domain Fault-Type TSVR: Nominal vs DC3S (5 seeds × 48 steps)", fontsize=9)
