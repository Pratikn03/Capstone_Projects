...
def _load_test() -> dict[str, np.ndarray]:
    ...
    return {k: arr[k] for k in arr.keys()}
...
def _load_model():
    return joblib.load(RUN_DIR / f"models/gbm_lightgbm_{TARGET}.pkl")
...
def _compute_wt(signal: np.ndarray, dropout_p: float = 0.15,
                spike_p: float = 0.08, window: int = 10):
...
def fig_model_comparison() -> None:
    ...
    persist = np.roll(y_true, 1); persist[0] = y_true[0]
    ...
    for ax, (met, vals) in zip(axes, metrics.items()):
...
def fig_drift_sample() -> None:
    ...
    in_fault = False; f_start = 0
    for i in range(n):
        if fault_mask[i] and not in_fault:
            f_start = i; in_fault = True
        elif not fault_mask[i] and in_fault:
            ax1.axvspan(f_start, i, alpha=0.15, color=FAULT_COLOR); in_fault = False
    if in_fault:
...
def fig_multi_horizon_backtest() -> None:
    ...
    for h_idx in range(len(horizons)):
        ...
        gbm_rmse.append(_rmse(yt, yp));   gbm_mae.append(_mae(yt, yp));   gbm_smape.append(_smape(yt, yp))
        pers_rmse.append(_rmse(yt, yp0)); pers_mae.append(_mae(yt, yp0)); pers_smape.append(_smape(yt, yp0))
    ...
    for ax, (met, gbm_ys, pers_ys) in zip(axes, metrics):
    ...
    fig.suptitle(f"{DISPLAY} — Multi-Horizon Backtest (DE load\_mw)", fontsize=9)
    plt.tight_layout()
