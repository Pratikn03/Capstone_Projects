...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train ORIUS models on German and US energy datasets")
    parser.add_argument(
        "--ba", default="MISO", help="EIA-930 Balancing Authority code (e.g., MISO, ERCOT, PJM)"
    )
    parser.add_argument("--start", default=None, help="Start date for EIA-930 data filtering (YYYY-MM-DD)")
    parser.add_argument("--end", default=None, help="End date for EIA-930 data filtering (YYYY-MM-DD)")
    ...
    if not Path("data/processed/features.parquet").exists():
        ...
        run(
            [
                "python",
                "-m",
                "orius.data_pipeline.build_features",
                "--in",
                "data/raw",
                "--out",
                "data/processed",
    ...
    run(
        [
            ...
            "--out",
            "data/processed/us_eia930/splits",
        ]
    )
    ...
    run(["python", "-m", "orius.forecasting.train", "--config", "configs/train_forecast.yaml"])

    # Train on US dataset (load, solar, wind targets for the BA)
    run(["python", "-m", "orius.forecasting.train", "--config", "configs/train_forecast_eia930.yaml"])
