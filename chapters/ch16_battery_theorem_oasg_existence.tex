...
Under Assumptions A1, A2, A4, A11, and A12, for any dropout rate $d > 0$ and
any optimal arbitrage controller satisfying the battery boundary-reachability
witness, there exists an admissible degraded-observation realization and a time
step $t^*$ such that
\[
...
observed state but violates a constraint in the true state.  This is an
existence witness, not a claim that every degraded step or every controller
produces an OASG.
\end{theorem}
...
co-occurrence condition in A12, there is an admissible degraded-observation
realization with an observation gap larger than the remaining boundary slack at
