...
piece of a five-result theory arc rather than as a request to promote every
numbered extension.  T11 is the main transfer theorem.  The
T10/T11 observation-ambiguity sandwich is the supporting optimality corollary.
T9 and T10 are supporting lower-bound extensions after the theorem-promotion
gates discharge their stronger constants and domain assumptions for the locked
Battery, AV, and Healthcare artifacts.
...
domain-discharge artifacts: T9 frames the structural risk of ignoring
reliability, and T10 records a boundary-indistinguishability frontier for the
stated binary testing model.  T11 then identifies the obligations under which
...
single-domain accident; T9 records the corresponding assumption-qualified
extension under explicit mixing and boundary-reachability assumptions.  The
locked Battery, AV, and Healthcare artifacts discharge these constants through
the promotion evidence files \texttt{T9\_battery.json}, \texttt{T9\_av.json},
and \texttt{T9\_healthcare.json}.
...
By Assumption~A10b (Chapter~\ref{ch:assumptions}), the fault injection
...
on the boundary-reachability conditions of Assumption~A11.
...
T9 is a supporting defended necessity extension for the universal kernel:
under the explicit A10b/A11 hypotheses, and under the locked three-domain
mixing and witness artifacts, it records why a controller that ignores
reliability cannot remove the violation scaling by algorithmic tuning alone.
Broader portability still requires rerunning the same discharge builder on the
new domain rather than inheriting the Battery/AV/Healthcare constants.
...
The locked T10 evidence files discharge the boundary-mass, paired
safe/unsafe observation-law, and TV-bridge assumptions for Battery, AV, and
Healthcare.  In the Battery case, the unsafe-side law is supplied by the
locked 48-hour boundary-violation trace and transformed into the same margin
units as the runtime trace.
...
\begin{enumerate}
  ...
  \item every unsafe-side mistaken release has OASG severity at least
    $\delta_t \ge 0$.
\end{enumerate}
...
\]
Moreover, the severity-weighted OASG satisfies
\[
  \mathbb{E}\!\left[\mathrm{OASG}_T(\pi)\right]
  \;\ge\;
...
true-state violation and at least $\delta_t$ severity-weighted OASG, so
\[
  \Prob[Z_t = 1]
  \;\ge\;
...
\[
  ...
  \frac{1}{2}\sum_{t=1}^T p_t(1-w_t).
\]
Multiplying the same per-step event by $\delta_t$ and summing gives
\[
...
without a domain-specific argument that justifies the boundary-mass and
severity sequences $p_t,\delta_t$.
\end{proof}

\begin{remark}
T10 is an assumption-qualified supporting extension.  It is an information-theoretic lower bound
...
pure artifact of one repair heuristic.  In its OASG form, the theorem states
the explicit rate: the unavoidable gap grows linearly with the degradation
budget $\sum_t(1-w_t)$, scaled by boundary mass and minimum violation severity.
\end{remark}
