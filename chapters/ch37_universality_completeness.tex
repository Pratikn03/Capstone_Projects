...
ladder.  T9 and T10 are draft/scoped extensions under explicit assumptions:
T9 frames the structural risk of ignoring reliability, and T10 records a
boundary-indistinguishability frontier for the stated binary testing model.
T11 then identifies the obligations under which the same repair-and-bound
...
single-domain accident; T9 records the corresponding draft/scoped extension
under explicit mixing and boundary-reachability assumptions.
...
T9 is a draft/scoped necessity extension for the universal kernel: under the
explicit A10b/A11 hypotheses, it records why a controller that ignores
reliability cannot remove the violation scaling by algorithmic tuning alone.
The defended evidence remains tiered.  Battery anchors the proof lineage
directly; healthcare and bounded autonomous-vehicle replay support the same
runtime story under defended contracts; broader portability remains outside
the active proof-validating surface.
...
\begin{remark}
T10 is a draft/scoped but substantive extension.  It is an information-theoretic lower bound
...
\begin{enumerate}
  \item \textbf{T9} is a draft/scoped extension that frames observation-aware
    safety logic as structurally necessary under persistent degradation.
  \item \textbf{T10} is a draft/scoped extension that turns mean reliability
    into a quantitative infrastructure-to-risk frontier under explicit binary
    testing hypotheses.
...
as draft/scoped extensions, record necessity and lower-bound directions under
explicit assumptions.  T11 shows how the same kernel transfers when a new
domain discharges the explicit structural obligations.  This is a defensible
control-theory claim; it is not an assertion that all evaluated domains
already enjoy equal proof closure.
