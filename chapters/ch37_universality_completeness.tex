...
can separate three things cleanly: what the battery proof already establishes,
...
\begin{theorem}[Stylized Reliability-Risk Frontier, T10]
...
\begin{remark}
T10 is intentionally conservative.  It does \emph{not} claim that the
battery upper bound is globally minimax without further work; it isolates the
mechanism by which degraded observation creates a nontrivial reliability-risk
frontier.  The constant $1/4$ is therefore read as a stylized frontier
witness, not as a universally sharp asymptotic constant.
\end{remark}
...
\begin{theorem}[One-step structural transfer theorem, T11]
...
one-step safety-preservation statement as the battery reference theorem:
\[
...
\]
When a particular domain instantiation justifies the battery-style envelope
...
\[
  ...
  \sum_{t=1}^T r_t.
\]
The battery-style envelope is the special case $r_t \le \alpha(1-w_t)$.
\end{proof}
...
one-step obligations, and the stronger episode-level claim appears only when a
domain-specific per-step risk budget is justified explicitly.
