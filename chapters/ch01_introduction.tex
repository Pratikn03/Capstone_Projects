...
\begin{enumerate}
  ...
  \item \textbf{Extension results.}  Certificate half-life analysis
    ...
    (Chapter~\ref{ch:compositional-safety-fleets}), and adversarial
    robustness (Chapter~\ref{ch:adversarial-robustness}).
\end{enumerate}
...
\begin{description}
  ...
  \item[Chapters~5--12] develop the DC3S pipeline components, prove the
    coverage theorem, and present ablation studies.
  \item[Chapters~\ref{ch:cert-half-life-blackout}--\ref{ch:adversarial-robustness}]
    extend the framework to certificate temporal validity, graceful
    degradation, fleet composition, and adversarial robustness.
