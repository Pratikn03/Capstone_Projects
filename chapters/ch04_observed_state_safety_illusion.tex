...
\begin{theorem}[Informal safety guarantee of DC3S]
\label{thm:dc3s-informal}
If the OQE score~$w_t$ is a lower bound on the true reliability (i.e.,
$w_t \le I(X_t; \hat{X}_t) / H(X_t)$ in a normalised sense), and the
conformal calibration set satisfies approximate exchangeability, then the
DC3S shield achieves
\[
  \mathrm{TSVR} \;\le\; \alpha + o(1),
\]
where $\alpha$ is the conformal miscoverage level and the $o(1)$ term
vanishes as the calibration set grows.
\end{theorem}
