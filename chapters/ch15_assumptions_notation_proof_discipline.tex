...
disciplinary standards that every subsequent proof must satisfy.  The point of
the chapter is not to enlarge the formal inventory for its own sake; it is to
make the later results readable as one coherent theorem program.  The
flagship DC3S ladder in Chapters~\ref{ch:theorem-oasg}--\ref{ch:temporal-extensions}
contains the battery reference theorems (T1--T8), but the full dissertation
surface is broader: Part~I contributes the precursor results, the later
chapters add lemmas, propositions, and corollaries, and the monograph
extension closes with T9--T11.  Counting the active master-manuscript include
tree yields \textbf{83 formal items} (26~theorems, 5~lemmas,
9~propositions, 12~corollaries, 15~definitions, 16~assumptions), all audited in
...
spelled out explicitly as an extra theorem-local hypothesis.  In
particular, later chapter statements do not identify $w_t$ with the
probability of clean telemetry unless the statement itself introduces a
calibration or indistinguishability contract that justifies that move.
...
\label{assm:A6}
The OQE and adjacent runtime detectors recognize supported telemetry faults
within channel-specific finite lags.  The active detector-lag contract is
\[
  \tau_{\max}(\text{dropout}) = 1,\;
  \tau_{\max}(\text{stale}) = 3,\;
  \tau_{\max}(\text{delay}) = 2,\;
  \tau_{\max}(\text{OOO}) = 10,\;
  \tau_{\max}(\text{spike}) = 1,\;
...
\]
If a fault begins at step $t_f$, then the corresponding reliability penalty
or dedicated detector flag must fire by step $t_f + \tau_{\max}$.
\end{assumption}
...
These assumptions are not merely declarative.  Their runtime realization is
concentrated in the typed ORIUS kernel, whose implementation surface records
observation packets, reliability assessments, observation-consistent state
sets, safe-action sets, repair decisions, and safety certificates.  The
battery-domain proofs remain the reference instantiation, but the reusable
universal claim now attaches to that typed kernel rather than to an informal
adapter narrative.

\IfFileExists{reports/publication/assumption_code_traceability.tex}{%
  \input{reports/publication/assumption_code_traceability}%
}{%
  \input{../reports/publication/assumption_code_traceability}%
}
...
\begin{table}[htbp]
\centering
\caption{Assumption and local-contract dependencies of the active T1--T11 audit surface.  Only a strict subset is defended as flagship or supporting in the June package.}
...
T4 No Free Safety & & \checkmark & & & & & & & & & Fixed-margin, quality-ignorant controller class and admissible degraded-observation witness \\
T5 Certificate Horizon & \checkmark & \checkmark & & \checkmark & \checkmark & & \checkmark & & \checkmark & & Forward-tube containment construction \\
T6 Certificate Expiration Bound & & & & \checkmark & \checkmark & & \checkmark & & \checkmark & & Drift-scale lower-bound argument for certificate expiration \\
T7 Feasible Fallback Existence & \checkmark & & \checkmark & \checkmark & & & & & & \checkmark & Interior-state fallback construction \\
...
The dependency matrix above covers the active T1--T11 audit surface, not a
flat defended ladder.  The registry-canonical defended core is smaller:
T1, T2, T3a, T4, T11, and T\textsubscript{trajectory\_PAC} carry the
flagship spine; T3b, T6, T8, T11\textsubscript{Byz}, and
T\textsubscript{stale} remain supporting defended results; the remaining rows
stay visible only as draft or scoped extension surfaces.  The monograph-level
extension adds T9--T11, each of which answers a distinct control-theory
question and introduces an explicit new obligation.
...
\midrule
T9 & Is reliability-aware repair structurally necessary under persistent degradation? & A6$'$ + boundary reachability witness & Necessity theorem for degraded-observation control \\
\addlinespace
T10 & When does degraded observation force a nontrivial reliability-risk frontier? & Explicit boundary prior and observation indistinguishability assumptions & Stylized lower-bound frontier theorem \\
\addlinespace
T11 & Which structural obligations let the battery safety argument transfer to a new domain? & Explicit soundness, repair-membership, and fallback obligations & One-step structural transfer theorem \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Gap-Closure Map}

The theorem program is easier to audit if each thesis-level gap claim is tied
to the theorem that closes it and to the residual boundary that still
remains.  Table~\ref{tab:gap-closure-map} makes that dependency explicit.

\begin{table}[htbp]
\centering
\caption{From gap statement to theorem closure and current claim boundary.}
\label{tab:gap-closure-map}
\begin{tabular}{p{3.0cm}p{2.3cm}p{3.1cm}p{3.6cm}}
\toprule
\textbf{Gap statement} & \textbf{Closing result} & \textbf{Formal object introduced} & \textbf{Current status} \\
\midrule
Observed-state safety can disagree with true-state safety. & S1, T1, T2 & Observation-consistent state set and repaired safe action & Closed on the battery reference surface. \\
\addlinespace
Ignoring reliability may still look operationally attractive. & T4, T9 & Necessity of reliability-aware repair under persistent degradation & Closed as a structural impossibility result; battery remains the witness surface. \\
\addlinespace
The $\alpha(1-\bar{w})T$ envelope may be only a loose upper bound. & T3, T10 & Reliability-risk frontier under explicit boundary-indistinguishability assumptions & Closed conservatively on a stylized frontier model; full sharpness remains battery-anchored. \\
\addlinespace
Cross-domain portability could be mistaken for equal proof depth. & T11 + tier tables & Explicit one-step transfer obligations and tiered discharge rules & Closed at the contract level; empirical discharge remains tiered by domain. \\
\addlinespace
The bounded AV contract could be misread as a contradiction of the defended core. & Universality-completeness chapter (constraint-class mismatch analysis) & Constraint-class mismatch diagnosis & Open promotion gap, but not a contradiction of the defended core. \\
\addlinespace
AV and Healthcare could be over-read as proof peers of the battery witness. & Claim-boundary discipline in Chapter~\ref{ch:battery-to-orius} & Evidence-tier interpretation & Bounded defended peers, not equal-depth witness proofs. \\
\bottomrule
\end{tabular}
\end{table}
...
The battery reference ladder uses monotone inflation to preserve the base
coverage surface.  The universal transfer theorem (T11) is kept at the
one-step level for exactly this reason: it does not silently upgrade
marginal coverage into a stronger episode-level conditioning claim.

\section{Theorem-Local Calibration Contracts}
\label{sec:theorem-local-calibration-contracts}
...
\begin{enumerate}
  \item \textbf{Risk-envelope contract.}  A chapter may introduce a
    ...
    This is the correct bridge from one-step conditional reasoning to an
    episode-level expectation bound.  The battery-style envelope
    $r_t \le \alpha(1-w_t)$ is one such contract, but it is not claimed to
    follow from vanilla conformal exchangeability alone.
...
These contracts are local on purpose.  They keep the theorem program honest:
coverage language stays marginal unless a stronger conditioning statement is
written into the theorem, and reliability language stays operational unless a
calibration or indistinguishability hypothesis is stated alongside it.
...
Multi-step claims (certificate horizon, graceful degradation, and any
episode-level transfer bound) require additional assumptions about temporal
dependence or explicit per-step risk budgets; these are stated explicitly in
Chapter~\ref{ch:temporal-extensions} and in the aggregation corollaries that
follow the relevant one-step theorems.
...
formal surface of the dissertation.  That count now includes the
foundational precursor results (S1--S2), the battery proof ladder (T1--T8),
and the monograph extension theorems (T9--T11).  The integrated theorem gate
still enforces the same minimum standard for every item: source file exists,
code anchor verified, artifact locked when applicable, and appendix mapping
confirmed.
...
assumptions (A1--A13) are individually justified in both domain-general
