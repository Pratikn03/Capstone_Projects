...
\section{Impossibility of Mandatory Release (T9)}
\label{sec:t9-ch19}

T4 establishes necessity for one battery witness sequence.  The active
universality program's T9 statement is no longer the historical
\(\Omega(dT)\) claim.  It is the empty-safe-core impossibility theorem: for an
observation \(o\), if the ambiguity class \(B(o)=\{x:h(x)=o\}\) is nonempty and
its common safe core \(K(o)=\cap_{x\in B(o)}C(x)\) is empty, then no total
observation-only mandatory-release controller can certify safety for every
latent state in that class.

This makes explicit what T4 implies structurally: quality-aware uncertainty
expansion, fallback, or denial is not merely a design convenience on the
witness class; it is the way ORIUS leaves the mandatory-release lower-bound
class.  Any future rate theorem over repeated fault windows must be promoted as
a separate result with its own mixing assumptions and artifacts.
