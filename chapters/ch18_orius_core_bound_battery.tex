...
$r_t \le \alpha(1-w_t)$ on the fixed certified budget trace.  Then the expected
...
If the reliability process is left random rather than conditioned on the
recorded budget trace, the corresponding defensible statement is
$\mathbb{E}[V]\le\alpha\sum_t\mathbb{E}[1-w_t]$.
\end{theorem}
...
Under Assumptions A1, A2, A4, A5, A6, and A7, the expected total violation
count under the DC3S shield on the fixed certified budget trace satisfies
\[
  \mathbb{E}[V] \;\le\; \alpha\,(1 - \bar{w})\,T,
\]
...
T3 establishes that DC3S achieves $\mathbb{E}[V] \le \alpha(1-\bar{w})T$ once
the domain supplies the predictable risk budget.  The monograph's active T10
lower-bound result is narrower: for a two-state boundary pair with observation
laws within TV distance \(\epsilon\) and disjoint safe-action sets, every
observation-only mandatory-release controller has worst-state release risk at
least \((1-\epsilon)/2\).  This is a local lower-bound primitive, not a
horizon-summed frontier and not an identification of \(w_t\) with TV distance.

The reliability-safety frontier plots are therefore interpreted as
domain-specific diagnostics that plug a modeled \(\epsilon(w_t)\) into T10 and
compare it with the conservative T3 upper envelope.  Any stronger coefficient
or matching minimax claim would require a separate theorem over the same policy
and distribution class.

The conservative upper envelope gives the minimum infrastructure reliability
suggested by the T3 budget for a target violation rate:
\[
