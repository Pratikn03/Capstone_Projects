...
\begin{enumerate}
  ...
  \item \textbf{Certificate half-life.}  The blackout study
    (Chapter~\ref{ch:cert-half-life-blackout}) characterises certificate
    decay over a 96-hour horizon.  Longer horizons (weeks, months) are
...
domain.  These claims are \emph{plausible by construction} but not yet
validated by experiment in a second domain:

\begin{enumerate}
  \item \textbf{Multi-domain portability.}  Chapter~\ref{ch:battery-to-orius}
    argues that the adapter pattern generalises.  No non-battery adapter has
  ...
  \item \textbf{Adversarial completeness.}  Active probing
    (Chapter~\ref{ch:adversarial-robustness}) detects the tested spoofing
    attack.  Completeness against all possible adversarial strategies is
    not claimed and likely unachievable in general.
\end{enumerate}
