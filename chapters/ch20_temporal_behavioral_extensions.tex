...
safety--work preorder. No global fallback optimality or absorbing-landing
dynamics claim is included in this theorem.
\end{theorem}

\begin{proof}
Summing \(Z_t^G\le Z_t^U\) over the shared fault trace gives
\(\sum_t Z_t^G\le\sum_t Z_t^U\), the safety coordinate of the preorder.  The
hypothesis \(W_G\ge\lambda W_U\) is the useful-work coordinate.  Immediate
shutdown can be safe, but it is not a T8 witness unless it preserves the same
declared useful-work fraction.
\end{proof}
