...
\begin{enumerate}
  ...
  \item \textbf{SPCI-lite online quantile} (Chapter~\ref{ch:dc3s-adapter}):
    an exponentially-weighted online quantile tracker following
    Gibbs \& Candès~\cite{gibbs2021adaptive} maintains a sliding window
    of conformity scores with weights
    $w_i = \exp\!\bigl(-\lambda(t-i)\bigr)$.  The $(1-\alpha)$-quantile
    is computed as
    $\hat{q}_t = \inf\!\bigl\{q : \textstyle\sum_i w_i\,\mathbb{1}[s_i \le q]
    \ge 1-\alpha\bigr\}$,
    yielding intervals that adapt smoothly to distribution shift without
    requiring a full recalibration pass.  The {\texttt{SPCIUpdater}} class
    in {\texttt{conformal.py}} implements this with configurable
    $\lambda$, window size, and $\alpha$.
\end{enumerate}
...
\paragraph{Mondrian finite-sample caveat (R2 audit response).}
When stratified CQR is used, each reliability bin receives only
$n_k = n_{\mathrm{cal}} / K$ samples.  With $K = 10$ bins and
$n_{\mathrm{cal}} = 1000$, each bin has $\sim\!100$ calibration points.
By the Hoeffding-DKW inequality, the binomial coverage estimate within
each bin has a standard error of
$\sigma_k \approx \sqrt{p(1-p)/n_k} \approx \sqrt{0.09/100} = 3.0\%$.
A Bonferroni correction across $K$ bins inflates the simultaneous
confidence interval by $\ln(2K/\delta)/(2n_k)$.  With $K=10$ and
$\delta=0.05$ this adds $\approx 2.4\%$ to each bin's uncertainty.

\textbf{Implication:}  Per-bin coverage claims are credible to
$\pm 3{-}5\%$ with the current calibration size.  Bins with fewer
than $\sim\!80$ points (e.g.\ the extreme low-$w_t$ tail) should
report coverage as directional rather than precise.  Increasing
$n_{\mathrm{cal}}$ to 5\,000 (available in the 168-hour episode
archive) would reduce per-bin uncertainty to $\pm 1.5\%$.
