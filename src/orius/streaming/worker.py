"""
Kafka Streaming Worker for ORIUS.
...
def load_config_from_env() -> WorkerConfig:
    ...
    return WorkerConfig(
        ...
        kafka_topic=os.environ.get("KAFKA_TOPIC", "orius-telemetry"),
        kafka_group_id=os.environ.get("KAFKA_GROUP_ID", "orius-streaming-worker"),
        duckdb_path=os.environ.get("DUCKDB_PATH", "data/streaming/events.duckdb"),
        checkpoint_path=os.environ.get("CHECKPOINT_PATH", "data/streaming/checkpoint.json"),
...
class StreamingWorker:
    ...
    def __init__(self, config: WorkerConfig):
        ...
        self._consumer: StreamingIngestConsumer | None = None
    ...
    def _process_message(self, message: dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                event_time = datetime.fromisoformat(event.utc_timestamp.replace("Z", "+00:00"))
                delay = (datetime.utcnow() - event_time.replace(tzinfo=None)).total_seconds()
                STREAMING_PROCESSING_DELAY.labels(topic=self.config.kafka_topic).set(max(0, delay))
            except Exception:
                STREAMING_PROCESSING_DELAY.labels(topic=self.config.kafka_topic).set(0.0)
    ...
    def run(self):
        """Main worker loop."""
        logger.info("Starting ORIUS Streaming Worker...")
                ...
                try:
                    ...
                    for message in self._consumer.consumer:
                        ...
                        if self._should_commit():
                            ...
                            self._last_commit_time = time.time()
                            logger.debug(f"Committed offsets, processed {self._messages_processed} messages")
