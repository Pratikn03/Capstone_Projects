...
class OPSDTelemetryEvent(BaseModel):
    ...
    Schema Versioning:
        ...
        create a new topic (e.g., orius.opsd.v2).
    ...
    DE_load_actual_entsoe_transparency: float | None = Field(None, description="Actual grid load in MW")
    DE_wind_generation_actual: float | None = Field(None, description="Wind generation in MW")
    DE_solar_generation_actual: float | None = Field(None, description="Solar generation in MW")
