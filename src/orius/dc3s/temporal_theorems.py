"""Helpers for the battery temporal and behavioral theorem layer."""
...
def forward_tube(
    *,
    ...
    """
    Construct the chapter-20 forward tube for a fixed safe action.

    The center drifts according to the battery action, while the uncertainty
    radius grows like sigma_d * sqrt(delta).
    """
    if horizon_steps < 0:
    ...
    net_delta = int(horizon_steps) * _safe_action_delta_mwh(
        safe_action,
        dt_hours=dt_hours,
...
def certificate_validity_horizon(
    *,
    ...
    """
    Compute the largest integer horizon whose forward tube stays inside bounds.
    """
    if max_steps < 0:
...
def certificate_expiration_bound(
    *,
    ...
    """
    Compute the chapter-20 lower bound floor(delta_bnd^2 / sigma_d^2).
    """
    if sigma_d <= 0.0:
        raise ValueError("sigma_d must be positive.")
    delta_bnd = min(float(interval_lower_mwh) - float(soc_min_mwh), float(soc_max_mwh) - float(interval_upper_mwh))
...
def zero_dispatch_fallback() -> dict[str, float]:
    """Return the chapter-20 battery fallback action."""
    return {"charge_mw": 0.0, "discharge_mw": 0.0}


def certify_fallback_existence(
    *,
    ...
    """
    Validate that zero dispatch preserves a safe interior SOC up to model error.
    """
    soc_min = _f(constraints.get("min_soc_mwh"), 0.0)
    soc_max = _f(constraints.get("max_soc_mwh"), soc_min)
...
def evaluate_graceful_degradation_dominance(
    ...
    """
    Check the battery theorem that graceful degradation dominates uncontrolled dispatch.
    """
...
def certificate_half_life(
    *,
    ...
    """Compute certificate confidence half-life.

    Given a validity horizon ``tau_t`` (in steps), the half-life is the
    number of steps at which the certificate confidence drops to 50%.
    The PDF defines this as  ``tau_t * ln(2) / |ln(decay_rate)|``
