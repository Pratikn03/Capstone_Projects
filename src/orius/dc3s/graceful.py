...
def plan_graceful_degradation(
    ...
    if remaining_horizon <= 0:
        transition_log.append(
            {"event": "fallback_termination", "reason": "Horizon exhausted.", "remaining_horizon": 0}
        )
        return {
            "actions": [{"charge_mw": 0.0, "discharge_mw": 0.0}],
            "reason": "Horizon exhausted.",
            "transition_log": transition_log,
        }

    transition_log.append(
        {"event": "fallback_invoked", "fallback_mode": fallback_mode, "remaining_horizon": remaining_horizon}
    )
    ...
    if fallback_mode == "ramp_down":
        ...
        transition_log.append(
            {"event": "policy_applied", "mode": "simple_ramp_down", "reason": "Ramp-down heuristic."}
        )
        ...
        transition_log.append(
            {"event": "policy_applied", "mode": "optimized_graceful", "reason": "Optimized fallback."}
        )
        return {"actions": action_plan, "reason": "Optimized fallback.", "transition_log": transition_log}

    else:  # hard_shutdown
        transition_log.append(
            {"event": "policy_applied", "mode": "immediate_shutdown", "reason": "Hard shutdown."}
        )
        return {
            "actions": [{"charge_mw": 0.0, "discharge_mw": 0.0}],
...
def _simulate_policy(
    ...
    if policy_name == "blind_persistence":
        ...
        actions = list(
            optimized_graceful(last_action, horizon_steps, soc_mwh, constraints, sigma_d, utility_weight=0.5)
        )
    elif policy_name == "optimized_graceful":
        actions = list(
            optimized_graceful(last_action, horizon_steps, soc_mwh, constraints, sigma_d, utility_weight=1.0)
        )
    else:
        actions = [{"charge_mw": 0.0, "discharge_mw": 0.0} for _ in range(horizon_steps)]
...
def compare_policies(
    ...
    return {
        p: _simulate_policy(p, dict(last_action), horizon_steps, soc_mwh, dict(constraints), sigma_d, seed)
        for p in policies
    }
