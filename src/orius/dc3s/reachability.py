...
def compute_validity_horizon_from_reachability(
    ...
    for k, (lower, upper) in enumerate(zip(tube["lower_mwh"], tube["upper_mwh"], strict=False)):
...
def compute_expiration_bound(
    ...
    raw_tau = (delta_bnd_mwh - radius) / sigma_d if sigma_d > 1e-06 else float(_MAX_VALIDITY_STEPS)
