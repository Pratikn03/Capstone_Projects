...
class AdaptivePageHinkleyDetector:
    """Page-Hinkley with adaptive threshold that scales with residual variance.

    PDF §4 specifies the threshold should adapt:
        threshold_t = base_threshold * (1 + k_var * running_std / mean_residual)

    When residual variance is low the threshold tightens to detect smaller
    shifts; when variance is high it widens to avoid false alarms.
    """
    ...
    warmup_steps: int = 48
    cooldown_steps: int = 24
    _inner: PageHinkleyDetector = field(default_factory=PageHinkleyDetector)
    _sum_sq: float = 0.0
    _count: int = 0
    _running_mean: float = 0.0

    def __post_init__(self):
        self._inner = PageHinkleyDetector(
            delta=self.delta,
    ...
    def update(self, r_t: float) -> Dict[str, float | bool | int]:
        ...
        self._sum_sq += (value - old_mean) * (value - self._running_mean)

        if self._count > 1 and abs(self._running_mean) > 1e-9:
            std = math.sqrt(max(0.0, self._sum_sq / (self._count - 1)))
            adaptive_threshold = self.base_threshold * (1.0 + self.k_var * std / abs(self._running_mean))
        else:
            adaptive_threshold = self.base_threshold

        self._inner.threshold = adaptive_threshold
    ...
    def to_state(self) -> Dict[str, Any]:
        ...
        inner_state["_sum_sq"] = self._sum_sq
        inner_state["_count"] = self._count
        inner_state["_running_mean"] = self._running_mean
        return inner_state
    ...
    def from_state(cls, state: Dict[str, Any] | None, cfg: Dict[str, Any] | None = None) -> "AdaptivePageHinkleyDetector":
        ...
        detector = cls(
            ...
            k_var=float(cfg.get("adaptive_k_var", 1.0)),
            delta=float(cfg.get("ph_delta", 0.01)),
            warmup_steps=int(cfg.get("warmup_steps", 48)),
            cooldown_steps=int(cfg.get("cooldown_steps", 24)),
        )
        if state:
            detector._inner = PageHinkleyDetector.from_state(state, cfg)
            detector._sum_sq = float(state.get("_sum_sq", 0.0))
            detector._count = int(state.get("_count", 0))
            detector._running_mean = float(state.get("_running_mean", 0.0))
        return detector
