...
def optimize_dispatch(
    ...
    objective_mode = str(objective.get("mode", "cost_minimize")).strip().lower()
    efficiency_mode = str(battery.get("efficiency_mode", "piecewise")).strip().lower()
    use_constant_efficiency = efficiency_mode == "constant"

    # When constant-efficiency mode is active, collapse piecewise regimes
    # to a single constant value (PDF Section 5 default formulation).
    if use_constant_efficiency:
        eta_a = nominal_eff
        eta_b = nominal_eff
    ...
    # Dual objective: discharge earns revenue at market price (negative cost)
    if objective_mode == "dual":
        c[idx_discharge] = (degradation_cost_per_mwh - cost_weight * price) * dt_hours
