...
def _repo_root() -> Path:
    ...
    # run.py -> src/orius/pipeline/run.py, so repo root is parents[3]
...
def _build_base_load_intervals(
    ...
    if bool(uncertainty_cfg.get("enabled", False)):
        ...
        if conformal_path.exists():
            try:
                from orius.forecasting.uncertainty import load_conformal
...
def _apply_faci_online(
    ...
    from orius.forecasting.uncertainty import AdaptiveConformal
...
def _build_robust_dispatch_config(opt_cfg: dict[str, Any]):
    from orius.optimizer.robust_dispatch import RobustDispatchConfig
...
def _calculate_vss(*args, **kwargs):
    from orius.evaluation.regret import calculate_vss

    return calculate_vss(*args, **kwargs)


def _calculate_evpi(*args, **kwargs):
    from orius.evaluation.regret import calculate_evpi
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ORIUS pipeline orchestrator")
    ...
    log = logging.getLogger("orius.pipeline")
    ...
    if "benchmark" in steps:
        ...
        runner_module = repo_root / "src" / "orius" / "cpsbench_iot" / "runner.py"
