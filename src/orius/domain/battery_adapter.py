...
from orius.domain.adapter import DomainAdapter, Plant, Optimizer, Action, UncertaintySet
...
from orius.optimizer.lp_dispatch import optimize_dispatch as _optimize_dispatch
...
def _l2_projection_repair(
    ...
    min_soc = _f(constraints.get("min_soc_mwh"), 0.0)
    max_soc = _f(constraints.get("max_soc_mwh"), capacity)
    max_power = _f(constraints.get("max_power_mw"),
                    max(_f(constraints.get("max_charge_mw"), 0.0),
...
def repair_battery_action(
    ...
    if mode == "robust_resolve":
        ...
        safe, proj_meta = _projection_repair(
            action_seed, state, uncertainty_set, constraints, cfg
        )
    ...
    if mode == "robust_resolve_cvar":
        cvar_action, cvar_meta = _robust_resolve_cvar(
            state, uncertainty_set, constraints
        )
    ...
    if mode == "l2_projection":
        return _l2_projection_repair(
            a_star, state, uncertainty_set, constraints, cfg
        )
