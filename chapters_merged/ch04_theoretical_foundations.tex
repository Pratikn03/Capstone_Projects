...
true-unsafe witness can be constructed.  This is an existence witness, not a
claim that every degraded step or every controller produces an OASG.
\end{theorem}
...
The proof uses A11 for the boundary-sensitive step and A12 for coexistence of
that step with the telemetry fault; domains that cannot discharge those
obligations do not inherit this theorem.
\end{proof}
...
Under Assumptions A1, A2, A4, A5, A6, A7, and A9, the expected total violation
...
For any fixed-margin quality-ignorant mandatory-release controller
$\pi : \mathcal{X} \to \mathcal{U}$ whose admissible observation gap can exceed
its fixed margin at a boundary-sensitive release step, there exists an
admissible degraded-observation sequence $\{\xi_t\}_{t=1}^T$ and a time index
$t^\star$ such that
\[
...
For any fixed-margin quality-ignorant controller $\pi$ operating on the battery
plant with dropout rate $d > 0$ and an admissible gap larger than its fixed
margin at a boundary-sensitive step, there exists a fault sequence
...
mandatory-release controller $\pi$ and any dropout rate $d > 0$ whose
admissible observation gap exceeds $m^\pi+\eta$ at a boundary-sensitive release
step, there exists an admissible fault sequence such that $\pi$ incurs at least
one true-state violation:
\[
  \exists\,t^* \in \{1, \ldots, T\} : \quad
  \phi_{\text{obs}}(\pi(\hat{x}_{t^*})) = 1 \;\;\text{and}\;\;
  \phi_{\text{true}}(\pi(\hat{x}_{t^*})) = 0.
\]
The theorem is qualitative and witness-based.  Any repeated-window rate claim
would require a separate promotion surface; the active T10 row is instead the
two-state boundary-indistinguishability lower bound.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:admissible-fault} and the theorem's gap hypothesis, there
exists a fault sequence with a concentrated fault window in which
$|\xi_t| > \delta_{\text{bnd}}+m^\pi$ at the boundary-sensitive step.
...
\begin{theorem}[Graceful degradation dominance with useful work]
\label{thm:graceful-dominance}
Under Assumptions A1--A5 and A8, consider paired graceful and uncontrolled
rollouts evaluated on the same admissible fault trace.  Let \(Z_t^G,Z_t^U\) be
their true-state violation indicators and \(W_G,W_U\) their useful-work totals.
For a declared \(\lambda\in(0,1]\), if
\[
...
safety--work preorder.  This paired-trace safety-plus-work comparison is the
full defended T8 surface; the stronger absorbing-landing interpretation remains
supporting-only.
\end{theorem}

\begin{proof}
Summing the paired stepwise inequality \(Z_t^G\le Z_t^U\) over
\(t=1,\dots,T\) gives
\[
  \sum_{t=1}^{T} Z_t^G
  \le
  \sum_{t=1}^{T} Z_t^U,
\]
which is the safety coordinate of the preorder.  The useful-work coordinate is
the stated \(W_G\ge\lambda W_U\) hypothesis.  Immediate shutdown may satisfy
the safety coordinate, but it is not a T8 witness unless it also satisfies the
declared useful-work threshold.
\end{proof}
