...
\subsection{Assumption IDs (A1--A8)}
...
system; the battery domain provides one concrete instantiation below (Chapter~\ref{ch:battery-to-orius} covers all six).
...
degradation in optimising controllers.  The energy management domain provides
a concrete witness: a TSVR of 3.9\% for the deterministic LP under
\texttt{drift\_combo} at 20\% dropout.

\def\domainblockid{T1}
...
The energy management domain provides the scalar instantiation below (SOC case), where
...
\section{The ORIUS Core Safety Bound}
\label{ch:core-bound}
...
compute and verify this bound for all six validated domains.
...
\subsection{Per-Step Risk Lemma}
...
\subsection{Main Theorem: ORIUS Core Bound}
...
\section{No Free Safety Theorem}
...
fixed margin.  The energy management domain provides one concrete construction below.
...
\subsection{Main Theorem: No Free Safety}
