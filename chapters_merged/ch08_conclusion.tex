...
records \textbf{83 formal items}: 26~theorem environments, 5~lemmas,
9~propositions, 12~corollaries, 15~definitions, and 16~standing
assumptions.  Those counts are computed over the active master-manuscript
include tree rather than the entire repository so that stale standalone
chapter surfaces are not double-counted.  The full inventory is governed by
the integrated theorem surface register, while the older hard gate still
audits the defended source-backed theorem rows rather than every theorem
...
\begin{enumerate}
  ...
  \item \textbf{Zero true-state violation rate.}  Energy management,
    ...
    repair benefit.
  \item \textbf{Formal safety program.}  The defended theorem program now
    spans the precursor results S1--S2, the reference ladder T1--T8, and the
    later necessity, lower-bound, and typed-transfer results T9--T11.  T2 is
    a one-step conditional shield theorem, T3 is a risk-envelope theorem
    under an explicit per-step budget contract, and T10--T11 are stated with
    the narrower scope their proofs actually justify.
...
\begin{enumerate}
  ...
  \item \textbf{Full reference proof witness.}  Theorems T1--T8 are stated,
    proved, and connected to code artifacts on the battery surface, while
    Chapter~\ref{ch:universality-completeness} closes the later T9--T11
    extension on top of that reference witness.  Battery is still the only
    row with the complete theorem-to-code-to-artifact lineage.
...
This thesis contributes an 83-item formal apparatus computed from the active
manuscript include tree and anchored to a narrower defended theorem program:
the precursor results S1--S2, the reference ladder T1--T8, and the later
extension theorems T9--T11.  The ORIUS principle is established on a tiered
runtime package: the OASG exists across the locked runtime rows, DC3S
achieves zero TSVR in the three highest-confidence rows (battery reference,
industrial, healthcare), AV remains proof-candidate, aerospace remains
experimental, and navigation adds portability evidence without entering the
forecasting-capable parity set.  Keeping the boundary explicit protects the
universal architecture claim from empirical maturity it has not yet earned.
...
This section is the operational reading of the claim boundary drawn above.
It does not introduce a second theorem surface.  It explains what the
defended theorem program means once it is read together with the tiered
evidence package, the release gate, and the cross-domain adapter contract.
...
\begin{enumerate}
  ...
  \item \textbf{Traceability is end-to-end.}  The precursor results, the
    reference ladder, and the later T9--T11 extension all map to explicit
    code witnesses, proof locations, and locked artifact surfaces.  The
    integrated theorem surface register
    (Appendix~\ref{app:integrated-theorem-surface-register}) inventories the
    full formal surface, while the defended theorem gate tracks the narrower
    source-backed release rows.
...
The ORIUS framework shows that the observation-action safety gap is a
universal CPS phenomenon with a principled runtime remedy, but it does not
pretend that every row has equal closure.  Energy management provides the
formally locked reference surface; industrial and healthcare provide the
strongest non-reference confirmation; the remaining rows stay explicitly
tiered.  The theorem program, bounded risk language, and governed replay
surfaces together constitute the defended safety argument for DC3S-shielded
...
The full formal apparatus of this dissertation consists of 26 theorem
environments, 5 lemmas, 9 propositions, 12 corollaries, 15 definitions, and
16 standing assumptions---83 formal items in total.  Within that inventory,
the defended unique theorem program is composed of two foundational precursor
results and the eleven permanent ORIUS audit surfaces.  These permanent labels
span theorem, definition, proposition, and corollary roles; they are not all
promoted as main-paper theorem claims:
...
\begin{itemize}
  ...
  \item \textbf{T1--T8}: the battery reference ladder for OASG existence,
    one-step safety preservation, the risk-envelope bound, no-free-safety,
    temporal certificate behavior, fallback existence, and graceful
    degradation.
  \item \textbf{T9--T11}: the later universal extension for necessity under
    persistent degradation, the scoped lower-bound frontier, and typed
    structural transfer with failure-mode converse.
\end{itemize}

The battery ladder is stated, proved, and connected to code artifacts in
Chapters~\ref{ch:theorem-oasg}--\ref{ch:temporal-extensions}; the later
extension is closed in Chapter~\ref{ch:universality-completeness} and indexed
in Appendix~\ref{app:proofs} and
Appendix~\ref{app:verified-theorems-gap-audit}.  The older defended release
gate still checks 18 hard-gated theorem rows---the precursor results, the
reference-ladder theorems, and the Appendix~C theorem restatements---for
source, code, artifact, and mapping integrity.  The full 26-theorem
inventory is instead governed by the integrated theorem register and the
