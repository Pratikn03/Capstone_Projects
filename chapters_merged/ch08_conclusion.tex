...
\begin{enumerate}
  \item \textbf{OASG existence.}  The Observation-Action Safety Gap is
    ...
    schedule.
    Baseline TSVR spans 2.78\% (autonomous vehicles) to 21.53\% (industrial)
    across the five defended domains under the locked three-seed, 48-step
    protocol before DC3S.
  ...
  \item \textbf{Zero true-state violation rate.}  Energy management,
    ...
    proof evidence package.  AV and aerospace carry additional per-domain
    evidence at N~$= 2000$ steps showing equivalent reduction.
...
shared universal replay harness.  AV and aerospace contribute experimental
evidence at higher evaluation budgets, and navigation provides simulation-backed
...
principle, while navigation extends the portability story without entering the
equal-depth claim.
...
the defended five-domain set: the OASG exists in all five, DC3S achieves
zero TSVR in the three proof-validated domains (energy management, industrial,
healthcare), and the formal safety bound holds empirically.  Energy management
provides the full theorem-to-artifact lineage; industrial and healthcare match
its empirical depth; AV and aerospace carry experimental evidence; navigation
adds simulation-backed portability evidence.  Being explicit about what remains
outside the current evidence package protects the core multi-domain result from
...
\begin{enumerate}
  ...
  \item \textbf{DC3S closes the gap reliably.}  In the three
    ...
    to the unshielded controller.  AV and aerospace carry experimental evidence
    of material TSVR reduction under the higher-budget N~$= 2000$ evaluation
...
T1 -- OASG Existence
  ...
  & 2.78\%--21.53\% baseline TSVR across 5 domains (locked protocol) \\
\addlinespace
T2 -- Safety Preservation
  & $\Pr[\phi_{\text{true}}(a_t) = 1] \ge 1-\alpha$ per step
  & TSVR $= 0$\% across the 3 proof-validated domains (energy, industrial, healthcare) \\
\addlinespace
T3 -- Core Bound
...
meaningful, and correctable.  ORIUS demonstrates this across six peer
cyber-physical domains.  Three proof-validated domains---energy management,
industrial process control, and medical monitoring---achieve zero true-state
violations under DC3S in the locked three-seed, 48-step protocol; autonomous
vehicles and aerospace show material TSVR reduction under the higher-budget
N~$= 2000$ evaluation and are classified as proof-candidate and experimental
surfaces respectively; the navigation domain provides simulation-backed
...
\begin{enumerate}
  ...
  \item The energy management domain provides the complete reference
    ...
    healthcare provide proof-validated empirical confirmation; AV and
    aerospace carry experimental evidence at additional evaluation budgets;
...
a locked, traceable, reproducible multi-domain safety package defended across
five domains, extended by simulation-backed navigation portability evidence,
