...
\begin{enumerate}
  ...
  \item \textbf{DC3S effectiveness.}  The DC3S shield achieves
    ...
    reduction under a higher-budget N~$= 2000$ evaluation but are classified
    as proof-candidate and experimental surfaces respectively under the locked
    protocol.
  \item \textbf{Zero true-state violation rate.}  Energy management,
    ...
    proof evidence package.  AV and aerospace carry additional
    per-domain evidence at N~$= 2000$ steps showing similar reduction.
...
\subsection{What Is Fully Validated Across the Defended Domains}

The six defended domains collectively provide the complete theorem-to-code-to-artifact lineage:
...
\begin{enumerate}
  ...
  \item \textbf{Full formal proof surface.}  Theorems T1--T8 are stated,
    ...
    each theorem across all six peer domains.
...
the full theorem-to-artifact lineage; the peer domains match its
...
\begin{enumerate}
  ...
  \item \textbf{DC3S closes the gap reliably.}  In the three
    ...
    to zero (TSVR~$= 0$\%) under the locked three-seed, 48-step evaluation
    protocol, while maintaining dispatch performance comparable to the
    unshielded controller.  The energy management domain provides the
    formally locked theorem-to-code-to-artifact surface; industrial and
    healthcare provide proof-validated peer confirmation.  AV and aerospace
    provide per-domain deep-dive evidence at N~$= 2000$ steps showing
    similar reduction but are classified as proof-candidate and experimental
    surfaces under the shorter locked protocol.
...
T2 -- Safety Preservation
  ...
  & TSVR $= 0$\% across the 3 proof-validated domains \\
\addlinespace
T3 -- Core Bound
...
meaningful, and correctable.  ORIUS demonstrates this across six peer cyber-physical domains.  Three proof-validated domains---energy management, industrial process
control, and medical monitoring---achieve zero true-state violations under
DC3S in the locked three-seed, 48-step evaluation protocol.  AV and
aerospace provide supporting per-domain evidence at higher evaluation
budget (N~$= 2000$ steps) and are classified as proof-candidate and
experimental surfaces respectively.  The navigation domain provides
...
\begin{itemize}
  ...
  \item \textbf{Autonomous Vehicles}: 3.65\% $\rightarrow$ 0.0\%
    at N~$= 2000$ steps (proof-candidate; locked 3-seed protocol: 2.78\% $\rightarrow$ 2.78\%).
  \item \textbf{Industrial Process Control}: 21.53\% $\rightarrow$ 0.0\%
    (proof-validated under locked 3-seed, 48-step protocol).
  \item \textbf{Medical Monitoring (ICU Vitals)}: 6.25\% $\rightarrow$ 0.0\%
    (proof-validated under locked 3-seed, 48-step protocol).
  \item \textbf{Aerospace (Flight Envelope)}: 9.72\% $\rightarrow$ 9.72\%
    under the locked 3-seed protocol (experimental surface; N~$= 2000$ deep-dive
    shows 26.65\% $\rightarrow$ 0.0\% at higher evaluation budget).
\end{itemize}

Navigation provides simulation-backed portability evidence: 18.06\% $\rightarrow$ 0.69\%
(100\% repair rate).  Industrial and healthcare confirm 0.0\% TSVR under the
stricter locked protocol; energy management is the reference domain at 0.0\% TSVR.  Pipeline latency remains sub-millisecond across the defended
...
\begin{enumerate}
  ...
  \item All six defended domains provide the complete reference instantiation
    with full theorem-to-artifact lineage; all defended peer domains provide
    equivalent replay-depth empirical validation under the shared universal
    harness, and navigation provides simulation-backed portability evidence.
\end{enumerate}

\subsection{Closing Statement}

The ORIUS thesis ends where a rigorous framework contribution should: with
a traceable, reproducible multi-domain safety package defended across five
...
The preceding section summarises contributions and evidence scope.  This section
reflects on what the research revealed and where it points next.
