...
observation-action safety gap.  This is an existence witness, not a frequency
or universality claim: A11 supplies the boundary-sensitive step, A12 permits
the fault/controller co-occurrence, and a new domain cannot reuse T1 unless it
discharges the same boundary-reachability obligation.  \qedhere
\end{proof}
...
\begin{theorem}[restated]
Under A1--A5, A7, and the explicit predictable battery risk-envelope contract
$\Prob[Z_t=1\mid\mathcal{H}_t]\le r_t$ with
$r_t=\alpha(1-w_t)$ on the fixed certified budget trace, one has
$\E[V]\le\alpha\,T\,(1-\bar{w})$ for
$\bar w=T^{-1}\sum_{t=1}^T w_t$.
\end{theorem}
\begin{proof}
Let
\[
...
\[
  ...
  r_t=\alpha(1-w_t),
\]
where $r_t$ is predictable from the runtime history used to issue the
certificate.  In the fixed replay/certificate trace used by the theorem
artifact, the recorded budget values are treated as constants.  If the
reliability process is left random instead, the defensible statement is the
weaker averaged form $\E[V]\le\alpha\sum_t\E[1-w_t]$.
Taking expectations of both sides gives
\[
  \Prob[Z_t=1]
  =
  \E[\Prob[Z_t=1\mid\mathcal{H}_t]]
  \le
  \E[r_t].
\]
On the fixed recorded budget trace, \(\E[r_t]=r_t\), giving the next display.

Substituting the per-step bound into the expectation identity yields
\[
  \E[V]
  \le \sum_{t=1}^{T} r_t
  = \sum_{t=1}^{T} \alpha(1-w_t)
...
budget; no temporal independence, exchangeability across time, or martingale
strengthening is introduced.  Therefore the battery ORIUS core bound holds for
any admissible horizon $T$ on the defended replay surface whenever that
per-step budget contract has been justified.
\qedhere
\end{proof}

\section{C.4\quad Observation Necessity / No Free Safety}\label{sec:proof-no-free}
\begin{theorem}[restated]
Under A1, A2, A4, and A11, let $\pi$ be a fixed-margin,
quality-ignorant mandatory-release controller with effective margin $m^\pi$.
If the battery witness row has an admissible degraded-observation gap
$\delta>m^\pi+\eta$ at a boundary-sensitive release step for some
$\eta>0$, then there exists an admissible fault sequence with
$\TSVR(\pi)>0$.
\end{theorem}
\begin{proof}
Fix such a controller $\pi$.  By definition, its effective safety
...
By the theorem's gap hypothesis and A2, choose a degraded-observation fault
realization with
\[
...
finite-horizon and class-scoped exactly as claimed in T4.  It does not rule out
controllers with adaptive margins, abstention, fallback, or a fixed margin so
large that the stated degraded-observation gap is impossible.  \qedhere
\end{proof}
