...
\textbf{Solution (60 sec).} The ORIUS/DC3S framework combines: (1) reliability-adaptive conformal prediction that inflates intervals when telemetry degrades, preserving coverage $\geq 1-\alpha$ (Theorem~2); (2) a projection-based shield that repairs unsafe actions to feasible safe actions; (3) per-step certificates with hash chaining for auditability. We prove monotonicity: higher reliability $w_t$ yields tighter margins (Theorem~3). All results are battery-only, 1-hour dispatch, with locked empirical evidence in \texttt{reports/publication/}.
