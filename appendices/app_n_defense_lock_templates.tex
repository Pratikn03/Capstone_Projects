...
\textbf{Defense.}  T\textsubscript{PAC} (Proof~C.22) is deliberately the
finite-horizon Bonferroni certificate: if
\(\Pr[Z_t=1]\le\epsilon_t\) and \(\sum_t\epsilon_t\le\delta\), then the
trajectory is violation-free with probability at least \(1-\delta\).  No
Ville/supermartingale strengthening is claimed unless a future theorem adds
the required martingale increment condition and code path.
