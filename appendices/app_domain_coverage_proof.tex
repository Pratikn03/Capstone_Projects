...
The per-step risk lemma (Lemma~\ref{lem:per-step-risk}) is used only after a
domain supplies the explicit predictable budget
\[
  \Pr[Z_t = 1\mid\mathcal H_t] \le r_t,
  \qquad r_t\le\alpha(1-w_t),
\]
on the fixed certified budget trace.  Marginal conformal coverage alone is not
treated as sufficient.  The current defended paper uses this contract only
across the active Battery, AV, and Healthcare claim surface.  Earlier
exploratory rows are not promoted in this proof check.
