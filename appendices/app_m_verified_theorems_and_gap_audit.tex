...
\begin{itemize}
  \item \textbf{Flagship defended}: T1, T2, T3a, T4, T11, and T\_trajectory\_PAC.
  \item \textbf{Supporting defended}: T3b, T6, T8, T11\textsubscript{Byz}, and T\textsubscript{stale}.
  \item \textbf{Draft / non-defended}: T5, T7, T9, T10, L1--L4,
...
remains a proposition. Appendix~\ref{app:integrated-theorem-surface-register}
still audits the older hard-gated traceability surface, but it is a
...
\textbf{Proof status.} Supporting defended auxiliary theorem with
machine-checkable helper linkage.  The theorem-facing API now requires
\texttt{delta} explicitly and marks legacy no-\texttt{delta} semantics as
non-defended.
...
\textbf{Proof status.} Constructive battery fallback theorem, currently
bounded to interior states with sufficient slack against model error.
...
T6 Certificate Expiration Bound & V2-linked auxiliary theorem & Yes & Yes & Canonical helper is delta-aware and rejects legacy no-delta theorem semantics \\
T7 Feasible Fallback Existence & Narrowed to interior states & Partial & Partial & Constructive zero-dispatch fallback is validated only for bounded-error interior states \\
...
\midrule
T9 Universal Impossibility & Structural / scoped witness constant & Yes & Partial & $\varphi$-mixing + Azuma windowing remain explicit assumptions; battery T4 is the reference witness; ch37 \S\,\ref{sec:t9-universal-impossibility} \\
