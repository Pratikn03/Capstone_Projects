...
\textbf{Statement.} Within the fixed-margin, quality-ignorant
mandatory-release controller class, if an admissible degraded-observation gap
exceeds the fixed margin at a boundary-sensitive release step, there exists an
admissible degraded-observation sequence that produces an observation-action
safety gap and hence a true-state violation.

\textbf{Assumptions.} A1, A2, A4, A11, plus the finite fixed-margin and
boundary-sensitive release witness. Quality-ignorant baselines lack
reliability-aware inflation.
...
\subsection{Theorem 5: Finite-Horizon Certificate Validity}
\label{sec:thm-cert-horizon}

\textbf{Statement.} If the reachable tube from the observation-consistent
state set remains inside the domain safe-state set through horizon~$h$, then
the runtime certificate issued at time~$t$ is valid for that finite horizon
unless an invalidating event occurs.

\textbf{Assumptions.} A1, A2, A3, A4, and A5.

\textbf{Proof status.} Flagship finite-horizon theorem.  The proof is an
induction over the reachable tube containment condition; no unconditional
future-step probability law is claimed beyond the certified horizon or after
contradictory evidence.

\textbf{Code anchor.} \texttt{src/orius/dc3s/temporal\_theorems.py}
(\texttt{forward\_reachable\_tube()},
\texttt{certificate\_validity\_horizon()},
\texttt{certificate\_invalidating\_event()}); registered as T5 in
\texttt{THEOREM\_REGISTER}.  Runtime expiry logic consumes this quantity and
fails closed when the horizon is exhausted.
...
\textbf{Statement.} The defended T8 surface is the paired safety--work
comparison: for graceful and uncontrolled policies produced under the same
admissible fault trace,
\[
  Z_t^G \le Z_t^U \;\; \forall t
  \qquad\text{and}\qquad
  W_G\ge\lambda W_U
\]
for a declared \(\lambda\in(0,1]\).  This proves weak dominance in the
safety--work preorder and does not claim global fallback optimality.

\textbf{Assumptions.} A1--A5, A8.

\textbf{Proof status.} Flagship paired-trace dominance theorem.  The useful-work
threshold prevents the theorem from being satisfied by immediate shutdown alone,
and the broader absorbing-landing interpretation remains outside this claim.

\textbf{Code anchor.} \texttt{src/orius/benchmarks/graceful\_degradation.py}
(\texttt{graceful\_dominance\_with\_useful\_work()},
\texttt{evaluate\_policy\_frontier()}).

\subsection{Theorem 9: Impossibility of Quality-Ignorant Mandatory Release}
\label{sec:thm-universal-impossibility}

\textbf{Statement.} For an observation~$o$ with ambiguity class
$B(o)=\{x:h(x)=o\}$ and common safe core
$K(o)=\cap_{x\in B(o)}C(x)$, if $B(o)\ne\varnothing$ and
$K(o)=\varnothing$, then no total observation-only mandatory-release controller
can guarantee true-state safety for every latent state in $B(o)$.

\textbf{Assumptions.} Mandatory observation-only release and an empty common
safe-core witness for the ambiguity class.

\textbf{Proof status.} Flagship scoped impossibility theorem.  The result is
not a statement about controllers allowed to abstain, expand uncertainty,
fallback, or deny release.

\textbf{Code anchor.} \texttt{src/orius/universal\_theory/ambiguity.py}
(\texttt{compute\_common\_safe\_core()},
\texttt{find\_mandatory\_release\_counterexample()}).

\subsection{Theorem 10: Boundary-Indistinguishability Lower Bound}
\label{sec:thm-info-lower-bound}

\textbf{Statement.} For two latent boundary states \(x_0,x_1\) with observation
laws \(P_0,P_1\), if \(d_{\mathrm{TV}}(P_0,P_1)\le\epsilon\) and
...
\textbf{Assumptions.} Two-state boundary pair, TV bound, disjoint safe-action
sets, and mandatory observation-only release.

\textbf{Proof status.} Flagship scoped lower-bound theorem.  The proof reduces
release success to binary testing success and therefore no longer depends on an
unstated one-sided Le~Cam step or on identifying \(w_t\) with TV distance.

\textbf{Code anchor.} \texttt{src/orius/universal\_theory/boundary\_indistinguishability.py}
(\texttt{two\_state\_lower\_bound}, \texttt{estimate\_total\_variation}).
