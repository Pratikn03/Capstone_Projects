...
def main() -> None:
    ...
    ft = pd.read_csv(REPO_ROOT/'reports/publication/fault_performance_table.csv').rename(columns={'TSVR (%)':'tsvr_pct','Sev P95 (MWh)':'sev_p95','Controller':'controller'})
    base = ft[ft['controller']=='deterministic_lp'][['Fault Dimension','Severity','tsvr_pct','sev_p95']].rename(columns={'tsvr_pct':'base_tsvr','sev_p95':'base_sev'})
    dc = ft[ft['controller']=='dc3s_ftit'][['Fault Dimension','Severity','tsvr_pct','sev_p95']].rename(columns={'tsvr_pct':'dc_tsvr','sev_p95':'dc_sev'})
    ...
    write_manifest(out, 'run_aging_proxy_analysis_manifest.json', {'rows': int(len(m)), 'source': 'reports/publication/fault_performance_table.csv'})
