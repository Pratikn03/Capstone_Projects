% =============================================================================
% Legacy non-canonical battery-centric short draft.
% Superseded by paper/ieee/orius_ieee_main.tex and paper/ieee/orius_ieee_appendix.tex.
% =============================================================================
...
\begin{table}[htbp]
\centering\small
\caption{Methodological positioning of \DCThreeS{} relative to prior work.}
...
\PaperFigure{FIG01_ARCHITECTURE}{ORIUS architecture with \DCThreeS{} at the dispatch boundary. The shield wraps the optimizer, inflating intervals and repairing actions before execution.}
...
\caption{\DCThreeS{}: One dispatch step}
\label{alg:dc3s}
\begin{algorithmic}[1]
...
\begin{proposition}[One-step invariance under a tightened SOC tube]\label{prop:r1_tightened_tube}
If the controller enforces observed next-state feasibility inside $[\mathrm{SOC}_{\min}+e_t,\mathrm{SOC}_{\max}-e_t]$ and the true-vs-observed SOC error is bounded by $e_t$, then the true next state remains inside $[\mathrm{SOC}_{\min},\mathrm{SOC}_{\max}]$.
\end{proposition}
\begin{proof}
The result follows by triangle inequality: observed feasibility leaves an $e_t$ margin on each side, and the bounded telemetry error cannot consume more than that margin.
\end{proof}
...
The promoted forecasting comparison uses six model families on each region:
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{N-BEATS}: fully connected residual backcast/forecast architecture.
  \item \textbf{TFT}: temporal fusion transformer baseline under the same feature framing.
  \item \textbf{PatchTST}: patch-based transformer for long-horizon forecasting.
\end{itemize}
Each model is trained on three targets (load, wind, solar) with the same 24-hour forecast horizon, 168-hour lookback, 10-fold time-aware cross-validation, split boundaries, seed policy, feature framing, and 24-hour gap between folds. Deep models use early stopping as the primary budget control, while GBM uses Optuna-based tuning; this is not a perfectly equalized compute budget, but it is a principled and reproducible comparison aligned with standard practice for each model family.
...
Alongside the locked CQR path, the repo now contains an additive reliability-binned conformal tool that groups calibration points by telemetry reliability and assigns group-specific conformal widths. Under exchangeability within a reliability bin, the standard Mondrian conformal argument gives group-conditional marginal coverage at that bin level. We treat this as research analysis tooling rather than as a change to the locked production CQR intervals in the current release family.
...
The promoted forecasting comparison examines all six model families under a shared feature framing and training budget. The paper-facing table shows DE results. GBM dominates on all three targets; deep models lag substantially, confirming the tabular-regime effect. Critically, all six model families now receive CQR-calibrated 90\% conformal prediction intervals from the same held-out calibration split, enabling a fair cross-model UQ comparison. GBM's narrower intervals reflect its superior point-forecast accuracy. Only GBM intervals enter the operational DC\textsuperscript{3}S dispatch pipeline, since GBM is the production-candidate forecaster; deep-model intervals are reported for completeness.
...
\PaperTableCSV{TBL01_MAIN_RESULTS}{Main controller comparison under aggregate telemetry faults.  \DCThreeS{} (wrapped and FTIT) achieve zero true-SOC violations.  CI: 95\% bootstrap over 10 seeds.}
...
\PaperFigure{FIG03_TRUE_SOC_VIOLATION}{True-SOC violation rate versus fault severity.  \DCThreeS{} maintains zero violations across all severity levels while baselines deteriorate monotonically.}

\PaperFigure{FIG04_TRUE_SOC_SEVERITY}{P95 true-SOC violation severity under fault sweep.  The deterministic baseline's severity grows superlinearly with fault severity; \DCThreeS{} remains at zero.}
...
\PaperFigure{FIG07_COST_SAFETY_FRONTIER}{Cost--safety Pareto frontier.  \DCThreeS{} dominates the zero-violation, low-cost region.  Dashed line: acceptable violation threshold.}
...
The paper-facing transfer table is a compact DE/US summary drawn from the current publication artifact rather than the still-pending explicit four-case rerun. It compares DC\textsuperscript{3}S and deterministic LP under nominal, dropout, and drift-combo stress to show that safety transfers more robustly than cost optimality.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Deep models do not currently outperform GBM} in the materialized rows of the promoted six-model comparison---GBM remains the strongest operational model across the published DE/US forecast surface.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Simulation only}: all results are from software simulation with replayed telemetry.  No hardware-in-the-loop or field deployment validation is claimed, and those hardware-facing stages remain future work only.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item Extend to hardware-in-the-loop validation with physical inverter/BMS integration as a future validation stage rather than as a claim of the current paper.
  \item Improve base calibration on under-covered targets (solar high-regime) and make the main-text US analysis explicitly balancing-authority-specific across MISO, PJM, and ERCOT.
  \item Add a clean deployment-facing latency/runtime benchmark and a publication-grade case trace showing telemetry degradation, interval widening, action repair, and safe dispatch over time.
