...
The forecasting result surface is now a promoted six-model comparison rather than the older three-model snapshot. All six models share one split, horizon, lookback, seed, and metric contract. In the currently materialized rows, GBM remains the strongest operational model, while LSTM and TCN continue to degrade sharply on wind and solar. N-BEATS, TFT, and PatchTST are now part of the same main comparison contract; where the table still shows dashes, the row is pending artifact completion rather than silently omitted.
