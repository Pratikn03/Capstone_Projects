...
% ---------- Title ----------------------------------------------------------
\title{DC\textsuperscript{3}S: Telemetry-Reliability-Weighted Conformal Safety Shield\\
       for Battery Dispatch under Degraded Mixed Telemetry}
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{RAC-Cert}: a reliability-adaptive conformal certification law that inflates intervals as a function of telemetry quality, drift risk, and sensitivity, with proved marginal coverage preservation (\Cref{thm:rac_coverage}).
  ...
  \item \textbf{Governance stack}: automated metric-locking and claim-validation tooling that prevents manuscript--artifact drift.
\end{enumerate}
...
Conformal prediction~\cite{vovk2005conformal} provides distribution-free prediction intervals with finite-sample coverage guarantees under exchangeability.  Extensions to time-series settings include adaptive conformal inference (ACI)~\cite{gibbs2021adaptive}, which adjusts the miscoverage rate online, and conformalized quantile regression (CQR)~\cite{romano2019conformalized}, which wraps quantile regressors with conformal calibration.  Regime-aware variants~\cite{barber2023conformal} stratify calibration by volatility bin.  None of these methods condition calibration on \emph{telemetry quality}, leaving a gap when input measurements are themselves degraded.

\paragraph{Robust and stochastic battery dispatch.}
Robust optimization for energy dispatch~\cite{bertsimas2011theory,ben2009robust} hedges against worst-case uncertainty within prescribed sets.  Stochastic programming~\cite{birge2011stochastic} optimizes expected cost over sampled scenarios.  CVaR-based formulations~\cite{rockafellar2000optimization} manage tail risk via Rockafellar--Uryasev linearization.  These methods assume well-specified uncertainty sets; when the sets are miscalibrated due to sensor faults, the robustness guarantees degrade.  DC\textsuperscript{3}S addresses this by coupling the uncertainty-set construction to telemetry reliability.

\paragraph{Safety shields and runtime monitors.}
Shield synthesis~\cite{alshiekh2018safe,konighofer2020shield} enforces safety by constraining the action space to pre-computed safe regions.  Simplex architectures~\cite{sha2001using} switch to a safe baseline controller upon detecting anomalies.  Runtime verification monitors~\cite{leucker2009brief} check trace properties post-hoc.  DC\textsuperscript{3}S combines elements of all three: it monitors telemetry health, inflates the optimizer's constraint set, and projects actions to the safe set---while emitting a per-step certificate for auditability.

\paragraph{Telemetry reliability in CPS.}
Telemetry degradation in industrial control has been studied in the context of SCADA security~\cite{dibaji2019systems} and anomaly detection~\cite{chandola2009anomaly}, but integrating a telemetry-quality score directly into the dispatch optimization loop is, to our knowledge, novel.

\paragraph{Positioning.}
\Cref{tab:positioning} summarizes the methodological positioning.  DC\textsuperscript{3}S is the first method to combine conformal calibration, telemetry-reliability weighting, dispatch-level action repair, and per-step certification in a single framework.
...
% ===== 4. METHOD =============================================================
\section{DC\textsuperscript{3}S: Detect--Calibrate--Constrain--Certify--Shield}\label{sec:method}

\Cref{fig:FIG01_ARCHITECTURE} shows the full system architecture.  DC\textsuperscript{3}S operates as a safety wrapper around any dispatch optimizer: at each control step, it scores telemetry, inflates uncertainty, solves the constrained optimization, repairs unsafe actions, and emits a certificate.
...
\subsection{Step 1: Telemetry Reliability Scoring}\label{sec:reliability}
...
\subsection{Step 2: RAC-Cert Interval Inflation}\label{sec:rac}
...
\begin{theorem}[RAC-Cert marginal coverage]\label{thm:rac_coverage}
...
\subsection{Step 3: Robust Dispatch with Inflated Intervals}\label{sec:dispatch}
...
\subsection{Step 4: Action Repair and Projection}\label{sec:repair}
...
\subsection{Step 5: Certificate Emission}\label{sec:certificate}
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Coverage theorem}: \Cref{thm:rac_coverage} provides marginal coverage preservation under inflation; it does not guarantee \emph{conditional} coverage or optimal interval width.
\end{enumerate}
...
\bibitem{falliere2011stuxnet}
N.~Falliere, L.~O'Murchu, and E.~Chien.
\newblock W32.{S}tuxnet dossier.
\newblock Technical report, Symantec Security Response, 2011.
