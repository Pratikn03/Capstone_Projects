...
The forecasting result surface is now a promoted six-model comparison rather than the older three-model snapshot. The paper-facing table is a condensed DE slice of the full publication matrix, selected because it most clearly shows the ranking in the tabular regime. All six models share one split, horizon, lookback, seed, and metric contract. In the materialized rows, GBM remains the strongest operational model, while the deep baselines lag substantially and only GBM carries production-candidate conformal uncertainty artifacts.
