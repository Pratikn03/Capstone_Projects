# ORIUS: A Universal Physical-Safety Framework for Degraded Observation, with Battery-Domain DC3S Validation
...
## Abstract
ORIUS is a universal physical-safety framework for cyber-physical control under degraded observation. This paper presents its battery-domain reference implementation, DC3S, and uses that surface to quantify the hidden gap between observed-state feasibility and true-state safety. Under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at **3.9% of steps with P95 severity of 333 MWh**, even while observed-state evaluation reports zero violations. ORIUS organizes the response as a shared five-stage loop — Detect, Calibrate, Constrain, Shield, Certify — across Energy, AV, Industrial, Healthcare/Surgical, and Aerospace adapters, while battery dispatch remains the primary locked empirical and theorem-validation surface. On Germany (OPSD, 17,377 rows), the battery-domain DC3S implementation delivers 7.11% cost savings, 0.30% carbon reduction, and 6.13% peak shaving; stochastic value remains positive in both DE and US regions (VSS = 2,708.61 and 297,092.71 respectively) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). The current package also includes a unified cross-domain harness, but those non-battery surfaces should be read as framework-portability evidence rather than universal performance guarantees or production-hardware validation. All quantitative claims are tied to versioned run artifacts and checked against the locked release evidence.
