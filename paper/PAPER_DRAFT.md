...
## Abstract
ORIUS studies a domain-agnostic safety problem: under degraded telemetry, a controller can be observationally safe while physically unsafe. This paper formalizes that observation-reality gap and implements a typed Detect-Calibrate-Constrain-Shield-Certify kernel whose battery-domain reference instantiation, DC3S, quantifies the hidden gap between observed-state feasibility and true-state safety. Under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at **3.9% of steps with P95 severity of 333 MWh**, even while observed-state evaluation reports zero violations. The defended evidence surface is Battery + nuPlan AV + Healthcare: Battery is the witness row, AV is bounded by the all-zip grouped nuPlan replay runtime contract, and Healthcare is bounded by retrospective monitoring evidence. On Germany (OPSD, 17,377 rows), the battery-domain DC3S implementation delivers 7.11% cost savings, 0.30% carbon reduction, and 6.13% peak shaving; stochastic value remains positive in both DE and US regions (VSS = 2,708.61 and 297,092.71 respectively) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). All quantitative claims are tied to versioned run artifacts and checked against the locked release evidence.
...
The long-form paper also imports the thesis assumption register, including the A1-A13 master table and the chapter-by-chapter assumption usage map that supports the theorem block.
