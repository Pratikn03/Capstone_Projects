...
For T9 and T10 specifically, promotion status is governed only by
\texttt{reports/publication/theorem\_promotion\_scorecard.json}; the dashboard and
manuscript must keep both rows non-promoted while any three-domain discharge gate is
open.
...
Certificates are temporal objects. T5 (Certificate Validity Horizon) is the
constructive horizon definition: a certificate issued at time~$t$ remains valid
for the largest integer horizon~$\tau_t$ whose forward tube stays inside the
safe SOC bounds. T6 (Certificate Expiration Bound) adds a confidence-aware
...
T9 is a draft extension surface in the active package, not a promoted theorem.  The
statement below records the intended lower-bound shape; it becomes promotion-eligible
only after the generated package records complete Battery, AV, and Healthcare
discharge for the witness constant, mixing bridge, mechanized proof, and remaining
promotion gates.
...
\begin{proposition}[T9: Reliability-aware repair is necessary]
\label{thm:prof-t9}
...
T10 is likewise a draft extension surface.  The active generated package keeps it
non-promoted until all three domains discharge the unsafe-side boundary-mass and TV
bridge assumptions, and until the mechanized proof and proof-rigor gates pass.
...
\begin{proposition}[T10: Reliability-risk frontier]
\label{thm:prof-t10}
