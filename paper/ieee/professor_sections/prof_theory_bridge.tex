...
\(K(o)=\cap_{x\in B(o)}C(x)\). If \(K(o)=\varnothing\), no
observation-only mandatory-release controller can guarantee true-state safety
for every \(x\in B(o)\).
\end{theorem}

\begin{proof}[Proof sketch]
The controller must choose one action \(a=\pi(o)\). Empty common safe core means
no single action belongs to all \(C(x)\) for \(x\in B(o)\), so some latent state
in the ambiguity class makes that mandatory release unsafe.
\end{proof}
...
\begin{proof}[Proof sketch]
This is a Le Cam style reduction.  The two disjoint safe-action sets mean one
released action must be wrong for at least one latent state.  The smaller the
total variation gap between the two observation laws, the less information the
controller has about which state generated the observation.  The two-point
testing inequality gives the displayed worst-case risk lower bound.
\end{proof}
