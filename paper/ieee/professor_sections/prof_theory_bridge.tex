...
\subsection{T5--T7: Temporal certificate and fallback guarantees}
...
adds a confidence-aware closed-form first-passage lower bound on the remaining certificate
life, using the current boundary gap and the drift proxy $\sigma_d$. Theorem~T7
(Feasible Fallback Existence) then closes the operational loop with a piecewise battery
fallback: safe hold on the interior, boundary-aware safe landing near the boundary, and
fail-closed behavior when neither case is certifiable. These surfaces are runtime-enforced:
when the T6 lower bound drops below one step, the guarantee check fails and the shield
must discharge the promoted T7 fallback contract.
