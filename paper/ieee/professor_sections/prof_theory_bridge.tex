...
\subsection{T5 and T6: Temporal certificate guarantees}
Certificates are temporal objects. Theorem~T5 (Certificate Validity Horizon) proves that a
certificate issued at time~$t$ remains valid for the largest integer horizon~$\tau_t$ whose
forward tube stays inside the safe SOC bounds. Theorem~T6 (Certificate Expiration Bound)
gives a closed-form lower bound: $\tau_t \ge \lfloor(E_{\max} - E_t)/\sigma_d\rfloor$,
where $\sigma_d$ is the drift scale. Both are now runtime-enforced: when $\tau_t < 1$, the
guarantee check fails and the shield enters fallback.
