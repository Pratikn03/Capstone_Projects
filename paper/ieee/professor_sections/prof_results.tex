...
witness row and then at bounded depth across AV and healthcare.
...
Battery is the witness row. AV and healthcare are defended bounded rows with full
certificate rates and bounded runtime semantics. That is exactly the disciplined claim
...
\input{generated/orius_deployment_validation_scope.tex}
...
validation. Future additional domains remain outside the defended lane precisely so
that the paper's universal architecture claim stays larger than a single benchmark
number but smaller than a universal deployment claim.
