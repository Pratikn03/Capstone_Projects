...
\begin{table}[h]
\caption{Battery fault-family conformal miscoverage and mean interval width.
``Miscoverage'' is the fraction of steps on which the true SoC fell
\emph{outside} the conformal interval; lower is better and zero indicates
100\% containment.}
...
\begin{tabular}{llrr}
\toprule
\textbf{Controller} & \textbf{Fault family} & \textbf{Miscoverage} & \textbf{Mean width (MWh)} \\
\midrule
...
\begin{itemize}[nosep]
  \item 1{,}152 total certificates across the four promoted battery runtime variants.
...
The governance pipeline issues 1{,}152 certificates with full chain validity.  The
FTIT variant correctly responds to degraded telemetry by tightening intervals and
...
\begin{itemize}[nosep]
  ...
  \item DC3S achieves \textbf{TSVR = 0.0\%} in both markets, confirming T2 (Safety
        Preservation) and T3 (Core Bound under the observed $\bar{w} \in [0.905, 0.999]$
        range; see Tables~\ref{tab:battery_runtime} and~\ref{tab:battery_fault}).
...
The reliability-weight trace records the time series of $\wt$ across the 288 steps
for each fault scenario.  The key patterns are:
...
\Cref{fig:battery_safety_metrics} presents the primary battery safety--utility
diagnostic exported with the current artifact bundle: intervention rate and
graceful-degradation quality (GDQ) for the promoted FTIT wrapper relative to the
wrapped controller.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig_battery_deep_oqe_safety_metrics}
\caption{Battery domain safety--utility diagnostics: intervention rate and GDQ for
the promoted FTIT wrapper and the non-FTIT wrapped controller.  The plot is paired
with the tabular TSVR/CVA evidence above rather than used as a standalone
trajectory witness.}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.86\textwidth]{fig_observed_vs_true_counterexamples}
...
For the battery domain with $\alpha = 0.10$, $T = 288$, and the worst observed
episode-average reliability $\bar{w} = 0.905$ (deep lane under dropout):
\[
  \E[V_T] \le 0.10 \times 0.095 \times 288 \approx 2.74 \text{ violations.}
\]
