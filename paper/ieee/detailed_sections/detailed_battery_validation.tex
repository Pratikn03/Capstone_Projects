...
CPSBench replay episode with DeepOQE neural state-of-charge (SoC) forecasting.
...
\paragraph{Controller configurations.}  Four controller variants are evaluated:
\begin{enumerate}[nosep]
  ...
  \item \texttt{heuristic:dc3s\_ftit} --- rule-based dispatch with
        fault-tolerant interval tightening (FTIT) variant.
  \item \texttt{deep:dc3s\_wrapped} --- DeepOQE neural dispatch wrapped in \dcs{}.
  \item \texttt{deep:dc3s\_ftit} --- DeepOQE dispatch with FTIT variant.
\end{enumerate}
...
\begin{enumerate}[nosep]
  \item \textbf{Zero true-state violations}: All four controllers achieve
  ...
  \item \textbf{Intervention rate}: The dc3s\_wrapped controllers (0.000) never
        intervene because the heuristic and DeepOQE dispatchers already produce
...
\begin{itemize}[nosep]
  ...
  \item The \textbf{deep} lane has a lower mean reliability weight (0.905) even
        under nominal conditions due to the inherent uncertainty of the DeepOQE
        neural forecaster.
\end{itemize}
...
\subsection{Detailed DeepOQE Model Architecture}

The DeepOQE (Deep Observation Quality Estimator) model is a quantile regression
neural network that predicts the SoC at the next time step given historical
telemetry.  Key design choices:
...
The DeepOQE model achieves a calibration-set coverage of 97.9\% (target: 95\%),
indicating slight over-coverage that provides an additional safety margin.
...
conditions.  This is because the DeepOQE model introduces inherent prediction
uncertainty that the detect stage interprets as mild degradation.  This built-in
conservatism is a feature, not a bug: it ensures that neural-forecaster-based
dispatchers operate with a safety margin.
...
\begin{enumerate}[nosep]
  ...
  \item the DeepOQE model achieves calibration coverage of 97.9\% (above the
        target 95\%).
\end{enumerate}
