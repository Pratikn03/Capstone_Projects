...
Two inflation forms appear in ORIUS and serve distinct roles, which we make
explicit here to avoid conflation.  The \emph{theoretical envelope} that the
risk-bound proofs (\S\ref{sec:theory}) reference is the \emph{RAC-Cert} form:
...
$\kappa_r = 0.5$ is the reliability penalty, $\kappa_d = 0.3$ is the drift
penalty, $d_t \in \{0,1\}$ is the Page--Hinkley drift indicator, and
$\kappa_s = 0.05$ is the staleness scaling with $s_t$ the staleness count in
seconds.  Equation~\eqref{eq:rac_cert} is a closed-form upper envelope: it
exposes each degradation source as a separate, additively monotone term, which
is the form used to discharge Assumption~A5 (Monotone Bounded Inflation) in the
theorem chain.

The \emph{runtime} executes a simplified, clipped form that collapses the drift
and staleness terms into a single shift-aware score and bounds the total
inflation between $1\times$ and $3\times$:
\begin{equation}\label{eq:runtime_inflation}
  \boxed{\;
  \alpha = \mathrm{clip}\!\bigl(1.0 + 0.7\,(1 - w_t) + 0.5\,s_{\mathrm{shift}},
  \; 1.0,\; 3.0\bigr).
  \;}
\end{equation}
Equation~\eqref{eq:runtime_inflation} is the only inflation function actually
evaluated at runtime.  Its implementation anchors are the AV runtime wrapper,
the training feature builder, and the shift-aware uncertainty adapter; the exact
file and line anchors are recorded in the theorem result cards and claim-evidence
registers rather than repeated as long paths in the manuscript.
The relationship between the two forms is that
Eq.~\eqref{eq:runtime_inflation} is a pointwise lower bound of
Eq.~\eqref{eq:rac_cert} once $s_{\mathrm{shift}}$ is identified with a
conservative combination of $\kappa_d d_t + \kappa_s s_t$, so any safety
guarantee proved under~\eqref{eq:rac_cert} continues to hold under the looser
clipped form whenever Eq.~\eqref{eq:runtime_inflation} dominates the
theoretical envelope, and the runtime is otherwise scoped by the explicit
$\alpha \le 3$ ceiling.

\paragraph{Output.}  An observation-consistent state set with:
\begin{itemize}[nosep]
...
\paragraph{Input.}  The Stage~2 uncertainty set and a
...
\State $\alpha_t \gets \mathrm{clip}(1 + 0.7(1-w_t) + 0.5\,s_{\mathrm{shift}},\, 1, 3)$
  \Comment{\cref{eq:runtime_inflation}; runtime form, $\le$ envelope \eqref{eq:rac_cert}}
\State $\Ut \gets [\mathrm{obs}_t.\hat{x} - \alpha_t\,C_t(\alpha),\; \mathrm{obs}_t.\hat{x} + \alpha_t\,C_t(\alpha)]$
\Statex
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Stage} & \textbf{Mean} & \textbf{Med.} & \textbf{P95} & \textbf{Max} \\
\midrule
...
\begin{itemize}[nosep,leftmargin=*]
  \item \texttt{kernel.py}: five-stage orchestration.
  \item \texttt{contracts.py}: typed runtime objects and adapter protocol.
  \item \texttt{risk\_bounds.py}: T10/T11 risk computations.
  \item \texttt{dc3s/drift.py}: Page-Hinkley drift detector.
  \item \texttt{dc3s/runtime.py}: DC3S runtime loop.
  \item \texttt{battery\_adapter.py}, \texttt{av\_adapter.py}: domain adapters.
\end{itemize}
