% === Section 10: Cross-Domain Synthesis ===
\section{Cross-Domain Synthesis}\label{sec:crossdomain}

The active ORIUS defense lane is a literal three-domain program:
\textbf{Battery} as the witness row, plus bounded defended rows for
\textbf{Autonomous Vehicles} and \textbf{Healthcare Monitoring}.  The role of this
section is not to assert equal-depth theorem closure across all three rows.  It is
to show that the same \dcs{} kernel, contract stack, and governance surface survive
domain change while the evidence tier of each row is stated explicitly.

\subsection{Domain Roles}

\begin{table*}[t]
\caption{Active three-domain roles and evidence posture.}
\label{tab:cross_domain_roles}
\centering\footnotesize
\begin{tabularx}{\textwidth}{p{0.14\textwidth} p{0.17\textwidth} p{0.31\textwidth} X}
\toprule
\textbf{Domain} & \textbf{Role} & \textbf{Runtime denominator} & \textbf{Bounded claim} \\
\midrule
Battery & witness row & locked battery witness runtime & deepest theorem-to-artifact closure \\
AV & defended bounded row & 1{,}531{,}104 all-zip grouped nuPlan replay steps & TTC + predictive-entry-barrier contract \\
Healthcare & defended bounded row & 137{,}262 promoted MIMIC runtime steps & monitoring / alert-release semantics \\
\bottomrule
\end{tabularx}
\end{table*}

\subsection{Metric Comparison}

\Cref{tab:cross_domain_metrics_three} summarises the promoted metrics that are
shared across the three active rows.

\begin{table*}[t]
\caption{Promoted three-domain metric comparison.}
\label{tab:cross_domain_metrics_three}
\centering\footnotesize
\begin{tabularx}{\textwidth}{p{0.30\textwidth} p{0.20\textwidth} p{0.20\textwidth} X}
\toprule
\textbf{Metric} & \textbf{Battery} & \textbf{AV (replay)}$^\ddagger$ & \textbf{Healthcare (retrospective)}$^\dagger$ \\
\midrule
Baseline TSVR           & 0.008300 & 0.289250 & 0.194489 \\
ORIUS TSVR              & 0.000000 & 0.000163 & 0.000000 \\
Absolute TSVR delta     & 0.008300 & 0.289087 & 0.194489 \\
Intervention rate       & 0.020833 & 0.500301 & 0.910099 \\
Certificate-valid release rate & 0.993056 & 0.999924 & 1.000000$^\dagger$ \\
Grouped coverage / status & 0.938 / 1.000 / 0.906 & 0.998 / 0.988 / 0.980 & 0.692 / 0.863 / 0.946 \\
P95 runtime latency (ms) & 0.812 & 0.744 & 0.701 \\
\bottomrule
\end{tabularx}
\end{table*}

{\footnotesize $^\ddagger$ All AV numbers in this column are measured on the
all-zip grouped nuPlan \emph{replay} surface (1{,}531{,}104 steps over 37{,}344
held-out scenarios).  They are not closed-loop road-deployment numbers.}

{\footnotesize $^\dagger$ Healthcare reports certificate-valid release from
runtime-emitted CVA on the promoted retrospective MIMIC export.  This remains a
bounded monitoring result and is not a claim of regulated clinical deployment.}

The canonical runtime release contract witnesses are reported in
\Cref{tab:runtime-release-contract-witnesses}; each row follows the same
observe--reliability--uncertainty--repair/fallback--certificate--append-only audit
release grammar.

\IfFileExists{reports/publication/runtime_release_contract_witnesses.tex}{%
  \input{reports/publication/runtime_release_contract_witnesses.tex}%
}{}

\paragraph{Utility and ablation checks.}
The runtime table alone would be incomplete because a controller can appear safe
by braking, alerting, or denying release too often.  The utility-preserving table
therefore separates two claims: where comparable compact evidence exists, ORIUS
is safer than predictor-only control; against shutdown/fallback-only references,
ORIUS preserves useful work without material excess TSVR.

\IfFileExists{reports/publication/utility_preserving_safety_claim_table.tex}{%
  \input{reports/publication/utility_preserving_safety_claim_table.tex}%
}{}

\IfFileExists{reports/publication/utility_preserving_safety_ablation_surfaces.tex}{%
  \input{reports/publication/utility_preserving_safety_ablation_surfaces.tex}%
}{}

\subsection{Analysis}

\paragraph{Battery witness row.}
Battery remains the deepest reference row.  It is the only domain for which the
theorem narrative, runtime witness, and locked artifact chain are intended to carry
the full constructive story end to end.  The promoted battery row achieves
$\TSVR = 0$, but the purpose of the witness row is not leaderboard dominance; it is
the cleanest theorem-to-code-to-artifact closure for ORIUS under degraded
observation.

\paragraph{Autonomous Vehicles bounded row.}
AV is promoted under the longitudinal TTC + predictive-entry-barrier contract only.
ORIUS reduces all-zip grouped nuPlan replay TSVR from 0.289250 to 0.000163 while the
relative-gap shift table remains in watch status.  This row demonstrates that the
same runtime contract can survive transfer to a faster, interaction-heavy prediction
setting, but it does \emph{not} claim full autonomous-driving field closure.

\paragraph{Healthcare bounded row.}
Healthcare is promoted only as a bounded monitoring and alert-release row.  ORIUS
reduces the bounded TSVR from 0.194489 to 0.000000, with grouped calibration and
governance evidence reported explicitly.  The grouped coverage remains weakest in
the low-reliability bucket, which is exactly why the prose keeps the healthcare row
bounded and avoids any clinical-deployment claim.
The clinical framing is retrospective and source-aware: MIMIC-IV and eICU
define the public critical-care dataset family, while TRIPOD+AI and PROBAST+AI
govern reporting, applicability, and risk-of-bias language
\cite{johnson2023mimiciv,physionet2024mimiciv31,pollard2018eicu,collins2024tripodai,moons2025probastai}.
CONSORT-AI, DECIDE-AI, and STARD-AI are used as boundary references for what the
paper does \emph{not} claim: no prospective trial, no live clinical deployment,
and no diagnostic approval
\cite{liu2020consortai,vasey2022decideai,stardai2025}.

\subsection{Kernel Invariance}

The cross-domain claim that ORIUS \emph{does} defend is kernel invariance under
typed adaptation:
\begin{enumerate}[nosep]
  \item The same \texttt{dc3s} runtime kernel is used across Battery, AV, and
        Healthcare.
  \item The same contract stack and governance surface remain active across all
        three domains.
  \item Domain-specific behavior is isolated in the adapter layer and the
        domain-local safety contract.
\end{enumerate}

This is the correct level of universality for the active branch: \emph{runtime and
governance universality}, not equal-depth empirical closure across every possible
physical-AI domain.

\subsection{What Transfers and What Does Not}

\paragraph{What transfers.}
The five-stage \dcs{} pipeline transfers cleanly: degradation detection, interval
calibration, action-set tightening, repair/fallback, and certificate issuance all
appear on each active row.

\paragraph{What does not transfer automatically.}
The witness strength does not transfer automatically.  Battery remains deeper than
AV and Healthcare.  The defended theorem core therefore treats the rows
asymmetric\-ally: Battery is the constructive witness, while AV and Healthcare are
bounded defended rows that validate the runtime contract under different observation
and actuation semantics.

\subsection{Implications}

The three-domain synthesis supports the flagship ORIUS claim in its reviewer-safe
form: OASG is a runtime hazard of degraded observation, and ORIUS provides a
reliability-aware safety layer that can be operationalized across Battery, AV, and
Healthcare without changing the kernel semantics.  The synthesis does \emph{not}
claim equal-depth theorem closure, a new universal controller, or unrestricted
deployment readiness in any of the bounded rows.
