% === Section 12: Limitations ===
\section{Limitations and Future Work}\label{sec:limitations}

\subsection{Limitations}

\paragraph{Three-domain scope.}
This paper validates ORIUS on three CPS domains.  While the kernel is designed to
be universal, the empirical evidence in the active repo-tracked program covers only
battery energy storage, autonomous vehicles, and healthcare monitoring.  The
universality claim is therefore architectural and contract-based rather than a claim
of broad all-domain empirical closure.

\paragraph{AV denominator and relative-gap weakness.}
The all-zip grouped nuPlan AV row reaches empirical ORIUS $\TSVR=0.000163$
on 1{,}531{,}104 replay steps, inside the explicit $10^{-3}$ runtime gate but
not equal to zero.  The relative-gap shift table remains in \texttt{watch} status,
and relative gap under degraded telemetry is the hardest cell.  The AV result should
be read as a runtime-contract replay closure, not as closed-loop road deployment or
full autonomous-driving field closure.

\paragraph{Conformal coverage assumptions.}
The finite-sample coverage guarantee of split conformal prediction holds
\emph{marginally} (averaged over the calibration set) but not \emph{conditionally}
(for every individual input).  The shift-aware mechanism provides robustness against
subgroup-level violations but does not eliminate them entirely.  Per-input guarantees
would require conditional conformal methods not yet integrated.

\paragraph{Shift guarantees are bounded.}
The shift-aware adaptive mechanism (Section~\ref{sec:shift}) compensates for
distribution drift within the observed rate during validation (A6).  It has
\textbf{not} been evaluated under intentionally adversarial telemetry attacks,
sudden-onset covariate shift, or concept drift that violates the bounded-rate
assumption.  Guarantees degrade gracefully but may fail silently under pathological
shift patterns.

\paragraph{Fallback action conservatism.}
Assumption A4 requires that the fallback action is safe for all states.  In many
domains, designing a universally safe fallback is straightforward (\eg~zero
dispatch for battery, maximum braking for AV).  In domains with coupled dynamics,
identifying a universally safe fallback may be non-trivial.

\paragraph{Single-sensor pipeline.}
The current detect stage focuses on single-channel telemetry faults.  Cross-sensor
consistency checks (\eg~lidar--camera fusion disagreement) are not yet implemented
in the adapter interface.

\paragraph{Computational cost.}
The bounded AV replay run is not a real-time deployment benchmark.  For real-time
operation at 10~Hz, the per-step budget is 100~ms, and the current Python reporting
path would still need compiled inference, batching, and transport integration before
it could support a live vehicle loop.

\paragraph{Cross-domain transfer scope.}
The adapter interface enables structural reuse across domains, but functional
equivalence (whether the same hyperparameters, calibration windows, and inflation
schedules are optimal) has not been demonstrated beyond the active Battery, AV,
and Healthcare rows.  Adapting to a new domain still requires implementing the six
typed adapter methods and conducting a domain-specific calibration run.

\IfFileExists{reports/publication/validation_boundary_table.tex}{%
  \input{reports/publication/validation_boundary_table.tex}%
}{}

\paragraph{Governance and security ablations.}
The runtime contract can also fail through provenance and governance channels rather
than through prediction error.  \Cref{tab:security-governance-ablations} records the
fail-closed ablations used to keep these channels visible: missing model hashes,
bad certificate signatures, stale or replayed device identity, invalid fallback
actions, and corrupted compact release witnesses.

\IfFileExists{reports/publication/security_governance_ablation_matrix.tex}{%
  \input{reports/publication/security_governance_ablation_matrix.tex}%
}{}

\paragraph{Falsification boundaries.}
ORIUS is strongest when it states not only what passes, but also what would falsify
the current bounded claim.  \Cref{tab:orius-failure-falsification} lists the main
ways the present evidence package could fail and the corresponding demotion or
rerun obligation.

\IfFileExists{reports/publication/orius_failure_modes_falsification_table.tex}{%
  \input{reports/publication/orius_failure_modes_falsification_table.tex}%
}{}

\subsection{Future Work}

\begin{enumerate}[nosep]
  \item \textbf{Additional domains}: Extend validation only after a new domain is
        admitted into the active program with matching replay, calibration,
        runtime, and governance surfaces.
  \item \textbf{Stronger predictors}: Replace LightGBM with transformer-based
        trajectory predictors and larger nuPlan denominators to stress the AV row.
  \item \textbf{Conditional coverage}: Integrate conditional conformal prediction
        methods (\eg~locally-adaptive intervals) for per-input guarantees.
  \item \textbf{Multi-sensor fusion}: Extend the detect stage to perform
        cross-sensor consistency checks using the adapter interface.
  \item \textbf{Real-time deployment}: Optimise the runtime for sub-100~ms latency
        via compiled inference and hardware acceleration.
  \item \textbf{Formal verification}: Apply model-checking or SMT-based verification
        to the kernel contracts for certified correctness beyond testing.
  \item \textbf{Adversarial robustness}: Evaluate ORIUS under intentionally
        adversarial telemetry attacks beyond the fault-injection families tested here.
\end{enumerate}
