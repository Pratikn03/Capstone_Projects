...
is certified near the safe boundary whenever feasible, and the runtime fails closed
otherwise.
\end{assumption}
...
\toprule
\textbf{Theorem} & \textbf{A1} & \textbf{A2} & \textbf{A3} & \textbf{A4} & \textbf{A5} & \textbf{A6} & \textbf{A7} & \textbf{A8} & \textbf{A9} \\
\midrule
...
T7 Feasible Fallback      &\checkmark &  &  &\checkmark &  &  &  &\checkmark &  \\
...
\begin{proof}[Proof sketch]
The lower bound conditions on each observation class and chooses the best
single action available to any observation-only policy.  If the common safe
core is empty, no mandatory-release action is safe for every state in the
class.  For ORIUS, violations can occur only on the coverage-miss event,
because on $X_t^\star\in\mathcal{X}_t$ the released action is safe for every
state in $\mathcal{X}_t$, hence for the true state.
\end{proof}
...
\begin{itemize}[nosep]
  \item \textbf{Foundation} (T1, T4): OASG existence + observation necessity establish
