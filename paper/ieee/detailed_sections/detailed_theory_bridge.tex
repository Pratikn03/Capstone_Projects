...
Typed transfer and covered optimality & T11; T10/T11 corollary & The typed ORIUS contract transfers to defended domains, with certificate/fallback discipline (T5--T8) and the covered Bayes lower/upper safety sandwich as supporting surfaces.\\
\bottomrule
\end{tabularx}
\end{table}

T5--T10 remain visible but scoped supporting surfaces.  They are not promoted
as additional headline theorem claims in this paper-facing draft; the generated
theorem-promotion package remains the authority for any future promotion.
...
\toprule
\textbf{Result} & \textbf{A1} & \textbf{A2} & \textbf{A3} & \textbf{A4} & \textbf{A5} & \textbf{A6} & \textbf{A7} & \textbf{A8} & \textbf{A9} \\
\midrule
...
\begin{proposition}[T8: Graceful Degradation Dominance]\label{thm:T8}
...
remain supporting lower-bound extensions until their universal constants and
assumptions are discharged across Battery, AV, and Healthcare. The authority for
their status is \texttt{reports/publication/theorem\_promotion\_scorecard.json},
not the raw theorem inventory or this prose.

\begin{proposition}[T9: Scoped Universal Impossibility --- Reliability-Aware Repair is
...
\paragraph{Interpretation.}  T9 is retained as a draft structural-necessity
extension. It is not promoted until the generated package records a discharged
positive witness constant and mixing bridge in Battery, AV, and Healthcare.
...
Under the explicit boundary-mass model above, T10 gives the draft frontier that the
minimum mean reliability needed to achieve an episode
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Construction} (T2, with T3 as a corollary): covered uncertainty
        ...
        guarantee.
  \item \textbf{Operationalization} (supporting T5--T8): certificate horizon, expiration,
...
The generated theorem-promotion package confirms that T9 and T10 are explicitly
non-promoted until the three-domain discharge, mechanized proof, proof-rigor, and
artifact-evidence gates all pass. The theorem audit register remains a metadata
inventory; it is not the authority for promoting T9/T10.
