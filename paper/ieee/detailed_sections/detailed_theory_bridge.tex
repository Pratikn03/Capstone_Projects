...
Typed transfer and ambiguity sandwich & T11; T10/T11 corollary & The typed ORIUS contract transfers to defended domains, with certificate/fallback discipline (T5--T8) and the covered lower/upper safety sandwich stated only under its scoped ambiguity class.\\
\bottomrule
\end{tabularx}
\end{table}
...
Under A1, A2, A4, and the boundary-reachability witness, let $\pi$ be a
fixed-margin quality-ignorant mandatory-release controller with margin
$m^\pi$.  If an admissible degraded-observation gap exceeds that fixed margin
at a boundary-sensitive release step, then there exists an admissible fault
sequence on which $\pi$ incurs a true-state violation.  Equivalently, on this
witness class, mandatory non-intervention cannot certify $\TSVR=0$.
\end{theorem}
...
with $\xi>m^\pi+\epsilon$.  A fixed-margin mandatory controller that does not
intervene treats the observed state as feasible while the same released dispatch
pushes the true SoC above $\SOC_{\max}$.  The result is scoped to this
fixed-margin, boundary-sensitive witness; adaptive margins, abstention,
fallback, and fail-closed denial are outside the lower-bound class.
\end{proof}
...
controllers with no abstention action.  For an observation $o$, define
$B(o)=\{x:h(x)=o\}$ and $K(o)=\cap_{x\in B(o)}C(x)$.  If
$B(o)\ne\emptyset$ and $K(o)=\varnothing$, then no
$\pi\in\Pi_{\mathrm{obs}}$ can guarantee true-state safety for all
$x\in B(o)$.
\end{proposition}

\begin{proof}
A total mandatory-release controller chooses a single action $a=\pi(o)$ for the
nonempty ambiguity class.  Since the common safe core is empty, no single action lies in every $C(x)$ for
...
\begin{proof}
For a deterministic release rule, define
$S_i=\{o:\pi(o)\in C(x_i)\}$.  Disjoint safe-action sets imply
$S_0\cap S_1=\emptyset$.  The average release success probability is
$\frac12(P_0(S_0)+P_1(S_1))$.  Since $S_1\subseteq S_0^c$, the test that
decides $x_0$ on $S_0$ and $x_1$ otherwise succeeds with probability at least
that release-success value.  The best testing success is
$(1+d_{\mathrm{TV}}(P_0,P_1))/2\le(1+\epsilon)/2$, so the average release loss
is at least $(1-\epsilon)/2$.  The maximum of the two state-conditioned risks is
at least that average.  Randomized controllers follow by conditioning on the
internal seed and averaging.
\end{proof}
...
\begin{corollary}[Covered Observation-Ambiguity Safety Sandwich]
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Operationalization} (scoped T5--T8): certificate horizon, expiration,
        ...
        fail-closed.
  \item \textbf{Transfer and scoped ambiguity} (T11 plus the T10/T11 ambiguity
        corollary): typed obligations transfer the argument across defended
        domains, while the lower/upper sandwich states only the covered
