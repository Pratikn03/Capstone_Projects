...
Unlike broader planning drafts that motivated ORIUS with additional candidate domains,
this manuscript restricts its empirical claim surface to \textbf{three active domains}
...
\begin{enumerate}[nosep]
        ...
        primary validation domain and carries the deepest artifact-to-proof closure.
  ...
  \item \textbf{Medical and Healthcare Monitoring}: bounded monitoring and alert release
        under degraded vital-sign observations.  Healthcare is validated as a bounded
        MIMIC-backed runtime domain; its claims remain narrower than the battery domain.
\end{enumerate}

The three active domains share the same kernel binary, the same contract
...
\begin{enumerate}[nosep]
  ...
  \item A bounded three-domain empirical lane:
        \begin{itemize}[nosep]
          \item \textbf{Battery primary validation domain}: zero true-state violations on the promoted
                runtime slice, with the deepest theorem-to-artifact alignment.
          \item \textbf{AV bounded validation domain}: 1{,}531{,}104 all-zip grouped nuPlan replay
          ...
          \item \textbf{Healthcare bounded validation domain}: TSVR reduced from 0.194489 to 0.000000 on
...
proof posture is tracked theorem-by-theorem in the active theorem-evidence matrix
...
\begin{itemize}[nosep]
  \item The Battery domain achieves $\TSVR = 0$ (zero true-state violations) and
        serves as the \emph{primary validation domain}: it demonstrates the full pipeline
        end-to-end with the deepest artifact-to-theorem closure.
  \item The AV domain is a \emph{bounded validation} domain: ORIUS is evaluated only
  ...
  \item The Healthcare domain is a \emph{bounded validation} domain: ORIUS is evaluated
        only for monitoring and alert-release semantics on the MIMIC-backed healthcare lane.
        Certificate-valid release is reported from runtime-emitted healthcare CVA on this surface.
