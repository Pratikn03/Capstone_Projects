...
T6  & Certificate Expiration Lower Bound & A7          & Proof + Empirical \\
T7  & Feasible Fallback Existence       & A4           & Proof + Code \\
...
All 11 theorems have complete formal proofs.  The ``Empirical'' tag indicates that
the theorem's predictions match the observed pipeline metrics.  The ``Code'' tag
...
The full fault-family coverage matrix (24 rows: 6 fault families $\times$ 2 targets
$\times$ 2 controllers) is recorded in
\texttt{reports/orius\_av/full\_corpus/tables/fault\_family\_coverage\_table.csv}.
The ORIUS controller achieves coverage $\ge 0.977$ in every cell; the baseline ranges
from 0.881 to 0.958.
...
\begin{itemize}[nosep]
  \item \textbf{Summary artifacts}: all CSV, JSON, and PNG files in
        \texttt{reports/battery\_av/} and \texttt{reports/orius\_av/full\_corpus/}.
...
\begin{itemize}[nosep]
  \item \textbf{AV audit database}: \texttt{dc3s\_av\_waymo\_dryrun.duckdb}
        ($\sim$120~MB).  Regenerate locally via
        \texttt{scripts/build\_waymo\_av\_dry\_run\_report.py}.
  \item \textbf{Raw Waymo Motion Dataset shards}: subject to Waymo licence;
        not redistributable.  Download from \url{https://waymo.com/open}.
...
A cold-start user with access to the Waymo licence can regenerate all artifacts
from scratch using the single command \texttt{make pipeline-full}.  The
