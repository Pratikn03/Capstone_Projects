...
the promoted AV runtime manifest.  The processed source manifest records completed
nuPlan archives and excludes partial downloads and map-only archives before feature
construction.  The runtime report contains 1{,}531{,}104 canonical ORIUS replay
rows over 37{,}344 held-out test scenarios.
...
\begin{table}[h]
\caption{ORIUS fault-family coverage on the all-zip grouped nuPlan replay
row.  Target coverage is $1-\alpha = 0.90$; values $\ge 0.90$ meet target
(all ego-speed rows; six of six dropout/spikes/delay\_jitter rel.\ gap rows
miss by $\le 0.07$), values $< 0.90$ miss it (the stale rel.\ gap row at
0.914 is borderline and the drift\_combo rel.\ gap row at 0.919 just meets;
the table is reported without averaging so the harder cells stay visible).}
\label{tab:av_fault}
\centering\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{llrr}
...
\Cref{tab:av_shift_aware_runtime} reports the shift-aware runtime table emitted by the
all-zip grouped nuPlan run.

\begin{table}[h]
\caption{Shift-aware statistics for the all-zip grouped nuPlan AV row.}
\label{tab:av_shift_aware_runtime}
\centering\small
\resizebox{\linewidth}{!}{%
...
\begin{tabularx}{\linewidth}{p{0.25\linewidth}Xr}
\toprule
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{training_interval_widths}
\caption{All-zip grouped nuPlan conformal interval widths.  This full-width
render is used in place of a compressed panel so that target/horizon labels and
relative-gap widening remain legible.}
\label{fig:av_training_widths}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{shift_aware_runtime}
\caption{All-zip grouped nuPlan shift-aware runtime status.  Ego speed remains
nominal while relative gap is retained as a watch-status limitation.}
\label{fig:av_shift_aware}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{fault_family_coverage_heatmap}
\caption{All-zip grouped nuPlan fault-family coverage heatmap.  Every
controller--target row and every degradation family is retained, but the plot is
rendered as a heatmap rather than a crowded legend-over-bar chart.}
\label{fig:av_fault_coverage}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{runtime_metrics}
\caption{All-zip grouped nuPlan runtime comparator metrics.  ORIUS is shown
against the no-wrapper baseline, RSS/CBF-style filter, and always-brake control
while the claim remains bounded to replay evidence rather than closed-loop
autonomy.}
