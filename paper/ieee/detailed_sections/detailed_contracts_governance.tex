...
Both domains achieve 100\% chain validity, zero failure rows, and 100\% audit
completeness.
...
\begin{itemize}[nosep]
  \item \textbf{Battery}: 288 certificates verified, \texttt{chain\_valid = true},
        0 failures.
  \item \textbf{AV}: 9{,}348 certificates verified, \texttt{chain\_valid = true},
        0 failures.
\end{itemize}
...
Multi-domain validated\textsuperscript{$\dagger$} & \checkmark & ---  & ---  & ---  & ---  & --- \\
\bottomrule
\end{tabular}
\end{table}

{\footnotesize $\dagger$ Validated on two domains (Battery, AV) with locked artifacts; four additional domains are architectural only.}

\paragraph{Key differentiators.}
\begin{itemize}[nosep]
  \item \textbf{Simplex}~\cite{sha2001simplex} provides safety switching between a
