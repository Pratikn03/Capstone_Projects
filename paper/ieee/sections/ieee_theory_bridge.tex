...
Temporal certificates add a fourth layer: Theorem~T5 (certificate validity horizon)
and Theorem~T6 (certificate expiration bound) guarantee that certificates are
time-bounded safety objects rather than approval stamps.  The runtime now enforces
$\tau_t \ge 1$ at each step, failing the guarantee check when the certificate's
forward tube exits the safe region before the next decision epoch.
