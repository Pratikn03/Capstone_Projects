...
of the pipeline.  A configurable stale-decay rate ensures freshness drops below the
shield threshold within a bounded number of stale steps, closing Assumption~A6.  An
optional adversarial mode overlays a Byzantine-robust estimator that prevents a spoofed
sub-score from inflating the aggregate reliability.
...
Domain-typed inflation multipliers ensure that each CPS domain's baseline uncertainty
scale is respected, while an online SPCI-lite quantile tracker~\cite{gibbs2021adaptive}
adapts the conformal interval width to distribution shift without a full recalibration
pass.
