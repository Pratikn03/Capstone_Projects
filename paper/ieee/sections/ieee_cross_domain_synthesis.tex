...
The paper therefore keeps the promotion boundary visible. Battery is the witness row.
AV is defended as an all-zip grouped nuPlan replay row. Healthcare is defended as a bounded
retrospective monitoring row. Future additional domains are not
smuggled into the public claim surface as quiet unverified rows. This is not defensive
writing. It is the mechanism that lets the paper make a large architectural claim
without pretending that universality of contract automatically implies equal evidence
depth.

\subsection{Three-domain evidence boundary}
The public claim surface is deliberately three-row. Battery establishes the witness
standard: theorem-facing assumptions, runtime traces, fallback semantics, and CertOS
evidence are all present in the same row. The AV row tests whether the same release
...
Healthcare row tests the same grammar in retrospective monitoring, but it remains a
