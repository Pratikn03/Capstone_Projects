...
accelerate on stale headway estimates, industrial plants can cross thermal envelopes,
clinical monitoring systems can suppress necessary intervention, and guidance systems
can drift outside feasible corridors. ORIUS is built around the claim that this failure
...
surface. The adapter contract allows batteries, autonomous vehicles, industrial plants,
healthcare monitoring, navigation, and aerospace to instantiate the same safety layer
without pretending they share the same nominal controller or safety objective.

The evidence posture of this flagship draft is intentionally visible. ORIUS already has
one witness row and several defended bounded rows, but equal-domain universality is
still gated by missing real-data navigation closure and a stronger aerospace runtime
surface. The paper therefore argues for a field-level runtime architecture while keeping
the domain promotion gate explicit in the body text rather than hiding it in editorial
support material.
...
\item It unifies benchmark and governance semantics across six domains using one
true-state-violation schema and one auditable runtime discipline.
\item It argues, with explicit parity gating, that runtime safety layers are the right
universal object for Physical AI safety research even when evidence depth remains
asymmetric across domains.
\end{enumerate}
...
bridge, walks through six domain instantiations under one shared template, and closes
with a synthesis that separates universality of contract from equality of evidence.
