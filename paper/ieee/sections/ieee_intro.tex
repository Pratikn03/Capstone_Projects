...
The paper is intentionally universal-first. Battery is the primary validation domain
because it has the deepest theorem-to-code-to-artifact closure, but it is not the
conceptual center of the argument. Instead, the central object is the
observation--action safety gap (OASG): the event in which an action is legal on the
observed state while unsafe on the true state. That object links conformal and
...
across battery storage, autonomous-vehicle replay, and healthcare monitoring.}
\end{quote}
...
surface. The adapter contract allows the Battery, AV, and Healthcare domains to
instantiate the same safety layer without pretending they share the same nominal
controller or safety objective.

The evidence posture is intentionally visible. ORIUS has one primary validation domain
and two bounded cross-domain validation domains. The paper therefore argues for a
field-level runtime architecture while keeping the three-domain evidence boundary
explicit in the body text.
...
\item It exposes one cross-domain typed release contract for the active three-domain
validation program,
...
contribution by distinguishing primary validation from bounded cross-domain validation
instead of hiding claim depth behind a flat universality narrative.
\end{enumerate}
