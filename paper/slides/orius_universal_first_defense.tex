...
\begin{frame}{Thesis Claim}
  \begin{itemize}
    ...
    \item The monograph now reads as one argument: hazard $\rightarrow$ architecture $\rightarrow$ theory $\rightarrow$ domains $\rightarrow$ parity gate.
    \item The book is universal-first in structure and parity-gated in evidence.
    \item Battery remains the reference witness; industrial, healthcare, and bounded AV are defended rows; navigation and aerospace remain explicit open gates.
  \end{itemize}
\end{frame}
...
\begin{frame}{Equal-Domain Parity Gate}
  \centering
...
\begin{frame}{Current Domain Status}
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.24\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}p{0.44\textwidth}}
    ...
    Industrial & Proof-validated & Strong defended non-battery row \\
    Healthcare & Proof-validated & Bounded monitoring/intervention row \\
    Navigation & Shadow-synthetic & Real-data row still missing \\
    Aerospace & Experimental & Placeholder flight-task surface still open \\
    \bottomrule
  \end{tabular}
\end{frame}
...
\begin{frame}{R1 Review Program}
  \begin{itemize}
    ...
    \item Review conclusion: structure is now R1-grade; remaining blockers are evidence symmetry in navigation and aerospace.
  \end{itemize}
\end{frame}
...
\begin{frame}{What Is Ready Now}
  \begin{itemize}
    ...
    \item parity gate artifact and review dossier
    \item six domain chapters under one common template
    \item explicit claim boundaries for defended vs open rows
  \end{itemize}
\end{frame}

\begin{frame}{What Still Blocks Flat Equal-Domain Rhetoric}
  \begin{itemize}
    \item Navigation: defended real-data train/validate/replay chain still not complete
    \item Aerospace: real multi-flight safety task and non-placeholder replay surface still missing
    \item The next technical program is parity closure, not conceptual reframing
  \end{itemize}
\end{frame}

\begin{frame}{Takeaway}
  \begin{block}{Bottom line}
    ORIUS is now packaged as a serious universal-first monograph with a clean R1-style review surface. The strongest honest submission claim is a universal physical-AI safety framework with asymmetric but explicit domain evidence.
  \end{block}
\end{frame}
