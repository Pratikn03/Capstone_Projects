...
\textbf{Question.} Table~\ref{tab:all_domain_comparison} shows ORIUS TSVR (0.9375)
exceeds baseline TSVR (0.8750) for the battery reference domain.  Is DC3S
\emph{harmful} for the primary application?

\textbf{Response.}
The battery domain uses the locked DE/US witness benchmark, where the baseline
...
hidden physical model (the key metric in the locked battery witness results).
...
and~\ref{tab:adversarial_tsvr}) provides the empirical counterpart: all five proof
domains pass the evidence gate on ORIUS-Bench synthetic benchmarks.
...
pipeline, and hardware-in-loop results that the proof domains do not yet replicate.
