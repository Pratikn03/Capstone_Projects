...
\begin{abstract}
ORIUS is a universal physical-safety framework for cyber-physical control under degraded observation. This manuscript presents one shared Detect--Calibrate--Constrain--Shield--Certify loop across a tiered six-row runtime package: battery dispatch is the reference domain, industrial process control and medical monitoring are proof-validated peers, autonomous vehicles is a proof-candidate row, navigation is shadow-synthetic, and aerospace remains experimental. The reference battery result remains the observation-action safety gap that motivated the framework: under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. Under the locked cross-domain gate, industrial process control and medical monitoring reduce true-state violation rate from \textbf{21.53\% to 0.0\%} and \textbf{6.25\% to 0.0\%} respectively; autonomous vehicles retains locked training and software-in-loop artifacts but remains proof-candidate because the current promotion gate does not yet clear a material-improvement requirement. Battery remains the reference domain for the locked economics and theorem-to-artifact lineage: on Germany (OPSD, 17{,}377 rows), the battery-domain DC3S implementation delivers \PaperMetric{DE_COST_SAVINGS_PCT}{7.11}\% cost savings, \PaperMetric{DE_CARBON_REDUCTION_PCT}{0.30}\% carbon reduction, and \PaperMetric{DE_PEAK_SHAVING_PCT}{6.13}\% peak shaving, while stochastic value remains positive in both DE and US regions (VSS\,=\,\PaperMetric{DE_VSS}{2,708.61} and \PaperMetric{US_VSS}{297,092.71}) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). Every quantitative claim is tied to versioned run artifacts, theorem registers, and release gates.
\end{abstract}
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{Tiered universal runtime validation.} We present ORIUS as a universal physical-safety framework organized around one five-stage Detect--Calibrate--Constrain--Shield--Certify loop and evaluate that loop across a tiered six-row runtime package with one reference row, two proof-validated peers, one proof-candidate row, one shadow-synthetic row, and one experimental row.
  ...
  \item \textbf{Shared cross-domain promotion gate.} We use one locked replay harness and one promotion contract across the non-battery rows, with industrial and healthcare clearing the proof-validated gate, AV remaining proof-candidate, navigation remaining shadow-synthetic, and aerospace remaining experimental.
  \item \textbf{Shared theorem package with tiered instantiations.} We keep one ORIUS theorem ladder and make its state, observation, safety, constraint, repair, code, and artifact instantiations explicit across the battery reference row and the non-battery replay rows without claiming equal proof depth for every domain.
