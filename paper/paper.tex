...
The unified validation harness (\texttt{scripts/run\_universal\_orius\_validation.py}) runs all six ORIUS-Bench tracks---battery, navigation, industrial, healthcare, aerospace, vehicle---with fault episodes (bias, noise, stuck\_sensor, blackout) and aggregates TSVR (true-state violation rate) and OASG metrics. \Cref{tab:TBL09_CROSS_DOMAIN_OASG} reports the cross-domain table from the locked run, while the companion validation report records that all six configured tracks completed without harness failure. The artifact is therefore best read first as evidence that one shared ORIUS-Bench protocol can execute comparable fault episodes across multiple domains. The TSVR outcomes themselves are mixed: several domains remain at zero in both baseline and ORIUS paths, while battery and vehicle expose non-zero rates and therefore do not support a universal dominance claim. Accordingly, the current cross-domain surface supports framework portability and auditability, not cross-domain superiority.
