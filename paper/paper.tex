...
\midrule
\multicolumn{4}{l}{\textit{Universal (synthetic): battery, navigation, industrial, healthcare, aerospace, vehicle $\to$ TBL09}} \\
\bottomrule
\end{tabular}
\end{table}
...
The paper theorems map to the canonical battery-8 theorem register (thesis Appendix~M). Compact paper form: \Cref{thm:rac_coverage} $\equiv$ T3 (ORIUS Core Bound); \Cref{thm:safety_margin} and \Cref{cor:zero_violation} $\equiv$ T2 (One-Step Safety Preservation); \Cref{thm:conditional_coverage} is an auxiliary Mondrian diagnostic. The full longform block (\Cref{sec:theorem_validation_longform}) and thesis chapters ch16--ch20 state all eight theorems: T1 OASG Existence, T2 One-Step Safety Preservation, T3 ORIUS Core Bound, T4 No Free Safety, T5 Certificate Validity Horizon, T6 Certificate Expiration Bound, T7 Feasible Fallback Existence, T8 Graceful Degradation Dominance. \Cref{tab:theorem_register_code} maps each to its implementation anchor. The full evidence mapping is in \texttt{orius-plan/theorem\_to\_evidence\_map.md}.

\begin{table}[htbp]
\centering\small
\caption{Battery-8 theorem register: T1--T8 and implementation anchors.}
...
\Cref{tab:universal-state} defines the state schema per domain. Energy uses SOC, load, and renewables; AV uses longitudinal kinematics (position, speed, lead vehicle); Industrial uses process variables (temperature, pressure, power); Healthcare uses vital signs (heart rate, SpO$_2$, respiratory rate); Aerospace uses flight-envelope state (altitude, airspeed, bank angle, fuel); and Navigation uses 2D robot position $(x, y)$ with velocity in a bounded arena. \Cref{tab:universal-safety} summarizes the safety predicates. \Cref{tab:universal-pipeline} lists the pipeline stages and adapter methods.
