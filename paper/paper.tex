...
\begin{abstract}
ORIUS studies a domain-agnostic safety problem: under degraded telemetry, a controller can be observationally safe while physically unsafe. This manuscript formalizes that observation--reality gap and implements one typed Detect--Calibrate--Constrain--Shield--Certify kernel that maps degraded telemetry to observation-consistent state sets, robust safe-action sets, repaired actions, and causal safety certificates. The battery domain remains the locked reference witness: under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. The same kernel is instantiated across a tiered six-row evidence ladder: industrial process control and medical monitoring are proof-validated defended peers, autonomous vehicles remains proof-candidate, navigation is shadow-synthetic, and aerospace remains experimental. The strongest supported claim is therefore not universal dominance across all domains, but a reusable degraded-observation repair contract with explicit assumption tags, auditable certificates, and locked validation artifacts. Every quantitative claim is tied to versioned run artifacts, theorem registers, and release gates.
\end{abstract}
...
ORIUS treats that failure mode as a general cyber-physical systems problem rather than a battery-specific quirk. Whenever a controller acts on degraded observation of a physical state, an observation-action safety gap can emerge between what appears feasible on the observed channel and what is actually safe in the physical system. The framework contribution of this paper is therefore architectural as well as empirical: one shared Detect--Calibrate--Constrain--Shield--Certify loop instantiated across a tiered cross-domain evidence package, with battery serving as the locked reference surface for economics and theorem-to-artifact lineage, industrial process control and medical monitoring acting as proof-validated peers, autonomous vehicles remaining proof-candidate, navigation shadow-synthetic, and aerospace experimental. In code, that reusable contract is now materialized explicitly in \path{src/orius/universal_theory/} as typed runtime objects for observation packets, reliability assessments, observation-consistent state sets, safe-action sets, repair decisions, and safety certificates.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{Universal degraded-observation kernel.} We present ORIUS as one typed Detect--Calibrate--Constrain--Shield--Certify kernel for safe control under degraded observation, with explicit runtime objects for observation-consistent state sets, repaired actions, and causal safety certificates.
  ...
  \item \textbf{Tiered defended-domain validation.} We use one locked replay harness and one promotion contract across the non-battery rows, with industrial and healthcare clearing the proof-validated gate, AV remaining proof-candidate, navigation remaining shadow-synthetic, and aerospace remaining experimental.
  \item \textbf{Theorem-to-code contract.} We make the reusable kernel explicit in code rather than leaving universality at the adapter story level: the theorem-facing objects and assumption tags are now attached to the runtime contract itself.
  \item \textbf{Artifact-locked universal evidence bundle.} The manuscript consumes the locked status, proof, training, SIL, comparison, and latency artifacts emitted by the universal validation workflow, and uses those tier tables as the governing boundary for cross-domain claims.
\end{enumerate}

\subsection{Contribution Boundary}

The central scientific contribution is now a tiered universal package rather than battery-plus-portability narrative. ORIUS is the reusable degraded-observation kernel; battery is the formal reference row, industrial and healthcare are the strongest non-battery empirical peers, AV remains proof-candidate, navigation remains shadow-synthetic, and aerospace remains experimental. The manuscript still keeps a bounded formal layer consisting of the ORIUS theorem package together with typed runtime contracts and tiered domain-instantiation maps. \Cref{thm:rac_coverage} explains why monotone inflation preserves coverage under the shared uncertainty construction, \Cref{thm:conditional_coverage} motivates the reliability-group calibration audits, and \Cref{thm:safety_margin} with \Cref{cor:zero_violation} explain why reliability-conditioned margins and repair can eliminate true-state failures on the reference battery surface. These results validate the construction and are instantiated across the replay rows, but they do not claim closed-loop stability, optimal inflation scheduling, or universal safety dominance outside the battery reference row and the proof-validated peer rows.

\subsection{Paper Roadmap}

Sections~\ref{sec:related}--\ref{sec:problem} position ORIUS and define the battery reference setting that motivated the framework. Sections~\ref{sec:forecasting_uq} and~\ref{sec:safe_dispatch} describe the forecasting, calibration, and shield mechanics. Sections~\ref{sec:experiments}--\ref{sec:us_analysis} preserve the core battery evidence, baseline diagnosis, and regime analysis. \Cref{sec:universal} then presents the tiered universal package: shared protocol, tier tables, training metrics, runtime and SIL results, and latency for the battery reference row plus the non-battery replay rows. The later sections extend that package through governance, hardening, benchmark formalization, and deployment discipline, and the closing sections restate the bounded claim boundary explicitly: battery reference, industrial and healthcare proof-validated, AV proof-candidate, navigation shadow-synthetic, aerospace experimental.
