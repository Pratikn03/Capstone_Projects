...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Tiered cross-domain instantiation.} We evaluate the same kernel across a six-row evidence package, with industrial and healthcare clearing the proof-validated gate, autonomous vehicles remaining proof-candidate, navigation remaining shadow-synthetic, and aerospace remaining experimental.
...
reference, industrial and healthcare proof-validated, AV proof-candidate,
navigation shadow-synthetic, aerospace experimental.
...
\midrule
\multicolumn{4}{p{0.9\linewidth}}{\textit{Universal replay: battery reference; industrial and healthcare proof-validated; AV proof-candidate; navigation shadow-synthetic; aerospace experimental.}} \\
\bottomrule
\end{tabular}
\end{table}
...
The universal claim is anchored by one canonical evidence generator, \texttt{scripts/run\_universal\_orius\_validation.py}, which emits the status, proof, training, runtime, SIL, comparison, and latency artifacts used in this section. The following two tables are the governing universal contract: the first fixes the claim tier for each row, and the second records why the tier holds.

\input{../reports/universal_orius_validation/tbl_domain_validation_summary.tex}

\input{../reports/universal_orius_validation/tbl_domain_proof_status.tex}
...
The shared runtime contract is exercised across all six rows: one fault protocol, one universal repair path, one promotion gate, and one artifact bundle. Battery remains the reference row; industrial and healthcare are proof-validated; AV remains proof-candidate; navigation remains shadow-synthetic; aerospace remains experimental.
...
\PaperFigureWide{FIG_MULTI_DOMAIN_VALIDATION}{Cross-domain validation surface from the canonical universal evidence-tier table. The labels encode the claim boundary: reference, proof-validated, proof-candidate, shadow-synthetic, and experimental.}

\input{../reports/universal_orius_validation/tbl_all_domain_latency.tex}

\subsection{Non-Defended Domains}

Navigation and aerospace remain in the canonical harness, but not in the proof-validated claim set. Navigation is still shadow-synthetic because the current trace is synthetic rather than locked telemetry. Aerospace remains experimental because the current replay surface executes correctly but does not yet satisfy the promotion contract. AV remains between those tiers: it is a proof-candidate row with locked telemetry, training, and SIL evidence, but it does not yet clear the full promotion gate. These rows remain valuable for adapter hardening and externalization planning, but they are intentionally outside the reference + proof-validated claim set used for the strongest universal wording in this manuscript.

\paragraph{Integrated ORIUS layers.}
The ORIUS program extends beyond the core DC\textsuperscript{3}S shield, but
the implementation should be read as one layered codebase rather than as five
separate mini-papers. \Cref{tab:orius_layers_modules} maps the main extension
layers to their implementation modules under \texttt{src/orius/}. The purpose
of the table is practical: it lets a reader move from the manuscript's claims
about temporal validity, graceful fallback, benchmark discipline,
...
DC\textsuperscript{3}S logic. Around that core, ORIUS adds five operational
layers: temporal certificate reasoning, graceful fallback, benchmark
...
some are primarily deployment-facing. Keeping them inside one table makes the
codebase easier to audit without forcing the manuscript itself to read like a
bundle of numbered sub-papers.
...
Paper 2 & Chs.~20, 28 & Certificate horizon, expiration bound, blackout safe-hold & theorem + bounded blackout artifacts & Temporal validity layer \\
Paper 3 & Chs.~20, 29 & Graceful fallback benchmark under prolonged blindness & canonical four-policy benchmark & Controlled degradation layer \\
Paper 4 & Chs.~10, 30 & ORIUS-Bench replay contract and benchmark discipline & replay + schema artifacts & Evaluation discipline layer \\
Paper 5 & Ch.~31 & Shared-feeder non-composition counterexample and coordinated repair & multi-agent scenario artifacts & Bounded composition layer \\
Paper 6 & Sec.~\ref{sec:governance}, Chs.~27, 32 & CertOS runtime lifecycle, audit chain, and recovery path & runtime + HIL artifacts & Runtime-governance layer \\
\bottomrule
\end{tabular}
\end{table*}
...
This paper is intentionally scoped as a tiered universal ORIUS manuscript, not as a claim of universal dominance. The strongest evidence set is limited to the battery reference row and the two proof-validated peers (industrial process control and medical monitoring). AV remains proof-candidate, navigation remains shadow-synthetic, and aerospace remains experimental. Battery still carries the locked economic narrative and the deepest theorem-to-artifact lineage.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item Battery, AV, industrial, and healthcare are defended in this release; navigation and aerospace are explicitly not.
