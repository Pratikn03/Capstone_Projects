...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Auditable implementation surface.} We tie the formal kernel to a typed runtime implementation so that theorem-facing objects, evidence tiers, and reported quantities remain inspectable without making implementation inventory the main scientific claim.
\end{enumerate}
...
Sections~\ref{sec:related}--\ref{sec:problem} position ORIUS and define the battery reference setting that motivated the framework. Sections~\ref{sec:forecasting_uq} and~\ref{sec:safe_dispatch} describe the forecasting, calibration, and shield mechanics. Sections~\ref{sec:experiments}--\ref{sec:us_analysis} carry the main battery evidence, baseline diagnosis, and regime analysis. \Cref{sec:universal} then asks the narrower transfer question: which parts of the same repair contract survive beyond the battery witness once replay, software-in-loop execution, and theorem obligations are all gated together? The later sections document the implementation surface, hardening, and governance needed to keep that evidence auditable, while the closing sections restate the bounded claim boundary explicitly: battery reference, industrial and healthcare proof-validated, AV proof-candidate, navigation shadow-synthetic, aerospace experimental.
...
\section{Governance and Reproducibility}\label{sec:governance}\label{ch:certos-runtime}
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item The formal results (\Cref{thm:rac_coverage,thm:conditional_coverage,thm:safety_margin}) formalize correctness of the ORIUS construction but do not establish closed-loop stability, optimal inflation, or universal performance guarantees.
