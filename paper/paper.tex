% =============================================================================
%  ORIUS: Universal Physical-Safety Framework with Defended-Domain
%        Validation Across Battery, AV, Industrial, and Healthcare
%  Canonical ORIUS Manuscript — IEEE-Style Two-Column Format
% =============================================================================
...
\title{ORIUS: Observation--Reality Integrity for Safe Control under Degraded Observation}
...
\graphicspath{{assets/figures/}{../paper/assets/figures/}{../reports/publication/}{../reports/hil/}{../reports/figures/}{../reports/universal_orius_validation/}}
...
\begin{abstract}
ORIUS studies a general safety failure in cyber-physical control: under degraded telemetry, a controller can be observationally safe while physically unsafe. This manuscript formalizes that observation--reality gap and introduces a typed Detect--Calibrate--Constrain--Shield--Certify kernel that constructs observation-consistent state sets, derives robust safe-action sets, repairs candidate actions, and emits causal safety certificates. The battery domain is the reference scientific witness: under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. The broader ORIUS claim is intentionally narrower than ``universal safety across domains.'' It is a reusable degraded-observation repair contract with one complete reference surface, explicit transfer obligations, and a tiered cross-domain evidence package in which industrial process control and medical monitoring clear the current proof-validated gate under bounded domain contracts, autonomous vehicles remains proof-candidate, navigation remains shadow-synthetic, and aerospace remains experimental. The implementation and artifact layer is kept auditable, but it is secondary to the paper's main point: once truth and observation separate, safety logic must adapt to measurement reliability.
\end{abstract}

\noindent\textbf{Keywords:} ORIUS, observation--reality integrity, safe control, degraded observation, battery dispatch, industrial control, medical monitoring, safety shield, telemetry reliability, cyber-physical systems
...
ORIUS treats that failure mode as a general cyber-physical systems problem rather than a battery-specific quirk. Whenever a controller acts on degraded observation of a physical state, an observation--action safety gap can emerge between what appears feasible on the observed channel and what is actually safe in the plant. The central contribution of this paper is therefore not another battery-only control heuristic, but a reusable repair-and-certify kernel for degraded-observation safety. Battery remains the reference surface on which the strongest proof and measurement claims are made. The non-battery rows matter, but they play a supporting role: industrial process control, medical monitoring, and autonomous vehicles are defended instantiations of the current contract, while navigation remains shadow-synthetic and aerospace remains experimental.
...
The central insight is that the measurement channel between the physical battery and the dispatch controller is not a passive conduit; it is part of the safety problem. Once telemetry degrades, the controller's internal picture of the world begins to drift away from physical reality. Standard robust optimization is designed to hedge uncertainty in \emph{future} load and generation scenarios, but it does not directly address uncertainty in the \emph{current} state estimate on which safety is checked. That gap motivates a different control stance: telemetry reliability must enter the safety logic itself.

DC\textsuperscript{3}S (Degradation-Conditioned Conformal Dispatch Safety Shield) is built around that failure mode. Rather than treating telemetry quality as a passive monitoring signal, the controller uses it to widen uncertainty sets, solve against the widened sets, repair unsafe candidate actions, and emit a step-level certificate within the same online control cycle. The guiding principle is \emph{conditional conservatism}: the shield should remain permissive when measurements are trustworthy and become more protective only when the observation channel weakens.
...
\begin{description}[leftmargin=1em,topsep=2pt,itemsep=1pt]
  ...
  \item[\textbf{RQ5}] Does the same ORIUS repair contract remain effective across defended non-battery domains when training readiness, runtime replay, software-in-loop execution, and theorem instantiation are all gated together?
\end{description}

\subsection{Contributions}

\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{Problem formalization.} We formulate degraded-observation safe control as a mismatch between observed feasibility and true physical safety, and make that gap the primary object of analysis rather than a side effect of forecast error.
  \item \textbf{Typed repair-and-certify kernel.} We present ORIUS as one typed Detect--Calibrate--Constrain--Shield--Certify kernel for safe control under degraded observation, with explicit runtime objects for observation-consistent state sets, repaired actions, and causal safety certificates.
  \item \textbf{Battery reference witness.} We establish the battery-domain hidden-gap result as the reference scientific witness: deterministic dispatch violates true SOC at 3.9\% of steps with P95 severity of 333\,MWh while appearing safe on observed state.
  \item \textbf{Program-first extension spine (Papers 2--6).} Beyond the battery witness, the dissertation contributes a second explicit ORIUS program block: Paper~2 develops temporal validity and blackout safe-hold semantics, Paper~3 closes the graceful-fallback benchmark surface, Paper~4 formalizes ORIUS-Bench as an evaluation contract, Paper~5 adds a bounded shared-constraint composition surface, and Paper~6 adds bounded runtime-governance and certificate-lifecycle evidence.
  \item \textbf{Tiered cross-domain instantiation.} We evaluate the same kernel across a six-row evidence package, with industrial and healthcare clearing the current proof-validated gate, autonomous vehicles retained as proof-candidate, navigation remaining shadow-synthetic, and aerospace remaining experimental.
  \item \textbf{Auditable implementation surface.} We tie the formal kernel and the program extensions to typed runtime implementations so that theorem-facing objects, extension layers, evidence tiers, and reported quantities remain inspectable without making implementation inventory the main scientific claim.
\end{enumerate}

\subsection{Contribution Boundary}

The central scientific contribution is a battery-anchored degraded-observation safety result together with a transferable kernel, not a claim of equal guarantees across six domains. ORIUS is the reusable degraded-observation kernel; battery is the formal reference row; industrial, healthcare, and the TTC-bounded autonomous-vehicle row are the strongest non-battery defended instantiations; navigation remains shadow-synthetic; and aerospace remains experimental. The bounded formal layer consists of the ORIUS theorem package together with typed runtime contracts and explicit transfer obligations. \Cref{thm:rac_coverage} explains why monotone inflation preserves coverage under the shared uncertainty construction, \Cref{thm:conditional_coverage} motivates the reliability-group calibration audits, and \Cref{thm:safety_margin} with \Cref{cor:zero_violation} explain why reliability-conditioned margins and repair can eliminate true-state failures on the reference battery surface. These results support a reusable degraded-observation contract; they do not imply closed-loop stability, optimal inflation scheduling, or equal proof closure for every instantiated domain.

For the dissertation as a whole, that boundary is implemented through two
explicit contribution spines. Paper~1 provides the battery proof witness and
the formal hidden-gap result. Papers~2--6 then extend ORIUS across temporal
validity, graceful fallback, benchmark discipline, bounded composition, and
runtime governance. Those extensions are thesis contributions in their own
right, but they do not all carry the same proof burden: Papers~2 and~3 depend
on the temporal battery ladder, Paper~4 is an evaluation-contract layer,
Paper~5 is a bounded composition extension, and Paper~6 is a bounded
runtime-governance extension.

\subsection{Paper Roadmap}

Sections~\ref{sec:related}--\ref{sec:problem} position ORIUS and define the
battery reference setting that motivated the framework. Sections
~\ref{sec:forecasting_uq} and~\ref{sec:safe_dispatch} describe the
forecasting, calibration, and shield mechanics. Sections~\ref{sec:experiments}
--\ref{sec:us_analysis} carry the main battery evidence, baseline diagnosis,
and regime analysis.

The canonical long-form thesis then proceeds in two blocks. The first block is
Paper~1: the battery theorem witness and its proof-facing chapters
(Chapters~15--20 in the imported long-form surface). The second block is the
explicit ORIUS program extension: Paper~2 contributes temporal validity and
blackout safe-hold, Paper~3 contributes graceful fallback, Paper~4 contributes
ORIUS-Bench evaluation discipline, Paper~5 contributes bounded
shared-constraint composition, and Paper~6 contributes bounded runtime
governance and certificate lifecycle. \Cref{sec:universal} situates those
extensions inside the narrower transfer question: which parts of the same
repair contract survive beyond the battery witness once replay,
software-in-loop execution, and theorem obligations are all gated together?
The closing sections restate the bounded claim boundary explicitly: battery
reference, industrial and healthcare proof-validated, AV proof-candidate, navigation
shadow-synthetic, aerospace experimental.
...
This paper makes a bounded governance claim: locked metrics, run-scoped provenance, and validator gates are used to keep the evidence auditable. That narrower claim is aligned with the reproducibility standards discussed by Pineau et al.~\cite{pineau2021improving}, but it is not presented as a new general governance framework or as the paper's primary novelty.
...
ORIUS is implemented as a forecast-to-dispatch stack whose main layers are data preparation, forecasting, uncertainty calibration, optimization, and monitoring. DC\textsuperscript{3}S sits at the dispatch boundary: it receives observed telemetry and forecast intervals, widens those intervals when telemetry quality or drift risk deteriorates, and prevents physically unsafe commands from reaching the battery. The system view matters here only insofar as it shows where the degraded-observation repair loop enters the control stack.
...
The implementation inventory in \Cref{tab:component_code_map} is included as a supporting reference for auditability. Readers focused on the scientific argument can treat it as companion material; the main paper claim remains the battery witness and the degraded-observation repair contract.

\begin{table*}[htbp]
\centering\small
\caption{Reference implementation map for the main manuscript surfaces.}
...
Universal kernel & Typed observation packets, state sets, repair decisions, and certificates & \path{src/orius/universal_theory/} \\
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item tracked release provenance: \path{reports/publication/release_manifest.json}
  \item tracked safety benchmark summary: \path{reports/publication/dc3s_main_table_ci.csv}
  \item tracked latency budget summary: \path{reports/publication/dc3s_latency_summary.csv}
...
\end{itemize}
Any manuscript claim that depends on these interfaces is treated as invalid unless it can be traced back to one of them. This lock exists to keep the evidence stable; it is not the paper's main research contribution.
...
\Cref{tab:dataset_framework_trace} maps each locked dataset to its role in the pipeline and the primary evidence tables. DE and US\_MISO feed the canonical battery evaluation (TBL01, TBL04); PJM and ERCOT extend the release-family scope. The tiered universal runtime package (\Cref{sec:universal}) uses locked telemetry for AV, industrial, healthcare, and aerospace rows, plus a synthetic navigation shadow surface; the governed cross-domain evidence is summarized by the universal status and proof tables.
...
\midrule
\multicolumn{4}{p{0.9\linewidth}}{\textit{Universal replay: battery reference; industrial and healthcare proof-validated under the bounded closure matrix; AV proof-candidate; navigation shadow-synthetic; aerospace experimental.}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Split and profile lock}: Temporal train/test splits with the 24-hour gap. Profile lock metadata is written to \path{reports/publication/release_manifest.json} to freeze the evidence base.
...
\section{The Observed-State Safety Illusion}\label{sec:safety_illusion}\label{ch:safety-illusion}
...
\section{Forecasting and Uncertainty Method}\label{sec:forecasting_uq}\label{sec:theory}
...
The paper-local theorems below keep the battery reference assumptions explicit.  In code, the broader reusable contract is now materialized in \path{src/orius/universal_theory/} as typed runtime objects (\texttt{ObservationPacket}, \texttt{ObservationConsistentStateSet}, \texttt{SafeActionSet}, \texttt{RepairDecision}, and \texttt{SafetyCertificate}) with per-step assumption tags.  That code surface is the universal kernel; the theorems below remain the battery reference instantiation.
...
\Cref{thm:rac_coverage} is a correctness condition for the RAC-Cert construction. It confirms that monotone inflation cannot erode the base conformal guarantee. The deeper quantitative guarantees --- finite-sample coverage under degraded observations (\Cref{thm:finite-sample}), necessity of reliability awareness (\Cref{thm:separation}), and adaptive inflation regret (\Cref{thm:regret}) --- are stated in \Cref{sec:deep_guarantees_paper}. The primary contribution remains systems-oriented: coupling reliability-adaptive inflation with dispatch-level action repair to close the hidden safety gap.
...
\subsection{Deep Theoretical Guarantees}\label{sec:deep_guarantees_paper}

The correctness conditions above (T1--T8) establish that the DC\textsuperscript{3}S construction is sound. The following three results go further, from correctness to \emph{quantitative guarantees}.

\begin{theorem}[Finite-sample coverage under reliability degradation]\label{thm:finite-sample}
Let $\{(X_i, Y_i)\}_{i=1}^n$ be calibration data with reliability weights $w_i \in [w_{\min}, 1]$. Let $\hat{C}_t^w$ be the DC\textsuperscript{3}S conformal set at level $1 - \alpha$. For any $\delta > 0$:
\[
  \Pr\!\bigl(Y_{n+1} \in \hat{C}_{n+1}^w\bigr) \ge 1 - \alpha - \sqrt{\frac{\ln(2/\delta)}{2\,n_{\mathrm{eff}}}}
\]
where $n_{\mathrm{eff}} = \lfloor n \cdot w_{\min} \rfloor$, with probability $\ge 1 - \delta$.
\end{theorem}
\begin{proof}
By weighted exchangeability, the reliability-weighted conformal quantile achieves exact $1-\alpha$ coverage under correct specification. Under degradation, effective sample size is $n_{\mathrm{eff}} = n \cdot w_{\min}$. Hoeffding on the weighted coverage indicator gives $\epsilon = \sqrt{\ln(2/\delta)/(2\,n_{\mathrm{eff}})}$.
\end{proof}

\Cref{tab:wmin_sweep} shows how the bound tightens as $w_{\min}$ increases for the locked DE calibration ($n=2{,}608$). At $w_{\min}=0.50$ the coverage guarantee is $\ge 0.862$; at $w_{\min}=1.0$ (perfect telemetry) it reaches $0.873$. The executable witness is \texttt{dc3s/theoretical\_guarantees.py: compute\_finite\_sample\_coverage\_bound()}.

\input{../reports/publication/tbl_wmin_sweep.tex}

\begin{theorem}[Separation --- necessity of reliability awareness]\label{thm:separation}
For any controller $\pi_{\mathrm{blind}}$ using a fixed uncertainty set $\mathcal{U}$ (not conditioned on $w_t$), there exist degradation sequences such that either:
\textup{(a)} $\mathrm{TSVR}(\pi_{\mathrm{blind}}) \ge \alpha(1-w_{\min})/2$, or
\textup{(b)} $\mathrm{IR}(\pi_{\mathrm{blind}}) \ge \mathrm{IR}(\mathrm{DC}^3\mathrm{S}) + (1-w_{\min})/2$.
\end{theorem}
\begin{proof}
By construction: alternate $w_t=1$ and $w_t=w_{\min}$. A narrow fixed set under-covers degraded steps (case~a); a wide fixed set over-intervenes during clean steps (case~b). DC\textsuperscript{3}S avoids both by conditioning on $w_t$.
\end{proof}

\noindent\textit{Operational meaning.}\enspace Reliability conditioning is \emph{necessary}: any controller that ignores observation quality is Pareto-dominated. For $w_{\min}=0.05$, the lower bound on the gap is $0.475$ in either violations or interventions.

\begin{theorem}[Adaptive inflation regret]\label{thm:regret}
Let $I_t^*$ be the oracle-optimal inflation and $I_t$ the DC\textsuperscript{3}S inflation under smoothing constant $\tau$, $\gamma = 1 - e^{-1/\tau}$. Then:
\[
  R_T = \sum_{t=1}^T (I_t - I_t^*)^2 \le \tau \cdot \Delta_{\max}^2 (1 + \ln T) + \Delta_{\max}^2/\gamma^2
\]
where $\Delta_{\max} = \max_t |I_t^* - I_{t-1}^*|$. Per-step regret: $R_T/T = O(\tau \ln T / T) \to 0$.
\end{theorem}
\begin{proof}
Exponential smoothing with rate $\gamma$ has tracking error bounded by $(1-\gamma)^k \Delta_{\max}$ after $k$ steps from a jump. Squaring and summing with geometric decay gives the $\tau \ln T$ tracking term.
\end{proof}

\noindent\textit{Operational meaning.}\enspace DC\textsuperscript{3}S with $\tau=30$ (locked value) tracks the oracle with vanishing per-step error. For $T=500$, $\Delta_{\max}=0.3$: $R_T/T \le 0.082$.

\paragraph{Theorem summary (T9--T11).}
T9 provides a computable finite-sample coverage guarantee; T10 proves reliability conditioning is necessary (lower bound); T11 proves the adaptive mechanism tracks the oracle. Together they elevate the contribution from correctness conditions to quantitative guarantees. Code witnesses: \texttt{dc3s/theoretical\_guarantees.py}; test suite: \texttt{tests/test\_theoretical\_guarantees.py} (28 tests).
...
\section{Safe Dispatch and DC3S Shield}\label{sec:safe_dispatch}\label{ch:dc3s}\label{sec:shield}
...
\section{Results}\label{sec:results}\label{ch:main-results}
...
To keep that claim tied to the exact promoted battery release, we also report the replay-backed controller-family comparison that is already locked in the artifact set. Table~\ref{tab:battery_reliability_controller_compare} re-aggregates the five promoted battery controllers present in \path{reports/publication/dc3s_main_table.csv}: deterministic LP, robust fixed interval, adaptive conformal with repair, and the two \DCThreeS{} variants. This is a narrower surface than the full conformal-method landscape, but it is the honest one for the current release: no weighted or Mondrian dispatch rows are synthesized because they are not present as promoted replay-backed controllers in the lock. Read this table as an operational comparison, not as a second hidden-gap theorem witness.

\input{assets/tables/generated/tbl_battery_reliability_baselines.tex}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{assets/figures/fig59_battery_reliability_baselines.png}
  \caption{Locked battery controller-family comparison on the promoted replay-backed artifact surface. The static non-repair baselines carry the true-state violation burden; the repair-enabled rows eliminate it at different width and intervention costs.}
  \label{fig:battery_reliability_controller_compare}
\end{figure}

The locked controller-family surface sharpens the empirical story in two ways. First, the unsafe rows are precisely the non-adaptive or non-repair rows: deterministic LP and robust fixed interval carry the true-state violation burden, while the repair-enabled rows drive it to zero. Second, zero violations alone are not the entire story. Adaptive conformal with repair, \DCThreeS{} wrapped, and \DCThreeS{} FTIT occupy different points on the width--intervention tradeoff, which is exactly why the paper's claim is about a reliability-conditioned repair-and-certify kernel rather than about one scalar interval-width metric.
...
\subsection{Ablation Study: What Matters for Safety}\label{sec:ablation}\label{ch:ablations}
...
\section{Cross-Region and US Regime Analysis}\label{sec:us_analysis}\label{ch:battery-lessons}
...
\section{Universal Domain Framework}\label{sec:universal}\label{ch:battery-to-orius}

This manuscript now treats ORIUS as a tiered universal framework rather than a battery-only system with portability notes. Battery remains the reference witness for the hidden-gap discovery, the locked economics, and the theorem-to-artifact lineage; industrial process control and medical monitoring are proof-validated peers under their current bounded contracts; autonomous vehicles remains proof-candidate; navigation remains shadow-synthetic; and aerospace remains experimental. The cross-domain claim boundary is governed by the locked tier and closure tables, not by raw TSVR alone.

\subsection{Multi-Domain Architecture}

\Cref{fig:FIG_UNIVERSAL_FRAMEWORK} illustrates the universal pipeline. Each domain provides a \emph{DomainAdapter} that implements: (1) telemetry ingestion, (2) reliability scoring (OQE), (3) uncertainty set construction, (4) action set tightening, (5) action repair (shield), and (6) certificate emission. The pipeline stages are identical across domains; only the state schema, safety predicates, and action spaces differ.

\PaperFigureWide{FIG_UNIVERSAL_FRAMEWORK}{ORIUS universal domain physical safety framework. The five-stage pipeline (Detect$\to$Calibrate$\to$Constrain$\to$Shield$\to$Certify) applies uniformly; domain adapters for six runtime domains (Energy, Vehicle/AV, Navigation, Industrial, Healthcare/Surgical, Aerospace) provide domain-specific state, safety predicates, and repair logic.}

\subsection{Tiered Evidence Surface}

The universal claim is anchored by one canonical evidence generator, \texttt{scripts/run\_universal\_orius\_validation.py}, together with the closure-matrix builder \texttt{scripts/build\_domain\_closure\_matrix.py}. The replay generator emits the raw cross-domain validation surface, while the closure builder fixes the final promotion tier once replay, soundness, fallback, and bounded Paper~5/Paper~6 portability checks are all available. The closure matrix is therefore the manuscript's promotion source of truth.

\input{../reports/universal_orius_validation/tbl_domain_closure_matrix.tex}

\subsection{Training and Calibration Surfaces}

The forecasting-capable non-battery replay rows now expose battery-style forecasting and calibration artifacts rather than a single readiness bit. AV, industrial, healthcare, and aerospace all have non-empty train/calibration/test splits, locked feature paths, model bundles, conformal artifacts, and publication-facing reports. Battery forecasting remains the locked reference surface in Sections~\ref{sec:forecasting_uq} and~\ref{sec:calibration}; the tables below summarize the non-battery forecasting surfaces and the locked training audit.

\input{../reports/universal_orius_validation/tbl_defended_domain_training_metrics.tex}

\input{../reports/universal_training_audit/tbl_domain_training_audit.tex}

\subsection{Runtime and Software-in-Loop Validation}\label{sec:universal_validation}

The shared runtime contract is exercised across all six rows: one fault protocol, one universal repair path, one promotion gate, and one artifact bundle. Battery remains the reference row; industrial and healthcare are proof-validated under the current bounded contracts; AV remains proof-candidate; navigation remains shadow-synthetic; and aerospace remains experimental.

\input{../reports/universal_sil_validation/tbl_domain_sil_summary.tex}

\input{../reports/universal_orius_validation/tbl_all_domain_comparison.tex}

\PaperFigureWide{FIG_MULTI_DOMAIN_VALIDATION}{Cross-domain validation surface from the canonical universal evidence-tier table. The labels encode the current claim boundary: reference, proof-validated, proof-candidate, shadow-synthetic, and experimental, with final promotion governed by the closure matrix.}

\input{../reports/universal_orius_validation/tbl_all_domain_latency.tex}

\subsection{Non-Defended Domains}

Navigation and aerospace remain in the canonical harness, but not in the proof-validated claim set. Navigation is still shadow-synthetic because the current trace is synthetic rather than locked telemetry. Aerospace remains experimental because the current replay surface executes correctly but does not yet satisfy the promotion contract. AV also remains outside the proof-validated set in the current locked build: the TTC~+ predictive-entry-barrier contract shows substantial progress on bounded replay, soundness, fallback, and runtime-governance checks, but it is still narrower than the defended promotion contract used for industrial and healthcare. That AV status is intentionally conservative. It does not imply that richer multi-lane or higher-dimensional vehicle settings already enjoy equal proof closure.

\paragraph{Integrated ORIUS layers.}
The ORIUS program extends beyond the core DC\textsuperscript{3}S shield, and
the dissertation now treats those extensions as one explicit Paper~1--6
research program. \Cref{tab:orius_layers_modules} maps the program layers to
their implementation modules under \texttt{src/orius/}. The purpose of the
table is practical: it lets a reader move from the manuscript's program-level
claims about temporal validity, graceful fallback, benchmark discipline,
shared-constraint coordination, and runtime governance directly into the
corresponding code paths.

\begin{table}[htbp]
\centering\small
\caption{Integrated ORIUS extension layers and implementation paths.}
\label{tab:orius_layers_modules}
\begin{tabular}{p{2.2cm}p{3.2cm}p{6.8cm}}
\toprule
Layer & Role in the integrated system & Primary paths \\
\midrule
Temporal validity & Certificate half-life, reachability, and bounded blackout semantics & \path{src/orius/dc3s/half_life.py}, \path{src/orius/dc3s/reachability.py} \\
Controlled degradation & Graceful fallback planner and safe landing logic under prolonged blindness & \path{src/orius/dc3s/graceful.py}, \path{src/orius/optimizer/fallback_dispatch.py} \\
Evaluation discipline & ORIUS-Bench replay harness, metric engine, and governed benchmark tracks & \path{src/orius/orius_bench/} (adapter, fault\_engine, metrics\_engine, battery\_track, navigation\_track) \\
Shared-constraint coordination & Fleet-level composition layer for batteries sharing transformer or feeder limits & \path{src/orius/multi_agent/} (plant, protocol, margin\_allocation, scenarios) \\
Runtime governance & CertOS runtime, certificate chain, audit ledger, and recovery path & \path{src/orius/certos/} (runtime, certificate\_engine, audit\_ledger, recovery\_manager, belief\_engine, graceful\_planner) \\
\bottomrule
\end{tabular}
\end{table}

Read in that integrated way, the repository has a simple structure. The
reference scientific witness stays in the battery path and the theorem-facing
DC\textsuperscript{3}S logic. Around that core, ORIUS adds five explicit
program layers: temporal certificate reasoning, graceful fallback, benchmark
formalization, shared-constraint coordination, and runtime governance. Some of
those layers are theorem-backed, some are bounded empirical extensions, and
some are primarily deployment-facing. The dissertation therefore reads as both
one layered codebase and one explicit six-paper research program.

\PaperFigureWide{FIG46_ORIUS_PROGRAM}{Program-first ORIUS dissertation spine. Paper~1 provides the battery proof witness; Papers~2--6 extend ORIUS across temporal validity, graceful fallback, evaluation discipline, shared-constraint coordination, and runtime governance under explicitly bounded claim tiers.}

\paragraph{Program contribution map.}
The canonical thesis is therefore not a battery-only document with optional
late extensions. It is a two-spine dissertation: Paper~1 establishes the
reference scientific witness, and Papers~2--6 make the temporal, fallback,
benchmark, compositional, and runtime layers explicit as defended thesis
contributions.

\begin{table*}[htbp]
\centering\small
\caption{Paper-to-thesis contribution map for the canonical ORIUS dissertation.}
\label{tab:orius_program_map}
\begin{tabular}{p{1.1cm}p{2.6cm}p{3.8cm}p{3.0cm}p{4.0cm}}
\toprule
Paper & Main thesis chapters & Central claim & Evidence status & Role in the thesis \\
\midrule
Paper 1 & Chs.~11, 15--20 & Battery OASG witness and T1--T8 ladder & locked theorem gate + battery artifacts & Reference scientific witness \\
Paper 2 & Chs.~20, 28 & Certificate horizon, expiration bound, blackout safe-hold & bounded theorem extension + blackout artifacts & Temporal validity layer \\
Paper 3 & Chs.~20, 29 & Graceful fallback benchmark under prolonged blindness & bounded empirical benchmark & Controlled degradation layer \\
Paper 4 & Chs.~10, 30 & ORIUS-Bench replay contract and benchmark discipline & replay contract + schema artifacts & Evaluation discipline layer \\
Paper 5 & Ch.~31 & Shared-feeder non-composition counterexample and coordinated repair & bounded composition counterexample + scenario artifacts & Bounded composition layer \\
Paper 6 & Sec.~\ref{sec:governance}, Chs.~27, 32 & CertOS runtime lifecycle, audit chain, and recovery path & bounded lifecycle invariants + runtime artifacts & Runtime-governance layer \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[htbp]
\centering\small
\caption{Papers~2--6 inclusion audit in the canonical thesis narrative.}
\label{tab:papers26_inclusion_audit}
\begin{tabular}{lcccl}
\toprule
Paper & Code & Artifacts & Main text & Classification \\
\midrule
Paper 2 & yes & yes & yes & fully integrated \\
Paper 3 & yes & yes & yes & fully integrated \\
Paper 4 & yes & yes & yes & fully integrated \\
Paper 5 & yes & yes & yes & fully integrated \\
Paper 6 & yes & yes & yes & fully integrated \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{What Papers 2--6 add to the thesis.}
Paper~2 turns certificate validity into an explicit temporal object with a
bounded expiration and blackout-safe-hold story. Paper~3 adds the canonical
four-policy graceful-fallback benchmark and constrains its claim surface to
that executable battery benchmark. Paper~4 makes ORIUS-Bench a replayable
evaluation contract rather than a loose benchmarking appendix. Paper~5 adds a
bounded shared-feeder counterexample showing that local certificates do not
compose automatically, while Paper~6 adds the runtime-governance layer that
tracks validation, expiry, fallback, audit continuity, and recovery as
explicit lifecycle events.

\paragraph{Cross-domain bounded evaluation for Papers 5 and 6.}
Paper~5 and Paper~6 are not promoted into new universal theorem families in
this manuscript.  They are strengthened through bounded cross-domain
evaluation only where the adapters currently support the required surfaces.
Paper~5 is evaluated on the battery shared-feeder counterexample and on an
industrial shared-budget stress row; the remaining domains stay gated until
they gain an explicit shared-resource scenario.  Paper~6 is evaluated on the
battery, industrial, healthcare, and vehicle lifecycle surfaces; navigation
and aerospace remain gated pending row closure.

\paragraph{How to read the inclusion audit.}
``Fully integrated'' here means that each paper is present in code, artifacts,
and the thesis main narrative. It does \emph{not} mean that each paper carries
the same theorem burden as Paper~1. The burden remains stratified: Paper~1 is
the full proof witness; Papers~2, 3, 5, and~6 are bounded defended layers;
Paper~4 is the benchmark-contract layer.
...
This paper is intentionally scoped as a tiered universal ORIUS manuscript, not as a claim of universal dominance. The strongest evidence set is limited to the battery reference row and the currently proof-validated peers (industrial process control and medical monitoring). Autonomous vehicles remains proof-candidate, navigation remains shadow-synthetic, and aerospace remains experimental. Battery still carries the locked economic narrative and the deepest theorem-to-artifact lineage.
\subsection{Threats to Construct and Conclusion Validity}

Construct validity depends on the proxy cost and carbon signals being appropriate enough for comparative analysis. Conclusion validity depends on scenario design and calibration definitions remaining stable. The original formal results (\Cref{thm:rac_coverage,thm:conditional_coverage,thm:safety_margin}) formalize correctness conditions for the DC\textsuperscript{3}S construction: coverage preservation, group-conditional coverage, and safety-margin monotonicity. The deep guarantees (\Cref{thm:finite-sample,thm:separation,thm:regret}) go further: T9 gives a finite-sample coverage bound, T10 proves necessity of reliability conditioning, and T11 bounds adaptive inflation regret. Together they do not establish closed-loop stability or optimal inflation in the LQR/Lyapunov sense, but they do provide quantitative, computable safety guarantees that go beyond pure correctness conditions.

The theorems should be read as a tiered guarantee structure: T1--T8 validate the \emph{construction}; T9--T11 provide \emph{quantitative bounds} on coverage, separation, and adaptation. They do not prove that the chosen inflation law is optimal among all monotone laws, do not guarantee minimum intervention rate, and do not remove the need for empirical validation. The practical safety of the system depends on how reliability scoring, drift behavior, forecast quality, optimization, and action repair interact under realistic faults. The empirical evidence in \Cref{sec:main_result} and the ablation study in \Cref{sec:ablation} provide the primary safety validation.

\section{Limitations and Future Work}\label{sec:limitations}\label{sec:future_work}

\subsection{Current Limitations}

\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item Battery, industrial, and healthcare are defended in this release; AV remains proof-candidate, and navigation and aerospace are explicitly not.
  \item The manuscript now includes controller-in-the-loop runtime rehearsal and software-in-loop validation, but not production field deployment. Physical battery control, inverter integration, BMS-level constraint enforcement, vehicle actuation, and clinical-device integration remain outside the current evidence lock.
  \item Cost and carbon signals are engineering proxies rather than full settlement models, so the strongest economic interpretation remains the battery domain.
  \item The correctness theorems (T1--T8) validate the DC\textsuperscript{3}S construction; the deep guarantees (T9--T11) provide finite-sample coverage bounds, a separation lower bound, and an adaptive regret bound. Together they establish quantitative safety guarantees but do not prove closed-loop stability, optimal inflation selection, or universal performance dominance.
  \item The non-battery promoted domains clear the current promotion gate, but some of them do so with high repair rates. The defended claim is therefore safety preservation under degraded observation, not uniform conditional-conservatism behavior across every domain.
\end{enumerate}
...
\paragraph{The core message.} Trustworthy cyber-physical control requires the uncertainty model to degrade when the measurement channel degrades. In that sense, telemetry is not an implementation detail but part of the control problem itself. ORIUS turns that idea into a single Detect--Calibrate--Constrain--Shield--Certify loop with per-step auditable evidence, and this release now supports that loop with a tiered universal evidence package rather than with battery alone. Battery remains the reference witness for the hidden-gap discovery and the locked economic story; industrial and healthcare are proof-validated peers under their bounded contracts; autonomous vehicles remains proof-candidate; navigation remains shadow-synthetic; aerospace remains experimental.

\paragraph{Scope boundary.} What this paper establishes is a tiered universal ORIUS package anchored by a battery reference row and two proof-validated non-battery peers under their current bounded contracts. It does not establish universal dominance across all domains, adversarial completeness, or production-hardware deployment safety. The strongest claim supported by the artifact bundle is that under degraded observation, the same ORIUS repair contract can expose and materially reduce true-state safety failures on the reference and proof-validated surfaces, while additional rows remain evidence-tiered as proof-candidate, shadow-synthetic, or experimental.
...
Artifact & Purpose \\
\midrule
\path{paper/paper.tex} & Canonical manuscript source \\
\path{paper/PAPER_DRAFT.md} & Narrative companion synced from the canonical manuscript \\
\path{paper/paper_r1.tex} & Shorter conference derivative \\
\path{paper/metrics_manifest.json} & Canonical universal safety lock and display contract \\
...
\path{reports/publication/release_manifest.json} & Canonical tracked release provenance \\
\path{reports/publication/dc3s_main_table_ci.csv} & Canonical tracked safety benchmark summary \\
\path{reports/publication/dc3s_latency_summary.csv} & Canonical tracked latency budget summary \\
\path{reports/impact_summary.csv} & Supporting DE impact artifacts \\
\path{reports/eia930/impact_summary.csv} & Supporting US impact artifacts \\
