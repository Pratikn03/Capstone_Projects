...
This corollary provides the theoretical link to the empirical zero-violation result: the locked CPSBench fault scenarios generate telemetry errors within the range that the FTIT SOC tube absorbs. The empirical evidence in \Cref{sec:main_result} confirms that this condition holds across all tested fault families and severity levels.
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{\Cref{thm:safety_margin} + \Cref{cor:zero_violation} $\leftrightarrow$ \Cref{tab:TBL01_MAIN_RESULTS}}: the monotone margin construction explains why both DC\textsuperscript{3}S variants achieve zero violations while deterministic and CVaR baselines fail. The 2.8\% intervention rate quantifies the cost of the margin.
\end{itemize}
