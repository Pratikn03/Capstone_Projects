...
This proposition is still intentionally local: it is a one-step invariance statement, not a full closed-loop stability proof. Its role is to formalize the safety-filter interpretation already latent in the FTIT tube and projection logic. The corresponding helper code lives alongside the existing guarantee checks and remains backward-compatible with the locked controller path.
...
Before evaluating shield behavior, we report the promoted six-model forecasting comparison that the optimizer consumes. \Cref{tab:TBL08_FORECAST_BASELINES} is now the main forecasting result table for the thesis: it spans DE load/wind/solar and US load/wind/solar under one shared experimental contract and reports RMSE, MAE, sMAPE, $R^2$, PICP@90, and interval width when model-specific uncertainty artifacts are available. The comparison is intentionally operational: all six models share the same split policy, forecast horizon, lookback, seed policy, feature framing, and evaluation rules so that performance differences reflect model behavior under a common forecasting regime rather than different data access or leakage exposure.

\PaperTableCSV{TBL08_FORECAST_BASELINES}{Promoted six-model forecasting comparison across DE and US under a shared split, horizon, lookback, seed policy, and metric contract. Dashes denote target--model cells whose candidate artifacts are not yet populated in the current release-family build.}

\textbf{Interpretation.} The promoted comparison surface preserves one common baseline protocol across all six model families. In the currently materialized rows, GBM remains the operational winner and is the only model with consistently strong point performance plus populated uncertainty outputs. LSTM and TCN remain substantially weaker on wind and solar, and the newly promoted N-BEATS, TFT, and PatchTST rows are now part of the main reporting surface rather than future-only baselines. Where the table still shows dashes, the interpretation is not ``good performance'' or ``bad performance'' but simply ``artifact not yet finalized in the current release-family build.'' This is the right place to be explicit about incompleteness instead of silently reverting to the older three-model comparison.
...
\PaperFigure{FIG06_TRANSFER_COVERAGE}{Transfer stress coverage summary. \DCThreeS{} maintains safety across transfer cases, though cost optimality degrades without retraining.}
...
In the currently materialized rows of the promoted six-model comparison, GBM dominates the available deep baselines on load, wind, and solar forecasting in both DE and US regimes. LSTM and TCN provide the clearest completed comparison today, while N-BEATS, TFT, and PatchTST are already part of the same reporting contract but still have pending artifact rows in the current release-family build. This is a finding about the data regime, not a tuning failure, and it has a direct practical implication.
...
All six baselines---GBM, LSTM, TCN, N-BEATS, TFT, and PatchTST---are now part of the locked publication comparison contract under a comparable training budget: deep models use early stopping as the budget control (up to 300 epochs with patience 30), while GBM uses Optuna with 100 trials. This is not a perfectly equalized budget, but it is a principled one that reflects the standard practice for each model family. The honest interpretation is that GBM is the operational winner in the currently materialized locked artifacts, and that this advantage is a property of the data regime (moderate size, rich tabular features) rather than a tuning artifact.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item Evaluation is software-in-the-loop rather than hardware field validation; CHIL, HIL, and field pilots remain explicit future work and are not claimed in the present evidence base.
  ...
  \item The forecasting comparison is materially stronger than the older three-model draft because the main paper surface now uses a six-model contract, but the architecture ranking is still bounded to the present data regime and current artifact state. Larger datasets, weaker tabular preprocessing, or longer-context forecasting could shift the ranking.
\end{enumerate}

\subsection{Near-Term Future Work}

Near-term work should focus on three items: decomposing the US narrative more explicitly into MISO/PJM/ERCOT in the main text, improving under-covered target-regime calibration before dispatch, and adding a clean deployment-facing latency/runtime benchmark plus a publication-grade case trace that shows telemetry degradation, interval widening, action repair, and safe dispatch behavior over time. These are now higher-value next steps than additional baseline reruns because the six-model forecasting surface is already the main paper-facing comparison.

\subsection{Longer-Term Future Work}

Longer-term work includes hardware-in-the-loop validation with physical inverter and BMS integration, richer market constraints that capture settlement rules more faithfully, conditional-coverage analysis under non-stationary degradation, and stronger release automation with cryptographically signed evidence bundles. A further research direction is extending the RAC-Cert framework to multi-agent dispatch settings where multiple batteries share a common telemetry infrastructure and must coordinate safety constraints. Those hardware-facing steps are intentionally deferred beyond the current thesis evidence lock.
