...
The deployment-evidence pipeline promotes the reliability-group coverage analysis into a governed, release-scoped artifact (\texttt{reliability\_group\_coverage.csv/json}, release \texttt{R1\_DEPLOY\_20260314}). The audit partitions 2{,}000~calibration samples into 10~quantile bins by reliability score and reports per-bin PICP. In the current release, overall PICP matches the nominal 90\% target, but the worst-bin coverage reveals the reliability-dependent pattern described above. This governed audit is a diagnostic surface in the current release, not a replacement for the locked production CQR+RAC-Cert calibration path. Its purpose is to make the under-coverage pattern precise and traceable so that future Mondrian-based calibration improvements can be evaluated against a stable baseline.
