...
\begin{abstract}
ORIUS is a universal physical-safety framework for cyber-physical control under degraded observation. This paper presents its battery-domain reference implementation, DC3S, and uses that surface to quantify the hidden gap between observed-state feasibility and true-state safety. Under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. ORIUS organizes the response as a five-stage loop---Detect, Calibrate, Constrain, Shield, Certify---that is shared across Energy, AV, Industrial, Healthcare/Surgical, and Aerospace adapters, while battery dispatch remains the primary locked empirical and theorem-validation surface. On Germany (OPSD, 17{,}377 rows), the battery-domain DC3S implementation delivers \PaperMetric{DE_COST_SAVINGS_PCT}{7.11}\% cost savings, \PaperMetric{DE_CARBON_REDUCTION_PCT}{0.30}\% carbon reduction, and \PaperMetric{DE_PEAK_SHAVING_PCT}{6.13}\% peak shaving; stochastic value remains positive in both DE and US regions (VSS\,=\,\PaperMetric{DE_VSS}{2,708.61} and \PaperMetric{US_VSS}{297,092.71}) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). The current release also includes a canonical universal replay harness: industrial control and medical monitoring are proof-validated non-battery rows, AV remains a proof candidate, navigation is synthetic-shadow evidence, and aerospace remains experimental. Every quantitative claim is tied to versioned run artifacts and checked automatically against the release evidence.
\end{abstract}
...
The battery DC3S architecture is the primary validated instantiation of ORIUS, a \textbf{universal domain physical-safety framework} that applies the same five-stage pipeline---Detect, Calibrate, Constrain, Shield, Certify---across six runtime domains. This section documents the shared framework, domain adapters, and current evidence boundary for the cross-domain layer. Historical incidents---Air France 447 (pitot dropout), 737 MAX MCAS (sensor spike/drift), Tesla Autopilot (perception dropout), Texas City refinery (level sensor failure), and Uber ATG (sensor fusion failure)---illustrate the OASG across aviation, automotive, and industrial control; ORIUS addresses the common observation-reality integrity gap underlying these events.
...
The Aerospace adapter targets avionics and flight control. State fields include airspeed, angle-of-attack, altitude, and sensor health (pitot heat status, AoA cross-check consistency, GPS satellite count). Safety predicates enforce flight-envelope limits. When sensor health degrades (e.g., pitot icing, GPS jamming), the uncertainty set expands and the safe action set contracts toward conservative commands. The adapter instantiates the OASG pattern observed in Air France 447 and 737 MAX MCAS.

\subsection{Universal ORIUS Validation Evidence}\label{sec:universal_validation}

The unified validation harness (\texttt{scripts/run\_universal\_orius\_validation.py}) is now the canonical cross-domain evidence generator. It combines the locked battery reference aggregate with replay-backed AV, industrial, healthcare, navigation, and aerospace rows under one shared fault protocol and one shared universal repair path. \Cref{tab:TBL09_CROSS_DOMAIN_OASG} reports the resulting cross-domain table from the locked run, while the companion validation report records that all six configured rows completed without harness failure. The defended claim boundary is tiered rather than uniform: industrial and healthcare satisfy the non-battery promotion gate, AV remains a proof candidate because the current locked traces do not show post-shield TSVR improvement, navigation remains synthetic-shadow evidence, and aerospace remains experimental. Accordingly, the cross-domain surface supports bounded promotion of specific non-battery rows plus disciplined scope control, not universal dominance.

\PaperTableCSV{TBL09_CROSS_DOMAIN_OASG}{Cross-domain OASG validation. Six ORIUS-Bench tracks with fault episodes; baseline vs.\ ORIUS TSVR and reduction.}

\PaperFigure{FIG_MULTI_DOMAIN_VALIDATION}{Multi-domain ORIUS validation. Baseline (nominal) vs.\ ORIUS (DC3S) true-state violation rate across six domains under fault episodes.}

\subsection{Implementation and Scope}

The universal framework is implemented in \texttt{src/orius/universal\_framework/} with domain registry, pipeline (\texttt{run\_universal\_step}), and six runtime domains: Energy (Battery), Vehicle/AV, Navigation, Industrial, Healthcare/Surgical, and Aerospace. The battery domain remains the primary locked empirical and theorem-validation surface. The additional adapters and the canonical replay harness demonstrate that the same ORIUS pattern can be instantiated beyond energy and evaluated under one shared contract, but the resulting evidence is explicitly tiered: battery reference, industrial/healthcare proof-validated, AV proof-candidate, navigation shadow-synthetic, aerospace experimental.
...
This paper is intentionally scoped as an ORIUS flagship manuscript with a battery-primary evidence boundary. The locked DE and US windows support the deepest quantitative claims. The cross-domain ORIUS sections now document a canonical replay harness with tiered non-battery evidence, but those rows are still not equivalent to the battery-domain theorem and artifact depth. The multi-BA US evidence surface improves external validity, but it does not convert this paper into a universal market study or a universal safety guarantee.
