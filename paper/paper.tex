...
\title{DC3S: Telemetry-Reliability-Weighted Conformal Safety Shield for Battery Dispatch under Degraded Mixed Telemetry, with a Universal Multi-Domain Framework}
...
\begin{abstract}
Battery dispatch controllers can look feasible on observed sensor state while simultaneously violating physical battery limits on true state when telemetry degrades---a hidden safety failure that standard evaluation protocols do not expose. We study this gap under realistic battery fault conditions and find that deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. The manuscript responds to that failure mode in three layers. First, \textbf{CPSBench} maintains separate truth and observed battery trajectories so that true-state safety becomes measurable rather than implicit. Second, \textbf{DC\textsuperscript{3}S} (Detect--Calibrate--Constrain--Certify--Shield) converts per-step telemetry reliability into adaptive uncertainty inflation (RAC-Cert), solves dispatch against the widened set, repairs infeasible actions by projection, and emits a per-step auditable certificate. Third, the full battery manuscript hardens that core with subgroup calibration, asset-preservation proxies, blackout and safe-landing behavior, fleet composition, active probing, and a controller-in-the-loop runtime rehearsal. Cost-side results on Germany (OPSD, 17{,}377 rows) yield \PaperMetric{DE_COST_SAVINGS_PCT}{7.11}\% cost savings, \PaperMetric{DE_CARBON_REDUCTION_PCT}{0.30}\% carbon reduction, and \PaperMetric{DE_PEAK_SHAVING_PCT}{6.13}\% peak shaving; stochastic value remains positive in both DE and US regions (VSS\,=\,\PaperMetric{DE_VSS}{2,708.61} and \PaperMetric{US_VSS}{297,092.71}) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). The current package includes runtime-rehearsal evidence but stops short of claiming full physical deployment or production-hardware validation. We extend the battery-only DC\textsuperscript{3}S architecture to a \textbf{universal domain physical safety framework} spanning Energy (battery), AV (autonomous vehicles), Industrial (process control), and Healthcare (vital signs), with a shared five-stage pipeline (Detect--Calibrate--Constrain--Shield--Certify) and domain-specific adapters. Every quantitative claim is tied to versioned run artifacts and checked automatically against the release evidence.
\end{abstract}

\noindent\textbf{Keywords:} conformal prediction, battery dispatch, safety shield, telemetry reliability, robust optimization, cyber-physical systems, uncertainty quantification, multi-domain framework, universal physical safety
...
Seen against this literature, the paper's originality lies in elevating the gap between observed-state feasibility and true-state physical safety to a first-class systems problem in battery dispatch under degraded telemetry. CPSBench makes that gap measurable by maintaining separate observed and truth trajectories under realistic telemetry faults, while DC\textsuperscript{3}S addresses it through a modular runtime design that does not replace forecasting or optimization, but instead wraps the dispatch boundary with telemetry-reliability scoring, reliability-conditioned uncertainty inflation, action repair, and per-step certificate emission. The resulting contribution is therefore not a broader claim of universal runtime safety, but a bounded systems synthesis: measurement-quality-aware uncertainty, safe dispatch-time intervention, and auditable evidence for each online control decision.
...
The battery DC\textsuperscript{3}S architecture generalizes to a \textbf{universal domain physical safety framework} (ORIUS) that applies the same five-stage pipeline---Detect, Calibrate, Constrain, Shield, Certify---across five cyber-physical domains. This section describes the framework, domain adapters, and implementation status. Historical incidents---Air France 447 (pitot dropout), 737 MAX MCAS (sensor spike/drift), Tesla Autopilot (perception dropout), Texas City refinery (level sensor failure), and Uber ATG (sensor fusion failure)---illustrate the OASG across aviation, automotive, and industrial control; ORIUS addresses the common observation-reality integrity gap underlying these events.
...
The unified validation harness (\texttt{scripts/run\_universal\_orius\_validation.py}) runs all six ORIUS-Bench tracks---battery, navigation, industrial, healthcare, aerospace, vehicle---with fault episodes (bias, noise, stuck\_sensor, blackout) and aggregates TSVR (true-state violation rate) and OASG metrics. \Cref{tab:TBL09_CROSS_DOMAIN_OASG} reports the cross-domain OASG table from the locked validation run. All six domains pass the harness gate; battery and vehicle show non-zero baseline TSVR under faults, with ORIUS reducing or maintaining safety. The harness is part of the locked replication flow (\texttt{orius\_program\_manifest.json}) and produces \texttt{reports/universal\_orius\_validation/cross\_domain\_oasg\_table.csv}.
...
The universal framework is implemented in \texttt{src/orius/universal\_framework/} with domain registry, pipeline (\texttt{run\_universal\_step}), and adapters for all five domains: Energy (Battery), AV, Industrial, Healthcare/Surgical, and Aerospace. The battery domain remains the primary validated surface with locked CPSBench evidence; the six ORIUS-Bench tracks (including vehicle) provide multi-domain validation evidence as in \Cref{tab:TBL09_CROSS_DOMAIN_OASG}. Locked evidence and theorem validation apply to the battery domain; multi-domain extensions are validated via the unified harness.
...
This paper is intentionally scoped to the locked DE and US windows plus the battery-only hardening extensions added later in the manuscript. Generalization to different tariff regimes, different battery sizes, different telemetry infrastructures, or non-battery domains remains future work. The multi-BA US evidence surface improves external validity, but it does not convert this paper into a universal market study.
