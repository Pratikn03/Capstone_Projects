...
\title{ORIUS: A Universal Physical-Safety Framework Validated Across Battery Dispatch, Autonomous Vehicles, Industrial Control, and Medical Monitoring}
...
\begin{abstract}
ORIUS is a universal physical-safety framework for cyber-physical control under degraded observation. This manuscript validates one shared Detect--Calibrate--Constrain--Shield--Certify loop across four defended domains: battery dispatch, autonomous vehicles, industrial process control, and medical monitoring. The reference battery result remains the observation-action safety gap that motivated the framework: under realistic battery telemetry faults, deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. The same universal replay protocol now promotes three additional defended domains with locked non-synthetic telemetry, verified training surfaces, and software-in-loop passes. Under the canonical cross-domain gate, ORIUS reduces true-state violation rate from \textbf{65.3\% to 4.9\%} in autonomous-vehicle car following, from \textbf{44.5\% to 0.0\%} in industrial process control, and from \textbf{97.9\% to 0.0\%} in medical monitoring. Battery remains the reference domain for the locked economics and theorem-to-artifact lineage: on Germany (OPSD, 17{,}377 rows), the battery-domain DC3S implementation delivers \PaperMetric{DE_COST_SAVINGS_PCT}{7.11}\% cost savings, \PaperMetric{DE_CARBON_REDUCTION_PCT}{0.30}\% carbon reduction, and \PaperMetric{DE_PEAK_SHAVING_PCT}{6.13}\% peak shaving, while stochastic value remains positive in both DE and US regions (VSS\,=\,\PaperMetric{DE_VSS}{2,708.61} and \PaperMetric{US_VSS}{297,092.71}) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). Navigation remains shadow-synthetic and aerospace remains experimental. Every quantitative claim is tied to versioned run artifacts, theorem registers, and release gates.
\end{abstract}

\noindent\textbf{Keywords:} ORIUS, universal physical-safety framework, degraded observation, battery dispatch, autonomous vehicles, industrial control, medical monitoring, safety shield, telemetry reliability, cyber-physical systems
...
ORIUS treats that failure mode as a general cyber-physical systems problem rather than a battery-specific quirk. Whenever a controller acts on degraded observation of a physical state, an observation-action safety gap can emerge between what appears feasible on the observed channel and what is actually safe in the physical system. The framework contribution of this paper is therefore architectural as well as empirical: one shared Detect--Calibrate--Constrain--Shield--Certify loop instantiated across four defended domains in this release---battery dispatch, autonomous vehicles, industrial process control, and medical monitoring---with battery serving as the locked reference surface for economics and theorem-to-artifact lineage.
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{Universal defended-domain validation.} We present ORIUS as a universal physical-safety framework organized around one five-stage Detect--Calibrate--Constrain--Shield--Certify loop, and we defend that loop across four domains in this release: battery dispatch, autonomous vehicles, industrial process control, and medical monitoring.
  \item \textbf{Battery reference result.} We preserve the battery-domain hidden-gap result as the reference scientific witness: deterministic dispatch violates true SOC at 3.9\% of steps with P95 severity of 333\,MWh while appearing safe on observed state.
  \item \textbf{Shared cross-domain repair gate.} We promote AV, industrial, and healthcare from portability rows to defended domains by requiring locked telemetry, verified training surfaces, clean software-in-loop execution, a nontrivial baseline true-state violation gap, material ORIUS improvement, and stable seed behavior under one canonical replay harness.
  \item \textbf{Shared theorem package with defended-domain instantiations.} We keep one ORIUS theorem ladder and make its state, observation, safety, constraint, repair, code, and artifact instantiations explicit for battery, AV, industrial, and healthcare rather than treating non-battery domains as informal portability notes.
  \item \textbf{Artifact-locked universal evidence bundle.} The same validator that assigns evidence tiers now emits the manuscript-facing dataset-scope, training-metric, runtime, fault-severity, theorem-instantiation, and flagship-trace artifacts consumed by the paper.
\end{enumerate}

\subsection{Contribution Boundary}

The central scientific contribution is now a defended universal package rather than battery-plus-portability narrative. ORIUS is the reusable architectural framework; battery, AV, industrial, and healthcare are the defended instantiations. The manuscript still keeps a bounded formal layer consisting of the ORIUS theorem package together with defended-domain instantiation maps. \Cref{thm:rac_coverage} explains why monotone inflation preserves coverage under the shared uncertainty construction, \Cref{thm:conditional_coverage} motivates the reliability-group calibration audits, and \Cref{thm:safety_margin} with \Cref{cor:zero_violation} explain why reliability-conditioned margins and repair can eliminate true-state failures on the reference battery surface. These results validate the construction and are instantiated across four defended domains, but they still do not claim closed-loop stability, optimal inflation scheduling, or universal safety dominance outside the defended set. Navigation remains shadow-synthetic and aerospace remains experimental in this release.

\subsection{Paper Roadmap}

Sections~\ref{sec:related}--\ref{sec:problem} position ORIUS and define the battery reference setting that motivated the framework. Sections~\ref{sec:forecasting_uq} and~\ref{sec:safe_dispatch} describe the forecasting, calibration, and shield mechanics. Sections~\ref{sec:experiments}--\ref{sec:us_analysis} preserve the core battery evidence, baseline diagnosis, and regime analysis. \Cref{sec:universal} then presents the defended universal package: shared protocol, dataset scope, training metrics, runtime and SIL results, flagship traces, and theorem instantiations for battery, AV, industrial, and healthcare. The later sections extend that package through governance, hardening, benchmark formalization, and deployment discipline, and the closing sections restate the defended claim boundary explicitly: four defended domains in this release, with navigation and aerospace outside the equal-depth claim.
...
\Cref{tab:dataset_framework_trace} maps each locked dataset to its role in the pipeline and the primary evidence tables. DE and US\_MISO feed the canonical battery evaluation (TBL01, TBL04); PJM and ERCOT extend the release-family scope. The defended universal framework (\Cref{sec:universal}) now uses locked telemetry for AV, industrial, healthcare, and aerospace rows, plus a synthetic navigation shadow surface; the governed cross-domain evidence is summarized in \Cref{tab:defended-domain-dataset-scope} and \Cref{tab:defended-domain-runtime}.
...
\midrule
\multicolumn{4}{l}{\textit{Universal replay: battery + AV + industrial + healthcare defended; navigation shadow-synthetic; aerospace experimental}} \\
\bottomrule
\end{tabular}
\end{table}
...
This manuscript now treats ORIUS as a defended universal framework rather than a battery-only system with portability notes. The defended set in this release is four-domain: battery dispatch, autonomous vehicles, industrial process control, and medical monitoring. Battery remains the reference witness for the hidden-gap discovery, the locked economics, and the theorem-to-artifact lineage; AV, industrial, and healthcare now share the same training, runtime, software-in-loop, and promotion gates. Navigation remains a shadow-synthetic surface and aerospace remains experimental.
...
\subsection{Defended Domain Scope}

The defended universal claim is anchored by one canonical evidence generator, \texttt{scripts/run\_universal\_orius\_validation.py}, which emits the dataset cards, training metrics, runtime tables, fault-severity sweeps, theorem-instantiation tables, and flagship traces used in this section. The resulting scope table is the manuscript's governing universal contract.

\input{../reports/universal_orius_validation/tbl_defended_domain_dataset_scope.tex}

\subsection{Training and Calibration Surfaces}

The non-battery defended domains now expose battery-style forecasting and calibration artifacts rather than a single readiness bit. AV, industrial, and healthcare all have non-empty train/calibration/test splits, locked feature paths, model bundles, conformal artifacts, and publication-facing reports. Battery forecasting remains the locked reference surface in Sections~\ref{sec:forecasting_uq} and~\ref{sec:calibration}; the table below equalizes the three promoted non-battery domains against that standard.
...
The defended runtime contract is shared across the four domains: one fault protocol, one universal repair path, one promotion gate, and one artifact bundle. Battery remains the reference row from locked publication artifacts; AV, industrial, and healthcare are replay-backed defended domains under the same gate.

\input{../reports/universal_orius_validation/tbl_defended_domain_runtime.tex}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../reports/universal_orius_validation/fig_defended_domain_validation.png}
\caption{Defended-domain ORIUS validation under the canonical replay gate. Battery is the locked reference aggregate; AV, industrial, and healthcare are replay-backed defended domains under the same promotion contract.}
\label{fig:defended-domain-validation}
\end{figure*}

\input{../reports/universal_orius_validation/tbl_defended_domain_fault_severity.tex}

\IfFileExists{../reports/universal_orius_validation/tbl_defended_domain_sil.tex}{\input{../reports/universal_orius_validation/tbl_defended_domain_sil.tex}}{}

\subsection{Flagship Traces}

The universal claim is not carried by summary tables alone. Each promoted non-battery defended domain now includes a flagship true-versus-observed trace showing the safety variable seen by the controller, the corresponding true variable used for evaluation, and the intervention points introduced by the ORIUS shield.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../reports/universal_orius_validation/fig_av_flagship_trace.png}
\caption{Autonomous-vehicle flagship trace. The defended AV surface is longitudinal car following under degraded observation, with violations defined on true next-step speed and headway.}
\label{fig:av-flagship-trace}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../reports/universal_orius_validation/fig_industrial_flagship_trace.png}
\caption{Industrial flagship trace. ORIUS clamps unsafe power-setpoint commands while the true plant state is evaluated against the defended process envelope.}
\label{fig:industrial-flagship-trace}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../reports/universal_orius_validation/fig_healthcare_flagship_trace.png}
\caption{Medical-monitoring flagship trace. ORIUS escalates alerts when degraded observation obscures defended SpO$_2$/HR/RR safety conditions.}
\label{fig:healthcare-flagship-trace}
\end{figure*}

\subsection{Shared Theorem Package and Instantiations}

The ORIUS theorem ladder is shared across the defended domains; what changes by domain is the concrete state, observation channel, safety predicate, constraint set, repair operator, and the empirical artifact family used to confirm the instantiation. This keeps the theorem layer universal without duplicating T1--T8 four times.

\input{../reports/universal_orius_validation/tbl_defended_domain_theorem_instantiation.tex}

\subsection{Non-Defended Domains}

Navigation and aerospace remain in the canonical harness, but not in the defended claim set. Navigation is still shadow-synthetic because the current trace is synthetic rather than locked telemetry. Aerospace remains experimental because the current replay surface executes correctly but does not yet satisfy the defended-domain promotion contract. They remain valuable for adapter hardening and externalization planning, but they are intentionally outside the equal-depth universal claim made in this manuscript.
...
This paper is intentionally scoped as a defended universal ORIUS manuscript, not as a claim of universal dominance. The defended set is limited to battery dispatch, autonomous vehicles, industrial process control, and medical monitoring. Battery still carries the locked economic narrative and the deepest theorem-to-artifact lineage; 
...
\paragraph{The core message.} Trustworthy cyber-physical control requires the uncertainty model to degrade when the measurement channel degrades. In that sense, telemetry is not an implementation detail but part of the control problem itself. ORIUS turns that idea into a single Detect--Calibrate--Constrain--Shield--Certify loop with per-step auditable evidence, and this release now defends that loop across four domains rather than presenting battery as the only serious validation surface. Battery remains the reference witness for the hidden-gap discovery and the locked economic story; AV, industrial, and healthcare now supply defended replay, training, software-in-loop, and theorem-instantiation evidence under the same promotion gate.

\paragraph{Scope boundary.} What this paper establishes is a defended universal ORIUS package across battery dispatch, autonomous vehicles, industrial control, and medical monitoring, with navigation and aerospace intentionally held outside the equal-depth claim. What it does not establish is universal dominance across all domains, adversarial completeness, or production-hardware deployment safety. The defended claim is exactly the one supported by the artifact bundle: under degraded observation, the same ORIUS repair contract can expose and materially reduce true-state safety failures across four distinct cyber-physical domains.
