...
\usepackage{booktabs}
% \usepackage{longtable}  % disabled: incompatible with twocolumn
\usepackage{array}
...
\newcommand{\PaperFigureWide}[2]{%
  ...
  \centering
  \@ifundefined{PaperFigurePath@#1}{%
    \fbox{\parbox{0.9\textwidth}{\textbf{Missing figure token:} #1}}%
  }{%
    \IfFileExists{\csname PaperFigurePath@#1\endcsname}{%
      \includegraphics[width=0.95\textwidth]{\csname PaperFigurePath@#1\endcsname}%
    }{%
      \fbox{\parbox{0.9\textwidth}{\textbf{Missing figure file for token #1:} \csname PaperFigurePath@#1\endcsname}}%
    }%
  }%
  \caption{#2}
  \label{fig:#1}
...
\newcommand{\PaperFigureHero}[2]{%
  ...
  \centering
  \@ifundefined{PaperFigurePath@#1}{%
    \fbox{\parbox{0.95\textwidth}{\textbf{Missing figure token:} #1}}%
  }{%
    \IfFileExists{\csname PaperFigurePath@#1\endcsname}{%
      \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{\csname PaperFigurePath@#1\endcsname}%
    }{%
      \fbox{\parbox{0.95\textwidth}{\textbf{Missing figure file for token #1:} \csname PaperFigurePath@#1\endcsname}}%
    }%
  }%
  \caption{#2}
  \label{fig:#1}
...
\begin{abstract}
Battery dispatch controllers can look feasible on observed sensor state while simultaneously violating physical battery limits on true state when telemetry degrades---a hidden safety failure that standard evaluation protocols do not expose. We study this gap under four realistic fault conditions and find that deterministic dispatch violates true state-of-charge limits at \textbf{3.9\% of steps with P95 severity of 333\,MWh}, even while observed-state evaluation reports zero violations. The manuscript responds to that failure mode in three parts. First, \textbf{CPSBench} maintains separate truth and observed battery trajectories across dropout, delay-jitter, spikes, and out-of-order faults so that true-state safety becomes measurable rather than implicit. Second, \textbf{DC\textsuperscript{3}S} (Detect--Calibrate--Constrain--Certify--Shield) converts per-step telemetry reliability into adaptive uncertainty inflation (RAC-Cert), solves dispatch against the widened set, repairs infeasible actions by projection, and emits a per-step auditable certificate. Third, we show a \textbf{conditional conservatism} result: DC\textsuperscript{3}S reaches zero true-SOC violations with a 2.8\% intervention rate, meaning the shield becomes more conservative only when telemetry quality demands it. Cost-side results on Germany (OPSD, 17{,}377 rows) yield \PaperMetric{DE_COST_SAVINGS_PCT}{7.11}\% cost savings, \PaperMetric{DE_CARBON_REDUCTION_PCT}{0.30}\% carbon reduction, and \PaperMetric{DE_PEAK_SHAVING_PCT}{6.13}\% peak shaving; stochastic value remains positive in both DE and US regions (VSS\,=\,\PaperMetric{DE_VSS}{2,708.61} and \PaperMetric{US_VSS}{297,092.71}) across three US balancing authorities (EIA-930 MISO/PJM/ERCOT). All results are software-in-the-loop; hardware and field validation remain future work. Every quantitative claim is tied to versioned run artifacts and checked automatically against the release evidence.
\end{abstract}
...
\PaperFigureHero{FIG01_ARCHITECTURE}{GridPulse architecture with \DCThreeS{} at the dispatch boundary. Offline forecasting and calibration produce promoted model and interval artifacts; online DC\textsuperscript{3}S turns degraded telemetry into safe dispatch actions, certificates, and operator-visible audit records.}
...
\PaperTableCSV{TBL08_FORECAST_BASELINES}{Promoted six-model forecasting comparison across DE and US. All six model families (GBM, LSTM, TCN, N-BEATS, TFT, PatchTST) receive CQR-calibrated 90\% conformal prediction intervals. GBM achieves the narrowest intervals due to its superior point-forecast accuracy in this tabular regime.}
...
\textbf{Interpretation.} The promoted comparison surface preserves one common baseline protocol across all six model families. GBM dominates across all target--region combinations, consistent with the well-documented advantage of gradient-boosted trees on moderate-sized tabular data with strong hand-engineered features~\cite{borisov2022deep}. LSTM and TCN remain substantially weaker on wind and solar, exhibiting negative $R^2$ in several cases (i.e., worse than predicting the mean), which confirms that raw sequence-to-sequence architectures do not automatically outperform tabular methods under the current feature contract. N-BEATS, TFT, and PatchTST show intermediate but clearly inferior performance. All six model families receive CQR-calibrated 90\% conformal prediction intervals, generated from the held-out calibration split using the same conformalized quantile regression procedure. GBM's superior point-forecast accuracy translates into the narrowest prediction intervals; deep models, where enabled, show materially wider intervals, consistent with their higher residual variance in this tabular regime. Only the GBM intervals enter the operational dispatch pipeline (DC\textsuperscript{3}S), since GBM is the production-candidate forecaster; the deep-model intervals are reported for completeness and to enable fair cross-model UQ comparison.
...
\paragraph{Broader implications.} The observed-state safety illusion is not unique to battery dispatch. Any cyber-physical system that acts on sensor readings that may lag or drift from physical reality is exposed to the same class of hidden violations. The DC\textsuperscript{3}S pattern --- reliability-conditioned uncertainty inflation followed by feasible-set projection --- should therefore be read as a reusable systems idea for CPS settings such as HVAC dispatch, water treatment, and autonomous vehicle motion planning, wherever measurement uncertainty and hard safety constraints meet.
