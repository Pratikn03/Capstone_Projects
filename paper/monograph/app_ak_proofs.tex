...
\texttt{T5} & A certificate is valid for a finite horizon when the reachable tube remains inside the safe set and no invalidating event occurs. & A1--A5 plus reachable-tube containment & \texttt{src/orius/dc3s/temporal\_theorems.py}; \texttt{reports/publication/T5\_certificate\_horizon\_by\_fault.csv}; \texttt{theorem\_result\_cards/T5.json}.\\
\texttt{T8} & ORIUS weakly dominates blind persistence in true-state violations while retaining a stated fraction of useful work under the same fault trace. & A1--A7 plus paired-trace comparability & \texttt{src/orius/benchmarks/graceful\_degradation.py}; \texttt{reports/publication/T8\_graceful\_policy\_comparison.csv}; \texttt{theorem\_result\_cards/T8.json}.\\
\texttt{T9} and \texttt{T10} & Empty common safe core implies mandatory-release impossibility; two indistinguishable boundary states imply the TV lower bound. & Mandatory release, disjoint safe sets, two-state boundary model & \texttt{src/orius/universal\_theory/ambiguity.py}; \texttt{src/orius/universal\_theory/boundary\_indistinguishability.py}; theorem result cards.\\
\texttt{T11\_Byzantine} and \texttt{T\_stale\_decay} & Robust reliability aggregation is valid for $b<n/2$ trimmed channels; stale uncertainty grows under bounded drift. & Honest-channel interval bound; bounded latent drift & \texttt{src/orius/dc3s/quality.py}; \texttt{src/orius/universal\_theory/stale\_decay.py}; theorem result cards.\\
\texttt{L1--L4}, \texttt{T\_minimax}, \texttt{T\_sensor\_converse} & Runtime monotonicity lemmas, scoped finite ambiguity minimax, and sensor necessity under adapter semantics. & Narrow runtime-law assumptions and finite ambiguity class & \texttt{src/orius/universal\_theory/runtime\_laws.py}; \texttt{src/orius/universal\_theory/minimax\_boundary.py}; \texttt{src/orius/universal\_theory/sensor\_necessity.py}.\\
\bottomrule
\end{longtable}
