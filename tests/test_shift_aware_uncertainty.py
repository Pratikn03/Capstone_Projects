...
def test_config_aliases_supported() -> None:
    cfg = ShiftAwareConfig.from_mapping(
        {
            ...
            "coverage_window_size": 33,
            "runtime_state_path": "reports/publication/state.json",
    ...
    assert cfg.coverage_window_size == 33
    assert cfg.runtime_state_path == "reports/publication/state.json"
    assert cfg.validity_weights["residual"] == 0.4


def test_nested_config_mapping_supported() -> None:
    cfg = ShiftAwareConfig.from_mapping(
        {
            "enabled": True,
            "thresholds": {"watch": 0.8, "degraded": 0.6, "invalid": 0.4},
            "widening_caps": {"max_inflation_multiplier": 3.5},
            "artifact_output_toggles": {"enabled": False},
        }
    )
    assert cfg.watch_threshold == 0.8
    assert cfg.degraded_threshold == 0.6
    assert cfg.invalid_threshold == 0.4
    assert cfg.max_inflation_multiplier == 3.5
    assert cfg.artifact_toggles["enabled"] is False
...
def test_runtime_state_persists_without_module_globals(tmp_path) -> None:
    cfg = ShiftAwareConfig(
        enabled=True,
        aci_mode="aci_basic",
        runtime_state_path=str(tmp_path / "runtime_state.json"),
    )
    d1 = build_runtime_interval(
        y_hat=10.0,
        base_half_width=2.0,
        reliability_score=0.2,
        drift_flag=True,
        residual_features={"abs_residual": 3.0, "covered": False, "volatility": 0.9},
        config=cfg,
    )
    d2 = build_runtime_interval(
        y_hat=10.0,
        base_half_width=2.0,
        reliability_score=0.2,
        drift_flag=True,
    ...
    assert d2.adaptive_quantile >= d1.adaptive_quantile
...
def test_subgroup_window_resize_recomputes_stats() -> None:
    ...
    tracker.update(group_key=key, covered=True, interval_width=2.0, abs_residual=1.0)
    tracker.set_window_size(2)
    stats = tracker.group_rows()[0]
    assert stats["count"] == 2
    assert stats["miss_count"] == 0
