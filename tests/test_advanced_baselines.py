    ...
    return pd.DataFrame(
        {
            ...
            "temperature": 15
            + 10 * np.sin(2 * np.pi * np.arange(n_samples) / 365)
...
class TestProphetBaseline:
    ...
    def test_prophet_baseline_creation(self, sample_timeseries_data):
        ...
        config = ProphetConfig(target="load_mw", yearly_seasonality=True, daily_seasonality=True)
    ...
    def test_prepare_prophet_df_accepts_duplicate_prophet_labels(self, sample_timeseries_data):
        ...
        assert np.allclose(
            prepared["y"].to_numpy(dtype=float), sample_timeseries_data.head(16)["y"].to_numpy(dtype=float)
        )
...
class TestBaselinesBeatPersistence:
    """Functional check: advanced baselines must beat naive persistence."""

    def test_prophet_rmse_below_persistence(self, sample_timeseries_data):
        try:
        ...
        train = sample_timeseries_data.iloc[:400].copy()
        test = sample_timeseries_data.iloc[400:].copy()
        model = ProphetBaseline(ProphetConfig(target="load_mw", uncertainty_samples=200))
        model.fit(train)
        prophet_pred = model.predict(test)
        persistence = test["load_mw"].shift(24).bfill().to_numpy()
        rmse_prophet = float(np.sqrt(np.mean((test["load_mw"].to_numpy() - prophet_pred[: len(test)]) ** 2)))
        rmse_persistence = float(np.sqrt(np.mean((test["load_mw"].to_numpy() - persistence) ** 2)))
        assert rmse_prophet < rmse_persistence, (
            f"Prophet rmse={rmse_prophet:.2f} did not beat persistence rmse={rmse_persistence:.2f}"
        )
