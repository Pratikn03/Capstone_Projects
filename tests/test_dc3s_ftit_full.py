...
class TestPreviewFaultState:
    ...
    def test_alpha_weights(self):
        heavy = preview_fault_state(
            adaptive_state={}, fault_flags={"dropout": True, **{k: False for k in FTIT_FAULT_KEYS if k != "dropout"}},
...
class TestUpdate:
    ...
    def test_degraded_state_narrower_tube(self):
        ...
        noisy = update(adaptive_state=clean["adaptive_state"], fault_flags=_NOISY, constraints=_constraints(), cfg=_cfg())
    ...
    def test_e_t_bounded(self):
        state = {}
        for _ in range(50):
            out = update(adaptive_state=state, fault_flags=_NOISY, constraints=_constraints(), cfg=_cfg(e_max_mwh=5.0))
            state = out["adaptive_state"]
        assert out["e_t_mwh"] <= 5.0 + 1e-9

    def test_e_t_min_floor(self):
        out = update(adaptive_state={}, fault_flags=_CLEAN, constraints=_constraints(), cfg=_cfg(e_min_mwh=2.0))
        assert out["e_t_mwh"] >= 2.0 - 1e-9

    def test_tube_collapse_to_midpoint(self):
        out = update(
            adaptive_state={"ftit": {"e_t_mwh": 50.0}},
            fault_flags=_NOISY, constraints=_constraints(min_soc_mwh=10.0, max_soc_mwh=90.0),
    ...
    def test_sigma2_tracking(self):
        out = update(
            adaptive_state={}, fault_flags=_CLEAN, constraints=_constraints(),
            cfg=_cfg(sigma2_init=1.0, sigma2_decay=0.95), sigma2_observation=10.0,
        )
    ...
    def test_sigma2_floor(self):
        out = update(
            adaptive_state={}, fault_flags=_CLEAN, constraints=_constraints(),
            cfg=_cfg(sigma2_floor=0.5), sigma2_observation=0.0,
        )
    ...
    def test_stale_tracker_passthrough(self):
        ...
        out = update(
            adaptive_state={}, fault_flags=_CLEAN, constraints=_constraints(),
            cfg=_cfg(), stale_tracker=tracker,
        )
    ...
    def test_gamma_power_shapes_curve(self):
        partial = {"dropout": True, "stale_sensor": False, "delay_jitter": False, "out_of_order": False, "spikes": False}
        linear = update(adaptive_state={}, fault_flags=partial, constraints=_constraints(), cfg=_cfg(gamma_power=1.0, alpha_dropout=0.5))
        quadratic = update(adaptive_state={}, fault_flags=partial, constraints=_constraints(), cfg=_cfg(gamma_power=2.0, alpha_dropout=0.5))
    ...
    def test_all_output_keys_present(self):
        out = update(adaptive_state={}, fault_flags=_CLEAN, constraints=_constraints(), cfg=_cfg())
        required = {"adaptive_state", "w_t", "p_drop", "p_stale", "p_delay", "p_ooo", "p_spike",
                     "gamma_mw", "e_t_mwh", "soc_tube_lower_mwh", "soc_tube_upper_mwh", "sigma2"}
        assert required <= set(out.keys())

    def test_custom_constraints(self):
        out = update(
            adaptive_state={}, fault_flags=_CLEAN,
