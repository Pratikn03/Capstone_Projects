...
class TestPrometheusMetrics:
    ...
    def test_request_counter_increment(self):
        ...
        try:
            ...
            initial = REQUESTS_TOTAL.labels(method="GET", endpoint="/test", status="200")._value.get()

            # Increment
            REQUESTS_TOTAL.labels(method="GET", endpoint="/test", status="200").inc()

            # Verify increment
            new_value = REQUESTS_TOTAL.labels(method="GET", endpoint="/test", status="200")._value.get()
    ...
    def test_histogram_observation(self):
        ...
        try:
            ...
            FORECAST_DURATION.labels(model="lightgbm", target="load_mw", region="DE").observe(0.5)

            FORECAST_DURATION.labels(model="lightgbm", target="load_mw", region="DE").observe(0.7)
...
class TestMetricsUpdateFunctions:
    ...
    def test_update_cost_savings_metrics(self):
        ...
        try:
            ...
            update_cost_savings_metrics(region="DE", savings_eur=2708.61, savings_pct=7.11)

            # Verify the gauge was set
            value = OPTIMIZATION_COST_SAVINGS.labels(region="DE")._value.get()
    ...
    def test_update_streaming_metrics(self):
        ...
        try:
            ...
            update_streaming_metrics(topic="orius-telemetry", partition=0, lag=100)

            # Verify the gauge was set
            value = KAFKA_CONSUMER_LAG.labels(topic="orius-telemetry", partition="0")._value.get()
            assert value == 100
        except ImportError:
            pytest.skip("prometheus_metrics module not available")
    ...
    def test_update_drift_metrics(self):
        ...
        try:
            ...
            update_drift_metrics(feature="load_mw", drift_detected=True, drift_score=0.85)
...
class TestMetricsEndpoint:
    ...
    def test_custom_metrics_in_output(self):
        ...
        try:
            ...
            REQUESTS_TOTAL.labels(method="GET", endpoint="/metrics-test", status="200").inc()
        ...
        try:
            ...
            for endpoint in ["/api/v1/forecast", "/api/v1/optimize", "/api/v1/anomaly"]:
                for status in ["200", "400", "500"]:
                    REQUESTS_TOTAL.labels(method="GET", endpoint=endpoint, status=status).inc()
                ...
                try:
                    for _ in range(100):
                        REQUESTS_TOTAL.labels(
                            method="GET", endpoint="/thread-test", status=random.choice(["200", "201", "202"])
                        ).inc()
