...
def test_boost_coverage_train_and_pipeline(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    _write_yaml(
        repo / "configs" / "train_forecast.yaml",
        {
            "features": {
            ...
            "reports": {"out_dir": str(repo / "reports")},
            "models": {
    ...
    monkeypatch.setattr(
        subprocess, "run", lambda *args, **kwargs: subprocess.CompletedProcess(args, 0, stdout="", stderr="")
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["run.py", "--steps", "research", "--run-id", "test-cov-boost", "--research-datasets", "de"],
    )
