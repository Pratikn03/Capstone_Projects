...
""".strip(),
    ...
    def fake_run_step(*, name, cmd, logs_dir, env=None, timeout_s=None):  # noqa: ANN001
        ...
        elif name == "audit_leakage":
            output = json.dumps({"fail": False, "violations": []})
        elif name == "audit_code_health":
            output = json.dumps({"fail": False, "violations": []})
    ...
    monkeypatch.setattr(
        ...
        lambda _db_path: {"ack_success_rate": 1.0, "hold_rate": 0.0, "db_exists": True, "acks_total": 10, "holds_total": 0, "commands_total": 10},
    )
    monkeypatch.setattr(
        fpa,
        "_compute_dc3s_typed_readiness",
        lambda _db, _table: {"exists": True, "rows_total": 10, "null_ratios": {"intervened": 0.0, "intervention_reason": 0.0, "reliability_w": 0.0, "drift_flag": 0.0, "inflation": 0.0}},
    )
...
""".strip(),
    ...
    (tmp_path / "reports" / "monitoring_summary.json").write_text(
        json.dumps({"data_drift": {"drift": False}, "model_drift": {"decision": {"drift": False}}, "dc3s_health": {"triggered": False}}),
        encoding="utf-8",
    )
    ...
    def fake_run_step(*, name, cmd, logs_dir, env=None, timeout_s=None):  # noqa: ANN001
        ...
        if name == "audit_git_delta":
            (tmp_path / "reports" / "publish" / "github_delta_report.json").write_text(
                json.dumps({"summary": {"total_changed_files": 1, "in_scope_files": 1, "out_of_scope_files": 0, "added_lines": 1, "deleted_lines": 0}}),
                encoding="utf-8",
            )
        if name == "audit_figure_inventory":
            (tmp_path / "reports" / "publish" / "figure_inventory.json").write_text(
                json.dumps({"summary": {"files_total": 1, "critical_missing": 0, "critical_zero_size": 0, "critical_ok": True}}),
                encoding="utf-8",
            )
        output = json.dumps({"ok": True}) if name in {"refresh_data_delta", "audit_na_tables", "audit_leakage", "audit_code_health"} else ""
        if name == "iot_sim_nominal":
            output = json.dumps({"safety_violations": 0, "certificate_completeness_rate": 1.0})
        step = fpa.StepResult(name=name, ok=True, return_code=0, duration_s=0.1, command=cmd, log_path=str(log_path))
        return step, output

    monkeypatch.setattr(fpa, "_run_step", fake_run_step)
    monkeypatch.setattr(fpa, "_compute_iot_ack_hold_metrics", lambda _db_path: {"ack_success_rate": 1.0, "hold_rate": 0.0})
    monkeypatch.setattr(fpa, "_compute_dc3s_typed_readiness", lambda _db, _table: {"exists": True, "rows_total": 1, "null_ratios": {}})
    monkeypatch.setattr(
        sys,
        "argv",
        [
