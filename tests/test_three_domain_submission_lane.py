...
def test_three_domain_scorecard_row_is_green() -> None:
    rows = {
        ...
        for row in csv.DictReader(
            (REPO_ROOT / "reports" / "publication" / "orius_submission_scorecard.csv").open()
        )
    }
...
def test_promoted_lane_artifacts_are_sanitized_and_healthcare_promoted() -> None:
    ...
    closure_rows = {
        row["domain"]: row
        for row in csv.DictReader(
            (REPO_ROOT / "reports" / "publication" / "orius_domain_closure_matrix.csv").open()
        )
    }
...
def test_runtime_denominator_surfaces_are_claim_governing_and_proxy_is_secondary() -> None:
    ...
    evidence_rows = {
        row["domain"]: row
        for row in csv.DictReader(
            (REPO_ROOT / "reports" / "publication" / "chapters40_44_domain_evidence_register.csv").open()
        )
    }
    proxy_rows = {
        row["domain"]: row
        for row in csv.DictReader(
            (REPO_ROOT / "reports" / "publication" / "three_domain_proxy_runtime_comparison.csv").open()
        )
    }
...
def test_runtime_governing_metrics_match_runtime_summaries() -> None:
    ...
    benchmark_rows = {
        row["domain"]: row
        for row in csv.DictReader(
            (REPO_ROOT / "reports" / "publication" / "three_domain_ml_benchmark.csv").open()
        )
    }
    closure_rows = {
        row["domain"]: row
    ...
    for display_name, runtime_summary in promoted_domains.items():
        ...
        assert "runtime-governed" in benchmark["note"] or "runtime-governed" in benchmark["note"].replace(
            "runtime-governing", "runtime-governed"
        )
...
def test_healthcare_benchmark_uses_runtime_emitted_certificate_metrics() -> None:
    ...
    assert (
        healthcare["certificate_valid_release_rate_semantics"]
        == "runtime_witness_certificate_valid_rate_promoted_healthcare_row"
    )
    assert healthcare["runtime_source"] == "reports/healthcare/runtime_summary.csv"
    assert (
        healthcare["certificate_valid_release_rate"]
        == f"{float(witness_summary['domains']['healthcare']['certificate_valid_rate']):.6f}"
    )
...
def test_av_and_healthcare_emit_domain_runtime_contract_witnesses() -> None:
    ...
    for domain, (path, contract_id) in trace_specs.items():
        with path.open() as handle:
            ...
            sample = (
                first
                if first["controller"] == "orius"
                else next(row for row in reader if row["controller"] == "orius")
            )
        ...
        if domain == "av":
            ...
            runtime_summary = {
                row["controller"]: row
                for row in csv.DictReader(
                    (REPO_ROOT / "reports" / "healthcare" / "runtime_summary.csv").open()
                )
            }
...
def test_non_degenerate_runtime_comparators_remain_visible() -> None:
    ...
    assert float(av_summary_rows["orius"]["useful_work_total"]) > float(
        av_summary_rows["always_brake"]["useful_work_total"]
    )

    assert float(healthcare_summary_rows["orius"]["tsvr"]) == 0.0
    assert float(healthcare_summary_rows["always_alert"]["tsvr"]) == 0.0
    assert float(healthcare_summary_rows["orius"]["max_alert_rate"]) < float(
        healthcare_summary_rows["always_alert"]["max_alert_rate"]
    )
