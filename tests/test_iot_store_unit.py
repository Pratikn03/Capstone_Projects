...
def test_enqueue_command(store):
    ...
    store.enqueue_command(
        device_id="d1", zone_id="DE", command_id=cmd_id, command={"setpoint": 200}, certificate_id="cert-123"
    )
...
def test_record_ack(store):
    ...
    store.record_ack(
        device_id="d1", command_id=cmd_id, status="success", reason="ok", payload={"acked": True}
    )
...
def test_hold_state(store):
    # Setup state
    store.record_telemetry(
        device_id="d1", ts_utc="2023-01-01", payload={}, reliability_w=1.0, reliability_flags={}
    )
