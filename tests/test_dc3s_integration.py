...
def _run_pipeline(events, inject_action=None):
    ...
    for i, event in enumerate(events):
        ...
        uset = {
            "meta": {"w_t": w_t, "drift_flag": drift_flag, "inflation": cal_meta["inflation"]},
            "lower": lo.tolist(),
            "upper": hi.tolist(),
        }
        safe, shield_meta = repair_action(proposed, {"current_soc_mwh": soc}, uset, constraints, _SHIELD_CFG)
        ok, reasons, proj_soc = evaluate_guarantee_checks(
            current_soc=soc, action=safe, constraints=constraints
        )
        cert = make_certificate(
            ...
            guarantee_checks_passed=ok,
            guarantee_fail_reasons=reasons,
        )
...
class TestStatePersistence:
    def test_state_store_round_trip(self, tmp_path):
        ...
        store.upsert(
            ...
            last_yhat=50000.0,
            last_y_true=51000.0,
            drift_state={"count": 50},
            adaptive_state={"ftit": {"n": 25.0}},
